...
begin
  if containsGarbageCollectedRef(v.typ) or (v.ast = nil) then
    ...
    if not (skipVarGenericRange(v.typ).Kind in [tyArray, tyArrayConstr, tySet,
      ...
      appf(p.s[cpsInit], '$1 = 0;$n', [v.loc.r])
    else
      appf(p.s[cpsInit], 'memset((void*)&$1, 0, sizeof($1));$n',
        [v.loc.r])
end;
  ...
  for i := 0 to sonsLen(t)-1 do begin
    ...
    if (c.ast <> nil) and (c.typ.kind in ConstantDataTypes) and
      ...
      if sfImportc in c.flags then
        appf(currMod.s[cfsData], 'extern $1$2 $3;$n',
...
begin
  ...
  p.blocks[len].id := p.unique; // positive because we use it right away:
...
begin
  ...
  if t.sons[0] <> nil then begin
    ...
    appf(p.s[cpsStmts], getRaiseFrmt(),
...
begin
  ...
  for i := 1 to sonsLen(t) - 1 do begin
    ...
    if t.sons[i].kind = nkOfBranch then
      ...
      // else statement
      appf(p.s[cpsStmts], 'goto L$1;$n', [toRope(p.unique)]);
  end;
...
begin
  ...
  useMagic('E_Base');
  useMagic('excHandler');
  appf(p.s[cpsLocals], 'volatile TSafePoint $1;$n', [safePoint]);
