...
begin
  ...
  for i := 1 to sonsLen(t.n)-1 do begin
    ...
    while arr.Kind = tyOpenArray do begin // need to pass hidden parameter:
      appf(params, ', const int $1Len$2', [param.loc.r, toRope(j)]);
      inc(j);
      arr := arr.sons[0]
    end;
  ...
  case typ.kind of
    tySequence, tyRecord, tyObject: begin
      ...
      if not isImportedType(typ) then
        appf(currMod.s[cfsForwardTypes],
...
begin
  if (rectype.sym <> nil)
  and ([sfImportc, sfExportc] * rectype.sym.flags <> []) then begin
    result := field.loc.r;
...
begin
  ...
  case n.kind of
        ...
        case n.sons[i].kind of
          nkOfBranch, nkElse: begin
            ...
            if m.kind <> nkSym then begin
              ...
              a := genRecordFieldsAux(m, ropef('$1.$2', [ae, sname]),
                                      rectype);
              if a <> nil then begin
                app(result, 'struct {');
                app(result, a);
    ...
    nkSym: begin
      ...
      appf(result, '$1 $2;$n', [getTypeDesc(field.loc.t), sname])
    end;
...
begin
  ...
  case typ.Kind of
    ...
    tyArrayConstr, tyArray: begin
      ...
      IdTablePut(currMod.typeCache, typ, result);
      if not isImportedType(typ) then
        appf(currMod.s[cfsTypes], 'typedef $1 $2[$3];$n',
    ...
    tySet: begin
      case int(getSize(typ)) of
        ...
        8: result := toRope('NS64');
          ...
          IdTablePut(currMod.typeCache, typ, result);
          if not isImportedType(typ) then
            appf(currMod.s[cfsTypes], 'typedef NS8 $1[$2];$n',
