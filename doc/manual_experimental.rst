...
It's a different implementation than ``notnil``: ``strictNotNil``. Keep in mind to be careful with distinguishing 
...
Currently we have ``Safe``, ``MaybeNil`` and ``Nil`` : we use internally ``Parent`` but this is an implementation detail(a parent layer has the actual nilability).
...
This is not great, as default values for non-nilables and nilables are usually actually ``nil`` , so we should think a bit more about this section.
...
direct stop flow rules
-------------------------

Stop keywords as ``return``, ``break``, ``continue``, ``raise`` mean that we jump from a branch out.
