...
teffects1.nim(38, 21) Error: type mismatch: got <proc (x: int): string{.noSideEffect, gcsafe.}> but expected 'MyProcType = proc (x: int): string{.closure.}'
...
var p: MyProcType = foo #[tt.Error
                    ^
type mismatch: got <proc (x: int): string{.noSideEffect, gcsafe.}> but expected 'MyProcType = proc (x: int): string{.closure.}'
