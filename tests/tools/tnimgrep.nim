import osproc, os, streams, unittest
...
template nimgrep(optsAndArgs): untyped =
  process = startProcess("nimgrep " & optsAndArgs, options = {poEvalCommand})
...
suite "nimgrep":
  test "`--matchFile` with matching file":
    nimgrep "-r --matchFile:CONTEXTPAT PATTERN"
    check ngExitCode == 0
...
"""
  ...
  test "`--includeFile` with matching file":
    nimgrep "-r --includeFile:KJKJHSFSFKASHFBKAF PATTERN"
    check ngExitCode == 0
    check ngStdErr.len == 0
    check ngStdOut == """
...
"""
  ...
  test "`--includeDir` with matching dir":
    nimgrep "-r --includeDir:.hid PATTERN"
    check ngExitCode == 0
    check ngStdErr.len == 0
    check ngStdOut == """
...
"""
  ...
  test "`--includeDir` with matching grandparent path segment":
    nimgrep "-r --includeDir:a PATTERN"
    check ngExitCode == 0
    check ngStdErr.len == 0
    check ngStdOut == """
0 matches
...
"""
  ...
  test "`--inContext` with missing context option":
    # Using `--inContext` implies one of the context options are used.
    nimgrep "-r --inContext:CONTEXTPAT PATTERN"
    check ngExitCode == 1
    check ngStdErr.len == 0
#     check ngStdOut == """
# SOME ERROR NEEDS TO GO HERE
# """
...
"""
  ...
  test "`--!inContext` with PAT matching some contexts":
    nimgrep "--context:2 --!inContext:CONTEXTPAT PATTERN context_match_filtering"
    check ngExitCode == 0
    check ngStdErr.len == 0
    check ngStdOut == """
...
"""
  ...
  test "`--!inContext` with PAT not matching any of the contexts":
    nimgrep "--context:1 --!inContext:CONTEXTPAT PATTERN context_match_filtering"
    check ngExitCode == 0
    check ngStdErr.len == 0
    check ngStdOut == """
context_match_filtering:3  -
