...
when defined(windows):
  from winlean import WSAECONNRESET
else:
  ...
  while true:
    ...
    if not await withTimeout(sendFut, timeout):
      ...
      if timeoutFut.failed:
        ...
        # The behaviour differs across platforms. On Windows WSAECONNRESET is
        # raised which is considered a "diconnection error", hence we overwrite
        ...
        # necessary to not raise when `SocketFlag.SafeDisconn` is passed (TODO)
        when defined(windows):
