...
expression: bar(fn1)
tproc_mismatch.nim(40, 8) Error: type mismatch: got <proc (){.inline, noSideEffect, gcsafe.}> but expected 'proc (){.closure.}'
  Calling convention mismatch: got '{.inline.}', but expected '{.closure.}'.
tproc_mismatch.nim(45, 8) Error: type mismatch: got <proc ()> but expected 'proc (){.closure, noSideEffect.}'
