...
proc getMimeTypeOfFile(pathToFile): string =
...
proc createMessage*(mSubject, mBody: string, mFrom:string, mTo, mCc: seq[string],
  ...
  result.boundary = "ThisShouldBeARandomString"
...
when not defined(testing) and isMainModule:
  ...
  proc main() {.async.} =
    ...
    var client = newAsyncSmtp("mail.klangfragment.de", Port(587), false)
    await client.connect()
    await client.auth("david@klangfragment.de", "Cibercom181")

    var msg = createMessage("Hello from Nim's SMTP!",
                            "Hello!!!!.\n Is this awesome or what? \n\nGreetings from Germany!\nenthus1ast",
                            "david@klangfragment.de",
                            @["kekschaot@gmail.com"])

    # png test
    msg.attach(r"C:\Users\dkrause\Desktop\test1.png",
               r"test1.png",
              "fooobaabaz"
              )

    # pdf test
    msg.attach(r"C:\Program Files (x86)\GnuWin32\man\pdf\wget-man.pdf",
               r"wget-man.pdf",
              "fooobaabaz"
              )
    ...
    ######await client.sendMail("david@klangfragment.de", @["dominik@picheta.me"], $msg)
    await client.sendMail("david@klangfragment.de", @["kekschaot@gmail.com"], $msg)
    # await client.sendMail("david@klangfragment.de", @["david@klangfragment.de"], $msg)
