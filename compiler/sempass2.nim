...
proc track(tracked: PEffects, n: PNode) =
  ...
  of nkProcDef, nkConverterDef, nkMethodDef, nkIteratorDef, nkLambda, nkFuncDef:
    if n[0].kind == nkSym and n[0].sym.ast != nil:
      trackInnerProc(tracked, getBody(tracked.graph, n[0].sym))
  of nkTypeSection, nkMacroDef, nkTemplateDef, nkDo:
    # XXX see what 'nkDo' means
    discard
  of nkCast:
    if n.len == 2:
    ...
    for i in 1..<params.len:
      ...
      if isSinkTypeForParam(typ) or
          (t.config.selectedGC in {gcArc, gcOrc} and
            (isClosure(typ.skipTypes(abstractInst)) or maybeWrappedInClosure(t, typ)) or param.id in t.escapingParams):
