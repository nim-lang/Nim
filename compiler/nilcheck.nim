...
proc checkCall(n, ctx, map): Check =
  ...
  for i, child in n:
    ...
    if i > 0:
      ...
      if child.kind == nkHiddenAddr and not child.typ.isNil and child.typ.kind == tyVar and child.typ.baseType.kind == tyRef:
        if not isNew:
...
proc checkNil*(s: PSym; body: PNode; conf: ConfigRef, idgen: IdGenerator) =
  ...
  map.store(context, resultExprIndex, if not s.typ.baseType.isNil and s.typ.baseType.kind == tyRef: Nil else: Safe, TResult, s.ast.info)
  ...
  if not s.typ.baseType.isNil and s.typ.baseType.kind == tyRef and tfNotNil in s.typ.baseType.flags:
