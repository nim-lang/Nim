...
proc processSwitch*(switch, arg: string, pass: TCmdLinePass, info: TLineInfo;
  ...
  of "nimcache":
    expectArg(conf, switch, arg, pass, info)
    var arg = arg
    # otherwise `--os:windows` messes up with `--nimcache` set in config nims files, e.g. via:
    # import os; switch("nimcache", "/tmp/somedir")
    if conf.target.targetOS == osWindows and DirSep == '/' and arg.startsWith '\\': arg = arg.replace('\\', '/')
