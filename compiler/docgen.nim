...
proc genComment(d: PDoc, n: PNode): string =
  ...
  if n.getComment.len > 0:
    renderRstToOut(d[], parseRst(n.getComment, toFullPath(d.conf, n.info),
...
proc genRecCommentAux(d: PDoc, n: PNode): Rope =
  ...
  if result == nil:
    ...
    when defined(nimNoNilSeqs): n.setComment("")
    else: n.comment = nil
...
proc prepareExamples(d: PDoc; n: PNode) =
  ...
  docComment.setComment "autogenerated by docgen from " & loc
...
proc generateTags*(d: PDoc, n: PNode, r: var Rope) =
  case n.kind
  of nkCommentStmt:
    if startsWith(n.commentStmt, "##"):
      let stripped = n.commentStmt.substr(2).strip
      r.add stripped
  of nkProcDef:
    when useEffectSystem: documentRaises(d.cache, n)
