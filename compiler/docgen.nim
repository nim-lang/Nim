...
proc genComment(d: PDoc, n: PNode): string =
  ...
  if n.kind == nkCommentStmt and n.len == 1 and n[0].strVal.len > 0:
    renderRstToOut(d[], parseRst(n[0].strVal, toFullPath(d.conf, n.info),
                               toLinenumber(n.info), toColumn(n.info),
                               dummyHasToc, d.options, d.conf), result)
  elif n.kind == nkExportDoc and n.len == 3 and
      n[2].kind in {nkStrLit..nkTripleStrLit} and n[2].strVal.len > 0:
    renderRstToOut(d[], parseRst(n[2].strVal, toFullPath(d.conf, n.info),
    toLinenumber(n.info), toColumn(n.info),
    dummyHasToc, d.options, d.conf), result)
...
proc genRecCommentAux(d: PDoc, n: PNode): Rope =
  ...
  #else:
  #  when defined(nimNoNilSeqs): n.strVal = ""
  #  else: n.strVal = nil
...
proc getPlainDocstring(n: PNode): string =
  ...
  result = ""
  if n == nil: return
  if n.kind in {nkExportDoc, nkCommentStmt} and n.len > 0 and
       n.lastSon.kind in {nkStrLit..nkTripleStrLit} and startsWith(n.lastSon.strVal, "##"):
    result = n.lastSon.strVal
  if result.len < 1:
...
proc prepareExamples(d: PDoc; n: PNode) =
  ...
  docComment.add newStrNode("autogenerated by docgen from " & loc, n.info)
...
proc generateTags*(d: PDoc, n: PNode, r: var Rope) =
  case n.kind
  of nkCommentStmt:
    if startsWith(n[0].strVal, "##"):
      let stripped = n[0].strVal.substr(2).strip
      r.add stripped
  of nkProcDef:
    when useEffectSystem: documentRaises(d.cache, n)
