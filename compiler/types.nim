...
proc isPureObject*(typ: PType): bool =
  var t = typ
  while t.kind == tyObject and t.baseType != nil:
    t = t.baseType.skipTypes(skipPtrs)
...
proc elemType*(t: PType): PType =
  assert(t != nil)
  case t.kind
  of tyGenericInst, tyDistinct, tyAlias, tySink, tyArray: result = elemType(t.baseType)
  of tyError: result = t
  else: result = t.baseType
  assert(result != nil)
...
proc isOrdinalType*(t: PType, allowEnumWithHoles: bool = false): bool =
  ...
  result = (t.kind in baseKinds and (not t.enumHasHoles or allowEnumWithHoles)) or
    (t.kind in parentKinds and isOrdinalType(t.baseType, allowEnumWithHoles))
...
proc iterOverTypeAux(marker: var IntSet, t: PType, iter: TTypeIter,
  ...
  if not containsOrIncl(marker, t.id):
    ...
    of tyGenericInst, tyGenericBody, tyAlias, tySink, tyInferred:
      result = iterOverTypeAux(marker, t.baseType, iter, closure)
    else:
      result = iterOverTypeAux(marker, t.baseType, iter, closure)
...
proc searchTypeForAux(t: PType, predicate: TTypePredicate,
  ...
  of tyObject:
    if t.baseType != nil:
      result = searchTypeForAux(t.baseType.skipTypes(skipPtrs), predicate, marker)
    if not result: result = searchTypeNodeForAux(t.n, predicate, marker)
  of tyGenericInst, tyDistinct, tyAlias, tySink, tySet:
    result = searchTypeForAux(t.baseType, predicate, marker)
  of tyArray, tyTuple:
    result = searchTypeForAux(t.baseType, predicate, marker)
    if result: return
    for a in t.argTypes:
...
proc isObjectWithTypeFieldPredicate(t: PType): bool =
  result = t.kind == tyObject and t.baseType == nil and
...
proc analyseObjectWithTypeFieldAux(t: PType,
  ...
  of tyGenericInst, tyDistinct, tyAlias, tySink, tyArray:
    result = analyseObjectWithTypeFieldAux(t.baseType, marker)
  of tyTuple:
    for a in t.argTypes:
      res = analyseObjectWithTypeFieldAux(a, marker)
      if res != frNone:
        return frEmbedded
  else:
    discard
  ...
  of tyRef, tyPtr, tyUncheckedArray:
    if t.kind == tyRef or hasTrace:
      if withRef and sameBackendType(t, orig):
        ...
        result = canFormAcycleAux(g, marker, t.baseType, orig, withRef or t.kind != tyUncheckedArray, hasTrace)
        if result: return
  of tyObject:
    if withRef and sameBackendType(t, orig):
      result = true
      ...
      if op != nil and sfOverridden in op.flags:
        hasTrace = true
      result = canFormAcycleAux(g, marker, t.baseType, orig, withRef, hasTrace)
      if result: return
...
proc typeToString(typ: PType, prefer: TPreferedDesc = preferName): string =
  ...
  proc typeToString(typ: PType, prefer: TPreferedDesc = preferName): string =
    ...
    if prefer in preferToResolveSymbols and t.sym != nil and
      ...
      if t.kind == tyInt and isIntLit(t):
      ...
      elif t.kind == tyAlias and t.baseType.kind != tyAlias:
        result = typeToString(t.baseType)
        ...
        of tyCompositeTypeClass:
          ...
          result = typeToString(t.bodyType.bodyType)
        else:
          result = t.sym.name.s
        ...
        result = t.sym.name.s
        if t.kind == tyGenericParam and t.argTypesLen > 0:
          result.add ": "
          var first = true
          for son in t.argTypes:
    ...
    of tyGenericBody:
      result = typeToString(t.bodyType) & '['
      var i = 0
      for a in t.argTypesWithoutBody:
        if i > 0: result.add(", ")
        result.add(typeToString(a, preferTypeName))
        inc i
      result.add(']')
    of tyTypeDesc:
      if t.baseType == nil or t.baseType.kind == tyNone: result = "typedesc"
      else: result = "typedesc[" & typeToString(t.baseType) & "]"
    of tyStatic:
      if prefer == preferGenericArg and t.n != nil:
        result = t.n.renderTree
      else:
        result = "static[" & (if t.baseType != nil: typeToString(t.baseType) else: "") & "]"
        if t.n != nil: result.add "(" & renderTree(t.n) & ")"
    of tyUserTypeClass:
      if t.sym != nil and t.sym.owner != nil:
        if t.isResolvedUserTypeClass: return typeToString(t.baseType)
        return t.sym.owner.name.s
      else:
        result = "<invalid tyUserTypeClass>"
    of tyBuiltInTypeClass:
      result = case t.baseType.kind
        of tyVar: "var"
        of tyRef: "ref"
        of tyPtr: "ptr"
        of tySequence: "seq"
        ...
        else: typeToStr[t.baseType.kind]
    of tyInferred:
      let concrete = t.previouslyInferred
      if concrete != nil: result = typeToString(concrete)
      else: result = "inferred[" & typeToString(t.baseType) & "]"
    of tyUserTypeClassInst:
      let body = t.baseType
      result = body.sym.name.s & "["
      var i = 0
      for a in t.argTypesWithoutHeadAndBody:
        if i > 0: result.add(", ")
    ...
    of tyOr:
      ...
      for a in t.argTypes:
        ...
        inc i
    of tyNot:
      result = "not " & typeToString(t.baseType)
    of tyUntyped:
    ...
    of tyArray:
      result = "array"
      if t.argTypesLen > 0:
        if t.headType.kind == tyRange:
          result &= "[" & rangeToStr(t.headType.n) & ", " &
              typeToString(t.baseType) & ']'
        else:
          result &= "[" & typeToString(t.headType) & ", " &
              typeToString(t.baseType) & ']'
    of tyUncheckedArray:
      result = "UncheckedArray[" & typeToString(t.baseType) & ']'
    of tySequence:
      if t.sym != nil and prefer != preferResolved:
        result = t.sym.name.s
      else:
        result = "seq[" & typeToString(t.baseType) & ']'
    of tyOrdinal:
      result = "ordinal"
      if t.baseType != nil:
        result &= "[" & typeToString(t.baseType) & ']'
    ...
    of tyOpenArray:
      ...
      if t.baseType != nil:
        result &= "[" & typeToString(t.baseType) & ']'
    of tyDistinct:
      result = "distinct " & typeToString(t.baseType,
    ...
    of tyTuple:
      ...
      if t.n != nil:
        result = "tuple["
        assert(t.n.len == t.argTypesLen)
        var i = 0
        for a in t.argTypes:
          assert(t.n[i].kind == nkSym)
          if i > 0: result.add ", "
          result.add(t.n[i].sym.name.s & ": " & typeToString(a))
          inc i
        result.add(']')
      elif t.argTypesLen == 0:
        result = "tuple[]"
      else:
        result = "("
        var i = 0
        for a in t.argTypes:
          result.add typeToString(a)
          if i < t.argTypesLen-1: result.add ", "
          elif t.argTypesLen == 1: result.add(",")
          inc i
        result.add(')')
    of tyPtr, tyRef, tyVar, tyLent:
      ...
      if t.argTypesLen > 0:
        ...
        for a in t.argTypes:
          result.add(typeToString(a))
          result.add ", "
        result.add typeToString(t.baseType)
        result.add ']'
      else:
        result.add typeToString(t.baseType)
    of tyRange:
    ...
    of tyProc:
      ...
      for a in t.argTypes:
        ...
        inc i
      result.add(')')
      if t.baseType != nil: result.add(": " & typeToString(t.baseType))
    ...
    of tyVarargs:
      result = typeToStr[t.kind] % typeToString(t.baseType)
    of tySink:
      result = "sink " & typeToString(t.baseType)
    of tyOwned:
      result = "owned " & typeToString(t.baseType)
    else:
      result = typeToStr[t.kind]
    result.addTypeFlags(t)
...
proc firstOrd*(conf: ConfigRef; t: PType): Int128 =
  ...
  of tyBool, tyChar, tySequence, tyOpenArray, tyString, tyVarargs, tyProxy:
    result = Zero
  of tySet, tyVar: result = firstOrd(conf, t.baseType)
  of tyArray: result = firstOrd(conf, t.headType)
  of tyRange:
  ...
  of tyEnum:
    ...
    if t.baseType != nil:
      result = firstOrd(conf, t.baseType)
    else:
      if t.n.len > 0:
  ...
  of tyGenericInst, tyDistinct, tyTypeDesc, tyAlias, tySink,
    ...
    result = firstOrd(conf, t.baseType)
  of tyOrdinal:
    if t.baseType != nil: result = firstOrd(conf, t.baseType)
    else:
      result = Zero
...
proc firstFloat*(t: PType): BiggestFloat =
  ...
  of tyRange:
    ...
    assert(t.n.kind == nkRange)
    getFloatValue(t.n[0])
  of tyVar: firstFloat(t.baseType)
  of tyGenericInst, tyDistinct, tyTypeDesc, tyAlias, tySink,
     tyStatic, tyInferred, tyUserTypeClasses:
    firstFloat(t.baseType)
  else:
...
proc lastOrd*(conf: ConfigRef; t: PType): Int128 =
  ...
  of tySet, tyVar: result = lastOrd(conf, t.baseType)
  of tyArray: result = lastOrd(conf, t.baseType)
  of tyRange:
    assert(t.n != nil)        # range directly given:
    assert(t.n.kind == nkRange)
  ...
  of tyGenericInst, tyDistinct, tyTypeDesc, tyAlias, tySink,
     tyStatic, tyInferred, tyUserTypeClasses, tyLent:
    result = lastOrd(conf, t.baseType)
  of tyProxy: result = Zero
  of tyOrdinal:
    if t.baseType != nil: result = lastOrd(conf, t.baseType)
    else:
      result = Zero
...
proc lengthOrd*(conf: ConfigRef; t: PType): Int128 =
  if t.skipTypes(tyUserTypeClasses).kind == tyDistinct:
    result = lengthOrd(conf, t.baseType)
  else:
...
proc sameTuple(a, b: PType, c: var TSameTypeClosure): bool =
  ...
  if a.argTypesLen == b.argTypesLen:
    result = true
    for aa, bb in argTypePairs(a, b):
      var x = aa
      var y = bb
...
proc sameObjectStructures(a, b: PType, c: var TSameTypeClosure): bool =
  # check base types:
  if not sameTypeOrNilAux(a.baseType, b.baseType, c): return false
  if not sameObjectTree(a.n, b.n, c): return false
  result = true

proc sameChildrenAux(a, b: PType, c: var TSameTypeClosure): bool =
  if not sameTypeOrNilAux(a.baseType, b.baseType, c): return false
  if a.argTypesLen != b.argTypesLen: return false
  result = true
  for aa, bb in argTypePairs(a, b):
    result = sameTypeOrNilAux(aa, bb, c)
    if not result: return

proc isGenericAlias*(t: PType): bool =
  return t.kind == tyGenericInst and t.baseType.kind == tyGenericInst
...
proc skipGenericAlias*(t: PType): PType =
  return if t.isGenericAlias: t.baseType else: t
...
proc sameTypeAux(x, y: PType, c: var TSameTypeClosure): bool =
  ...
  if y.kind != tyDistinct and x.kind == tyGenericInst and IgnoreTupleFields notin c.flags:
    let
      lhs = x.skipGenericAlias
      rhs = y.skipGenericAlias
    if rhs.kind != tyGenericInst or lhs.baseType != rhs.baseType:
      return false
    if rhs.argTypesLen != lhs.argTypesLen:
      return false
    for ff, aa in argTypesWithoutHeadAndBodyPairs(rhs, lhs):
  ...
  of tyStatic, tyFromExpr:
    ...
    if result and a.baseType != nil and b.baseType != nil:
      cycleCheck()
      result = sameTypeAux(a.baseType, b.baseType, c)
  of tyObject:
    ifFastObjectTypeCheckFailed(a, b):
      cycleCheck()
  ...
  of tyDistinct:
    ...
    if c.cmp == dcEq:
      ...
      result = sameTypeAux(a.baseType, b.baseType, c) and sameFlags(a, b)
  of tyEnum, tyForward:
  ...
  of tyBuiltInTypeClass:
    assert a.baseType != nil
    assert b.baseType != nil
    result = a.baseType.kind == b.baseType.kind and sameFlags(a.baseType, b.baseType)
    if result and a.baseType.kind == tyProc and IgnoreCC notin c.flags:
      let ecc = a.baseType.flags * {tfExplicitCallConv}
      result = ecc == b.baseType.flags * {tfExplicitCallConv} and
               (ecc == {} or a.baseType.callConv == b.baseType.callConv)
  ...
  of tyRange:
    cycleCheck()
    result = sameTypeOrNilAux(a.baseType, b.baseType, c) and
...
proc containsGenericTypeIter(t: PType, closure: RootRef): bool =
  ...
  of tyStatic:
    return t.n == nil
  of tyTypeDesc:
    if t.baseType == nil or t.baseType.kind == tyNone: return true
    if containsGenericTypeIter(t.baseType, closure): return true
    return false
...
proc containsCompileTimeOnly*(t: PType): bool =
  ...
  if t.baseType != nil:
    if isCompileTimeOnly(t.baseType): return true
  for a in t.argTypes:
...
proc isEmptyContainer*(t: PType): bool =
  ...
  of tyArray, tySet, tySequence, tyOpenArray, tyVarargs:
    result = t.baseType.kind == tyEmpty
  of tyGenericInst, tyAlias, tySink: result = isEmptyContainer(t.baseType)
  else: result = false
...
proc isTupleRecursive(t: PType, cycleDetector: var IntSet): bool =
  ...
  of tyAlias, tyRef, tyPtr, tyGenericInst, tyVar, tyLent, tySink,
    ...
    return isTupleRecursive(t.baseType, cycleDetector)
  else:
    return false
...
proc isException*(t: PType): bool =
  ...
  while t.kind == tyObject:
    ...
    if t.baseType == nil: break
    t = skipTypes(t.baseType, abstractPtrs)
  return false
...
proc isCharArrayPtr*(t: PType; allowPointerToChar: bool): bool =
  let t = t.skipTypes(abstractInst)
  if t.kind == tyPtr:
    let pointsTo = t.baseType.skipTypes(abstractInst)
    case pointsTo.kind
    of tyUncheckedArray:
      result = pointsTo.baseType.kind == tyChar
    of tyArray:
      result = pointsTo.baseType.kind == tyChar and firstOrd(nil, pointsTo.headType) == 0 and
        skipTypes(pointsTo.headType, {tyRange}).kind in {tyInt..tyInt64}
    of tyChar:
