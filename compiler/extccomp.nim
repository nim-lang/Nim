...
proc completeCFilePath*(cfile: string, createSubDir: bool = true,
                        prependModule: bool = true): string =
  result = completeGeneratedFilePath(cfile, createSubDir)

  if prependModule:
    var (head, tail) = splitPath(result)
    var (headProj, tailProj) = splitPath(gProjectName)
    result = joinPath(head, changeFileExt(tailProj, "") & "_" & tail)
...
proc getCompileCFileCmd*(cfilename: string, isExternal = false): string =
  ...
  var objfile = if not isExternal or noAbsolutePaths():
                  ...
                  completeCFilePath(toObjFile(cfile), prependModule=false)
