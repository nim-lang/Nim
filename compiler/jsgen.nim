...
import
  ...
  times, compiler_ropes, math, passes, ccgutils, wordrecg, renderer,
...
template lineF(p: PProc, frmt: FormatStr, args: varargs[Rope]) =
  add(p.body, indentLine(p, compiler_ropes.`%`(frmt, args)))
...
proc frameCreate(p: PProc; procname, filename: Rope): Rope =
  ...
  result.add p.indentLine(compiler_ropes.`%`("framePtr = F;$n", []))
