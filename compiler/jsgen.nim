...
proc genTry(p: PProc, n: PNode, r: var TCompRes) =
  ...
  if catchBranchesExist:
    p.body.addf("--exceptionHandler;$n} catch (EXCEPTION) {$n var prevJSError = lastJSError;$n" &
        " lastJSError = EXCEPTION;$n --exceptionHandler;$n", [])
