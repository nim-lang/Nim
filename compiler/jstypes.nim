...
proc objHasTypeField(t: PType): bool {.inline.} =
  tfInheritable in t.flags or t.baseType != nil
...
proc genObjectInfo(p: PProc, typ: PType, name: Rope) =
  ...
  if (typ.kind == tyObject) and (typ.baseType != nil):
    p.g.typeInfo.addf("$1.base = $2;$n",
         [name, genTypeInfo(p, typ.baseType.skipTypes(skipPtrs))])
...
proc genTupleFields(p: PProc, typ: PType): Rope =
  ...
  for i, a in typ.tupleTypes:
    ...
    s.addf("{kind: 1, offset: \"Field$1\", len: 0, " &
           ...
           [i.rope, genTypeInfo(p, a)])
  result = ("{kind: 2, len: $1, offset: 0, " &
            "typ: null, name: null, sons: [$2]}") % [rope(typ.argTypesLen), s]
...
proc genEnumInfo(p: PProc, typ: PType, name: Rope) =
  ...
  if typ.baseType != nil:
    p.g.typeInfo.addf("$1.base = $2;$n",
         [name, genTypeInfo(p, typ.baseType)])
...
proc genTypeInfo(p: PProc, typ: PType): Rope =
  ...
  case t.kind
  of tyDistinct:
    result = genTypeInfo(p, t.baseType)
  ...
  of tyStatic:
    if t.n != nil: result = genTypeInfo(p, t.baseType)
