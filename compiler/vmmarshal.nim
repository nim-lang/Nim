...
proc storeAny(s: var string; t: PType; a: PNode; stored: var IntSet;
  ...
  of tyTuple:
    s.add("{")
    for i, u in t.tupleTypes:
      if i > 0: s.add(", ")
      s.add("\"Field" & $i)
      s.add("\": ")
      storeAny(s, u, a[i].skipColon, stored, conf)
    s.add("}")
  of tyObject:
    s.add("{")
  ...
  of tySet:
    ...
    for i in 0..<a.len:
      ...
      if a[i].kind == nkRange:
        ...
        storeAny(s, t.baseType, x, stored, conf)
        inc x.intVal
        ...
        storeAny(s, t.baseType, a[i], stored, conf)
    s.add("]")
  of tyRange, tyGenericInst, tyAlias, tySink:
    storeAny(s, t.baseType, a, stored, conf)
  of tyEnum:
...
proc loadAny(p: var JsonParser, t: PType,
  ...
  of tyTuple:
    ...
    while p.kind != jsonObjectEnd and p.kind != jsonEof:
      ...
      result.add loadAny(p, t.argTypeAt(i), tab, cache, conf, idgen)
      inc i
  ...
  of tySet:
    ...
    while p.kind != jsonArrayEnd and p.kind != jsonEof:
      result.add loadAny(p, t.baseType, tab, cache, conf, idgen)
    if p.kind == jsonArrayEnd: next(p)
    else: raiseParseErr(p, "']' end of array expected")
  of tyPtr, tyRef:
    case p.kind
    ...
    of jsonArrayStart:
      ...
      if p.kind == jsonInt:
        let idx = p.getInt
        next(p)
        result = loadAny(p, t.baseType, tab, cache, conf, idgen)
        tab[idx] = result
