...
proc buildEchoStmt(c: PContext, n: PNode): PNode =
  ...
  if e != nil:
    ...
    result.add newError(n, n.info, "system needs: echo")
  result.add(n)
...
proc semMagic(c: PContext, n: PNode, s: PSym, flags: TExprFlags; expectedType: PType = nil): PNode =
  ...
  of mSpawn:
    ...
    when defined(leanCompiler):
      result = newError(n, n.info, "compiler was built without 'spawn' support")
    else:
      result = setMs(n, s)
      for i in 1..<n.len:
      ...
      if n.len > 1 and n[1].kind notin nkCallKinds:
        return newError(n, n[1].info, "'spawn' takes a call expression; got: " & $n[1])
  ...
  if tupexp.len > 0: # don't interpret () as type
    ...
    for i in 1..<tupexp.len:
      if isTupleType != (tupexp[i].typ.kind == tyTypeDesc):
        return newError(n, tupexp[i].info, "Mixing types and values in tuples is not allowed.")
