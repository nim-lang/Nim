...
import ast, tables, ropes, md5, modulegraphs, strutils, ccgutils
...
proc idOrSig*(s: PSym, currentModule: string,
              sigCollisions: var CountTable[string]): Rope =
  var
    name = mangle(currentModule & "_" & s.name.s)
  sigCollisions.inc name
  let
    count = sigCollisions[name]
  if count != 0:
    name &= "_" & $count
  result = rope(name)
