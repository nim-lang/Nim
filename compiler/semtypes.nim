    ...
    of nkEnumFieldDef:
      ...
      of tyString, tyCString:
        ...
        if not isOrdinalType(v.typ):
          var givenType = typeToString(v.typ, preferDesc)
          if v.typ.enumHasHoles:
            givenType.add(" with holes")
          localError(c.config, v.info, errOrdinalTypeExpected & "; given: " & givenType)
        x = getOrdValue(v)
      if i != 1:
