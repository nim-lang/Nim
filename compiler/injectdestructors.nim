...
proc makePtrType(c: var Con, baseType: PType): PType =
  result = newType(tyPtr, c.idgen, c.owner, baseType.skipIntLit(c.idgen))

proc genOp(c: var Con; op: PSym; dest: PNode): PNode =
  var addrExp: PNode
  if op.typ != nil and op.typ.argTypesLen > 0 and op.typ.argTypeAt(0).kind != tyVar:
    addrExp = dest
  else:
...
proc isDangerousSeq(t: PType): bool {.inline.} =
  ...
  result = t.kind == tySequence and tfHasOwned notin t.baseType.flags
...
proc p(n: PNode; c: var Con; s: var Scope; mode: ProcessMode; tmpFlags = {sfSingleUsedTemp}; inReturn = false): PNode =
  if n.kind in {nkStmtList, nkStmtListExpr, nkBlockStmt, nkBlockExpr, nkIfStmt,
    ...
    of nkCallKinds:
      ...
      result = shallowCopy(n)
      for i in 1..<n.len:
        if i < L and isCompileTimeOnly(parameters.argTypeAt(i-1)):
          result[i] = n[i]
        elif i < L and (isSinkTypeForParam(parameters.argTypeAt(i-1)) or inSpawn > 0):
