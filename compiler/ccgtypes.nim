...
proc genRecordFieldsAux(m: BModule, n: PNode,
                        rectype: PType,
                        check: var IntSet, union_prefix = ""): Rope =
  result = nil
  case n.kind
  of nkRecList:
    for i in 0..<n.len:
      result.add(genRecordFieldsAux(m, n[i], rectype, check, union_prefix))
  of nkRecCase:
    if n[0].kind != nkSym: internalError(m.config, n.info, "genRecordFieldsAux")
    result.add(genRecordFieldsAux(m, n[0], rectype, check, union_prefix))
    ...
    for i in 1..<n.len:
      ...
      of nkOfBranch, nkElse:
        let k = lastSon(n[i])
        if k.kind != nkSym:
          let a = genRecordFieldsAux(m, k, rectype, check, union_prefix = union_prefix & "_i" & $i & ".")
          if a != nil:
          ...
          unionBody.add(genRecordFieldsAux(m, k, rectype, check, union_prefix))
  ...
  of nkSym:
    ...
    fillLoc(field.loc, locField, n, union_prefix & sname, OnUnknown)
