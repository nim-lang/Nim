...
proc fillBackendName(m: BModule; s: PSym) =
  if s.loc.snippet == "":
    var result: Rope
    if s.kind in routineKinds and (sfExportNim in s.flags or
  ...
  if s.kind in routineKinds and sfExportNim in s.flags and
      not containsOrIncl(m.g.exportedNimSeen, s.id):
    m.g.exportedNimProcs.add s
