    ...
    if getMagic(n) == mSlice:
      ...
      for i in 1..<n.len:
        ...
        if typ != nil and i < typ.len:
          ...
          if sfEscapes in paramType.sym.flags or paramType.typ.kind in {tyVar, tyOut}:
...
proc depsArgs(w: var W; n: PNode) =
  ...
  for i in 1..<n.len:
    ...
    if i < typ.len:
      ...
      if sfWrittenTo in paramType.sym.flags or paramType.typ.kind in {tyVar, tyOut}:
        # p(f(x, y), X, g(h, z))
    ...
    if {rootIsHeapAccess, markAsWrittenTo} * a.destInfo != {}:
      for p in a.dest:
        if p.kind == skParam and p.owner == w.owner:
          ...
          if w.owner.kind == skFunc and p.typ.kind notin {tyVar, tyOut}:
