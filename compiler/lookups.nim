...
proc initIdentIter(ti: var TIdentIter; marked: var IntSet; im: ImportedModule; name: PIdent;
                   g: ModuleGraph): PSym =
  result = initIdentIter(ti, im.m.tab(g), name)
  while result != nil:
    let b =
      case im.mode
      of importAll: true
    ...
    if b and not containsOrIncl(marked, result.id):
      return result
    result = nextIdentIter(ti, im.m.tab(g))
...
iterator allSyms*(c: PContext): (PSym, int, bool) =
  ...
  isLocal = false
  for im in c.imports.mitems:
    for s in im.m.tab(c.graph).data:
      if s != nil:
    ...
    if m != nil and m.kind == skModule:
      ...
      if ident != nil:
        if m == c.module:
          ...
          result = strTableGet(m.tab(c.graph), ident).skipAlias(n, c.config)
        if result == nil and checkUndeclared in flags:
      ...
      if result != nil:
        ...
        if scope == nil:
          for i in 0..c.imports.high:
            result = initIdentIter(o.it, o.marked, c.imports[i], ident, c.graph).skipAlias(n, c.config)
            if result != nil:
              o.currentScope = nil
              o.importIdx = i
              return result
    ...
    if o.m != nil and o.m.kind == skModule:
      ...
      if ident != nil:
        if o.m == c.module:
          ...
          result = initIdentIter(o.it, o.m.tab(c.graph), ident).skipAlias(n, c.config)
      else:
...
proc nextOverloadIterImports(o: var TOverloadIter, c: PContext, n: PNode): PSym =
  ...
  while idx < c.imports.len:
    result = initIdentIter(o.it, o.marked, c.imports[idx], o.it.name, c.graph).skipAlias(n, c.config)
    if result != nil:
...
proc symChoiceExtension(o: var TOverloadIter; c: PContext; n: PNode): PSym =
  ...
  while o.importIdx < c.imports.len:
    result = initIdentIter(o.it, o.marked, c.imports[o.importIdx], o.it.name, c.graph).skipAlias(n, c.config)
...
proc nextOverloadIter*(o: var TOverloadIter, c: PContext, n: PNode): PSym =
  ...
  of oimNoQualifier:
    if o.currentScope != nil:
      ...
      result = nextIdentIter(o.it, o.marked, c.imports[o.importIdx], c.graph).skipAlias(n, c.config)
      if result == nil:
        result = nextOverloadIterImports(o, c, n)
    else:
      result = nil
  ...
  of oimOtherModule:
    result = nextIdentIter(o.it, o.m.tab(c.graph)).skipAlias(n, c.config)
  of oimSymChoice:
