...
proc getCopyright(conf: ConfigRef; cfile: Cfile): Rope =
  if optCompileOnly in conf.globalOptions:
    ...
    var nimFile = cfile.nimFile.string
    if optListFullPaths notin conf.globalOptions: nimFile = nimFile.lastPathPart
    result = ("/* Generated by Nim Compiler v$1 */$N" &
        "/*   (c) " & copyrightYear & " Andreas Rumpf */$N" &
        "/* The generated code is subject to the original license. */$N" &
        ...
        rope(getCompileCFileCmd(conf, cfile)), rope(nimFile)]
...
proc writeModule(m: BModule, pending: bool) =
  ...
  if true or optForceFullMake in m.config.globalOptions:
    ...
    var cf = Cfile(nimname: m.module.name.s, nimFile: m.filename, cname: cfile,
    ...
    let cf = Cfile(nimname: m.module.name.s, nimFile: m.filename, cname: cfile,
                   obj: completeCfilePath(m.config, toObjFile(m.config, cfile)), flags: {})
    mergeFiles(cfile, m)
    genInitCode(m)
    finishTypeDescriptions(m)
