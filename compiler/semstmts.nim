...
proc semTry(c: PContext, n: PNode; flags: TExprFlags): PNode =
  ...
  if isEmptyType(typ) or typ.kind in {tyNil, tyUntyped}:
    ...
    for i in 1..n.len-1: discardCheck(c, n[i].lastSon, flags)
...
proc semForVars(c: PContext, n: PNode; flags: TExprFlags): PNode =
  ...
  # len(n) == 3 means that there is one for loop variable
  # and thus no tuple unpacking:
  if iterAfterVarLent.kind != tyTuple or len(n) == 3:
    if len(n) == 3:
  ...
  elif len(n)-2 != len(iterAfterVarLent):
    localError(c.config, n.info, errWrongNumberOfVariables)
  else:
    for i in 0 .. len(n) - 3:
...
proc semLambda(c: PContext, n: PNode, flags: TExprFlags): PNode =
  ...
  if n[paramsPos].kind != nkEmpty:
    ...
    if len(gp) > 0 and n[genericParamsPos].kind == nkEmpty:
