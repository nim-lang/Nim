...
when defined(nimdoc):
  ...
  proc paramCount*(): int {.tags: [ReadIOEffect].} =
    ...
    ## Test for availability using `declared() <system.html#declared%2Cuntyped>`_.
    ## Example:
    ##
    ## .. code-block:: nim
  ...
  proc paramStr*(i: int): TaintedString {.tags: [ReadIOEffect].} =
    ...
    ## `paramCount() <#paramCount%2C>`_ with this proc you can call the
    ## convenience `commandLineParams() <#commandLineParams%2C>`_.
    ##
    ## Similarly to `argv`:idx: in C,
    ## it is possible to call ``paramStr(0)`` but this will return OS specific
    ## contents (usually the name of the invoked executable). You should avoid
    ## this and call `getAppFilename() <#getAppFilename%2C>`_ instead.
    ##
    ## **Availability**: When generating a dynamic library (see --app:lib) on
    ## Posix this proc is not defined.
    ## Test for availability using `declared() <system.html#declared%2Cuntyped>`_.
...
when declared(paramCount) or defined(nimdoc):
  proc commandLineParams*(): seq[TaintedString] =
    ...
    ## executable filename, call `getAppFilename() <#getAppFilename%2C>`_.
    ##
