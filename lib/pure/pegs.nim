...
func index*(p: Peg): range[0..MaxSubpatterns] = p.index
...
func term*(t: string): Peg {.noSideEffect, rtl, extern: "npegs$1Str".} =
...
func term*(t: char): Peg {.noSideEffect, rtl, extern: "npegs$1Char".} =
  ## constructs a PEG from a terminal char
  assert t != '\0'
  result = Peg(kind: pkChar, ch: t)

func charSet*(s: set[char]): Peg {.noSideEffect, rtl, extern: "npegs$1".} =
...
func `?`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsOptional".} =
...
func `*`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsGreedyRep".} =
...
func `!*`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsSearch".} =
  ## constructs a "search" for the PEG `a`
  result = Peg(kind: pkSearch, sons: @[a])

func `!*\`*(a: Peg): Peg {.noSideEffect, rtl,
                             extern: "npgegsCapturedSearch".} =
  ## constructs a "captured search" for the PEG `a`
  result = Peg(kind: pkCapturedSearch, sons: @[a])

func `+`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsGreedyPosRep".} =
  ## constructs a "greedy positive repetition" with the PEG `a`
  return sequence(a, *a)

func `&`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsAndPredicate".} =
  ## constructs an "and predicate" with the PEG `a`
  result = Peg(kind: pkAndPredicate, sons: @[a])

func `!`*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsNotPredicate".} =
...
func capture*(a: Peg): Peg {.noSideEffect, rtl, extern: "npegsCapture".} =
...
func `$` *(r: Peg): string {.noSideEffect, rtl, extern: "npegsToString".} =
...
template matchOrParse(mopProc: untyped) =
  # Used to make the main matcher func *rawMatch* as well as event parser
  ...
  func mopProc(s: string, p: Peg, start: int, c: var Captures): int =
    func matchBackRef(s: string, p: Peg, start: int, c: var Captures): int =
...
func rawMatch*(s: string, p: Peg, start: int, c: var Captures): int
      {.noSideEffect, rtl, extern: "npegs$1".} =
  ## low-level matching func that implements the PEG interpreter. Use this
...
template eventParser*(pegAst, handlers: untyped): (proc(s: string): int) =
  ...
  func rawParse(s: string, p: Peg, start: int, c: var Captures): int
      {.genSym.} =
  ...
  func parser(s: string): int {.genSym.} =
    # the func to be returned
    var
...
func replace*(s: string, sub: Peg, cb: proc(
  ...
  ## The callback func receives the index of the current match (starting with 0),
...
when not defined(js):
  proc transformFile*(infile, outfile: string,
    ...
    ##
    ## **Note**: this func does not exist while using the JS backend.
