...
macro scanf*(input: string; pattern: static[string]; results: varargs[
    typed]): bool =
  ...
  template matchError() =
    error("type mismatch between pattern '$" & pattern[p] & "' (position: " &
        $p & ") and " & $getTypeInst(results[i]) & " var '" &
        repr(results[i]) & "'")
  ...
  result = newTree(nnkStmtListExpr, newLetStmt(inp, input), newVarStmt(idx,
      newLit 0), newVarStmt(res, newLit false))
  ...
  while p < pattern.len:
    if pattern[p] == '$':
      ...
      of '$':
        var resLen = genSym(nskLet, "resLen")
        conds.add newLetStmt(resLen, newCall(bindSym"skip", inp, newLit(
            $pattern[p]), idx))
        conds.add resLen.notZero
        conds.add resLen
      of 'w':
      ...
      of 'f':
        ...
        inc i
      of 's':
        conds.add newCall(bindSym"inc", idx, newCall(bindSym"skipWhitespace",
            inp, idx))
  ...
  proc atm(it, input, idx, attached: NimNode): StmtTriple =
      ...
      if it[0].kind == nnkPar and it.len == 2:
        ...
        if a.cond.kind == nnkEmpty or b.cond.kind == nnkEmpty:
          ...
          result = (newStmtList(a.init,
              newIfStmt((a.cond, a.action), (newTree(nnkStmtListExpr, b.init,
                  b.cond), b.action))),
              newEmptyNode(), newEmptyNode())
      elif it.kind == nnkInfix and it[0].eqIdent"^*":
        ...
        result = (newLetStmt(resLen, placeholder(it, input, idx)),
                  newCall(interf"success", resLen), !!newCall(interf"nxt",
                      input, idx, resLen))
    of nnkStrLit..nnkTripleStrLit:
      var resLen = genSym(nskLet, "resLen")
      result = (newLetStmt(resLen, newCall(interf"skip", input, it, idx)),
                newCall(interf"success", resLen), !!newCall(interf"nxt", input,
                    idx, resLen))
    ...
    of nnkCurlyExpr:
      if it.len == 3 and it[1].kind == nnkIntLit and it[2].kind == nnkIntLit:
        ...
        for count in it[1].intVal .. it[2].intVal-1: h.add(newTree(nnkPrefix,
            ident"?", it[0]))
...
when isMainModule:
  ...
  proc someSep(input: string; start: int; seps: set[char] = {';', ',', '-',
      '.'}): int =
    result = 0
    while start+result < input.len and input[start+result] in seps: inc result
  ...
  proc parseGDB(resp: string): seq[string] =
    ...
    while true:
      var prc = ""
      var info = ""
      if scanp(resp, idx, *`whites`, '#', *`digits`, +`whites`, ?("0x",
          *`hexdigits`, " in "), demangle($input, prc, $index), *`whites`,
