...
when defined(windows):
  ...
  let allocationGranularity = getAllocationGranularity()
...
proc init*(T: type ReservedMem,
  ...
  when defined(windows):
    result.memStart = virtualAlloc(memStart, maxLen, MEM_RESERVE, accessFlags)
    check result.memStart
    if commitSize > 0:
      check virtualAlloc(result.memStart, commitSize, MEM_COMMIT, accessFlags)
  else:
    ...
    if d > 0:
      ...
      when defined(windows):
        check virtualAlloc(m.committedMemEnd, commitExtensionSize,
                           MEM_COMMIT, winAccessFlags(m.accessFlags))
      else:
