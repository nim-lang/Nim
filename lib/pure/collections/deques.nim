...
proc rotateLeft*[T](deq: var Deque[T], n = 1) {.since: (1, 5, 1).} =
    ...
    if n > 0:
      ...
      for _ in n..(-1):
        let newHead = (deq.head - 1) and deq.mask
        let newTail = (deq.tail - 1) and deq.mask

        deq.data[newHead] = deq.data[newTail]
        destroy deq.data[newTail]

        deq.head = newHead
        deq.tail = newTail

template rotateRight*(deq: var Deque, n = 1): untyped {.since: (1, 5, 1).} =
