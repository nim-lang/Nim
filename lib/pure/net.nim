...
proc newSocket(fd: SocketHandle, isBuff: bool): Socket not nil =
...
proc newSocket*(domain, typ, protocol: cint, buffered = true): Socket not nil =
...
proc newSocket*(domain: Domain = AF_INET, typ: SockType = SOCK_STREAM,
                protocol: Protocol = IPPROTO_TCP, buffered = true): Socket not nil =
  ## Creates a new socket.
  ##
  ## If an error occurs EOS will be raised.
  let fd = newRawSocket(domain, typ, protocol)
