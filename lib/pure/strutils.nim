...
proc split*(s: string, seps: set[char] = Whitespace, maxsplit: int = -1):
    seq[string] {.noSideEffect, rtl, extern: "nsuSplitCharSet".} =
...
proc rfind*(s: string, chars: set[char], start: Natural = 0, last = -1): int {.
    noSideEffect, rtl, extern: "nsuRFindCharSet".} =
...
when isMainModule:
  proc nonStaticTests =
    ...
    when not defined(js):
      doAssert formatBiggestFloat(1234.567, ffDecimal, 0) ==
          "1235." # <=== bug 8242
    ...
    block:  # formatSize tests
      when not defined(js):
        doAssert formatSize((1'i64 shl 31) + (300'i64 shl 20)) ==
            "2.293GiB" # <=== bug #8231
      doAssert formatSize((2.234*1024*1024).int) == "2.234MiB"
      doAssert formatSize(4096) == "4KiB"
      doAssert formatSize(4096, prefix = bpColloquial, includeSpace = true) == "4 kB"
