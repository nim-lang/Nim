...
proc width*(r: Rune): int =
  ## Returns the width of rune ``r`` in the console.
  ##
  ## Some East Asian characters have width of two in the console, and
  ...
  if p >= 0 and c >= wideRanges[p] and c <= wideRanges[p+1]:
    return 2
  p = binarySearch(c, wideSinglets, len(wideSinglets), 1)
  if p >= 0 and c == wideSinglets[p]:
    return 2
