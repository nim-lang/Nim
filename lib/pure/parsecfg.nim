  ...
  CfgEvent* = object of RootObj  # describes a parsing event
    ...
    of cfgKeyValuePair, cfgOption:
      ...
      value*: string             # `value` field is set for compatibility with 
                                 # older versions and may be deprecated in the
                                 # future.we recommend that packages that have
                                 # already used `value` field be replaced by 
                                 # `keyVal.value` field.
    ...
    of '\c', '\L', lexbase.EndOfFile: # did not read `=` or `:`
      ...
      of '\c', '\L', lexbase.EndOfFile: # value not read
        ...
        of '\c', '\L', lexbase.EndOfFile:
          ...
          if c.literal.startsWith("\"\"\"") and c.literal.endsWith("\"\"\""):
            result.value = c.literal.substr(3, len(c.literal) - 4)
          elif (c.literal.startsWith("r\"") or c.literal.startsWith("R\"")) and c.literal.endsWith('"'):
            result.value = c.literal.substr(2, len(c.literal) - 2)
          elif c.literal.startsWith('"') and c.literal.endsWith('"'):
            result.value = c.literal.substr(1, len(c.literal) - 2)
          else:
          ...
          if temp[0].startsWith("\"\"\"") and temp[0].endsWith("\"\"\""):
            result.value = temp[0].substr(3, len(temp[0]) - 4)
          elif (temp[0].startsWith("r\"") or temp[0].startsWith("R\"")) and temp[0].endsWith('"'):
            result.value = temp[0].substr(2, len(temp[0]) - 2)
          elif temp[0].startsWith('"') and temp[0].endsWith('"'):
            result.value = temp[0].substr(1, len(temp[0]) - 2)
          else:
            result.value = temp[0]
...
proc writeConfig*(dict: Config, filename: string) =
  ## Writes the contents of the table to the specified configuration file.
  ## This method has been deprecated and it is recommended that you use
  ## the `write` method.
  dict.write(filename)
