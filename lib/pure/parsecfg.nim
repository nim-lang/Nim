...
type
  CfgEventKind* = enum # enumeration of all events that may occur when parsing
    ...
    cfgSectionPair,    # a ``[section]`` has been parsed
...
proc open*(c: var CfgParser, input: Stream, filename, commentSeparato: string,
...
proc ignoreMsg*(c: CfgParser, e: CfgEvent): string {.rtl, extern: "npc$1".} =
  ...
  ## an entry is ignored.
  case e.kind
  of cfgSectionPair: result = c.warningStr("section ignored: " & e.section)
...
proc loadConfig*(stream: Stream, filename: string = "[stream]",
  ...
  while true:
    ...
    of cfgEof:
      break
    of cfgSectionPair: # Only look for the first time the Section
