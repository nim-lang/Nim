...
proc cmpStrings(a, b: string): int {.inline, compilerproc.} =
  ...
  if minlen > 0:
    result = int nimCmpMem(unsafeAddr a[0], unsafeAddr b[0], minlen)
    if result == 0:
      result = alen - blen
  else:
    result = alen - blen
...
proc eqCstrings(a, b: cstring): bool {.inline, compilerproc.} =
  if pointer(a) == pointer(b): result = true
    ...
    when defined(c_strcmp):
      ...
      let
        x = cast[ptr UncheckedArray[char]](a)
        y = cast[ptr UncheckedArray[char]](b)
      var i = 0
      while x[i] == y[i] and x[i] != '\0': inc(i)
      result = x[i] == y[i]
