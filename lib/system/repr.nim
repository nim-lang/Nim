...
when not defined(useNimRtl):
  ...
  proc reprArray(result: var string, p: pointer, typ: PNimType,
    ...
    for i in 0..typ.size div bs - 1:
      if i > 0: add result, ", "
      reprAux(result, cast[pointer](cast[ByteAddress](p) + uint(i*bs)), typ.base, cl)
    add result, "]"
  ...
  proc reprSequence(result: var string, p: pointer, typ: PNimType,
    ...
    for i in 0..cast[PGenericSeq](p).len-1:
      if i > 0: add result, ", "
      reprAux(result, cast[pointer](cast[ByteAddress](payloadPtr(p)) + uint(align(payloadOffset, typ.align)) + uint(i*bs)),
              typ.base, cl)
    add result, "]"
  ...
  proc reprRecordAux(result: var string, p: pointer, n: ptr TNimNode,
    ...
    of nkSlot:
      add result, $n.name
      add result, " = "
      reprAux(result, cast[pointer](cast[ByteAddress](p) + uint(n.offset)), n.typ, cl)
    of nkList:
      for i in 0..n.len-1:
        if i > 0: add result, ",\n"
...
when not defined(useNimRtl):
  proc reprOpenArray(p: pointer, length: int, elemtyp: PNimType): string {.
    ...
    for i in 0..length - 1:
      if i > 0: add result, ", "
      reprAux(result, cast[pointer](cast[ByteAddress](p) + uint(i*bs)), elemtyp, cl)
    add result, "]"
