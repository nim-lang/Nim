...
template dispose*[T](x: owned(ref T), alignment: int) = nimRawDispose(cast[pointer](x), alignment)
#proc dispose*(x: pointer) = nimRawDispose(x)

proc nimDestroyAndDispose(p: pointer, alignment: int) {.compilerRtl, raises: [].} =
