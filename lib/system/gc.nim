...
proc rawNewObj(typ: PNimType, size: int, gch: var GcHeap): pointer =
  ...
  when defined(useGcAssert):
    if not ((cast[uint](cellToUsr(res)) and cast[uint](alignment-1)) == 0):
      cstderr.rawWrite "[GCASSERT] newObj: alignment check failed\n"
      rawQuit(1)
...
proc newObjRC1(typ: PNimType, size: int): pointer {.compilerRtl, noinline, raises: [].} =
  ...
  # Check that the user data (after the Cell header) is properly aligned
  when defined(useGcAssert) or defined(useSysAssert):
    if not ((cast[uint](cellToUsr(res)) and cast[uint](alignment-1)) == 0):
      cstderr.rawWrite "[SYSASSERT] newObjRC1: alignment check failed\n"
      rawQuit(1)
  # now it is buffered in the ZCT
  res.typ = typ
  setFrameInfo(res)
