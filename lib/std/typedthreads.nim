...
const nimThreadsStrictRaises {.booldefine.} = true # emit warning on raising threads
...
template createThread*[TArg](t: var Thread[TArg],
  ...
  when nimThreadsStrictRaises and not compiles(checkRaises(tp)):
    {.warning: "`tp` may raise causing the application to terminate if unhandled - annotate with {.raises: [].} to see details".}
