when not defined nimExperimentalTypeInfoCore:
  static: doAssert false, "Experimental API, subject to change"
...
proc getDynamicTypeInfo*[T: object | ref object](x: T): PNimTypeAlt =
  ## Returns the dynamic type of `x`, which is an implementation defined runtime
  ## representation of the type of `x`.
  runnableExamples("-d:nimExperimentalTypeInfoCore"):
    ...
    assert a[].getDynamicTypeInfo == Sub().getDynamicTypeInfo
    assert a.getDynamicTypeInfo.size >= 100 # exact size implementation defined
