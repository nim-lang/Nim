 ...
 * that will try to increment the shared variable 'shared_i' while other thread
 * is performing a transaction that try to decrement the same shared variable
 * if they are greater than a fixed value, and blocks otherwise.
 */
...
increment_int (GTransaction *tr, gpointer user_data)
{
  ...
  if (id == NR_THREADS - 1)
    ...
    g_print ("Incrementor thread %d set shared int to %d\n", id, *i);
}
...
worker_dec (gpointer arg)
{
  ...
  tr = g_transaction_new ("Decrementor",
