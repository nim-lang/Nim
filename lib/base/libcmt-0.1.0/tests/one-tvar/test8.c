...
{
  GTransaction *tr[5], *trc;
  gint args[2];
  gchar *names[] = { "A", "B", "C", "D", "E" };
  gint i;
  ...
  tr[3] = g_transaction_new (names[3], increment_int, NULL);
  tr[4] = g_transaction_new (names[4], increment_int, NULL);
  for (i = 0; i < 5; i++)
      g_transaction_add_tvar (tr[i], tvar_i);

  /* (((A ; B) orElse (C ; D)) ; E), A unblocks first */
  trc = g_transaction_sequence (g_transaction_or_else (
                                    g_transaction_sequence (tr[0], tr[1]),
                                    g_transaction_sequence (tr[2], tr[3])),
                                tr[4]);
