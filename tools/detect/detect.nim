...
""")
  ...
  let dir = cleanDir(cur)
  discard os.existsOrCreateDir("libc")
  discard os.existsOrCreateDir("libc/" & dir)
  discard open(other, "libc/$1/other_consts.nim" % [dir], fmWrite)

  let keyword = "when"
  if open(f, "libc/$1.nim" % [dir], fmWrite):
    for os in ["linux"]:
      for cpu in ["amd64"]:
        f.write("""
# Generated by detect.nim

$1 defined($3) and defined($4):
  include $2/$3_$4_types
  include $2/$3_$4_consts
  include $2/$3_$4_procs
""" % [keyword, dir, os, cpu])
        var f2: File
        discard open(f2, "libc/$1/$2_$3_types.nim" % [dir, os, cpu], fmAppend)
        f2.close
        discard open(f2, "libc/$1/$2_$3_procs.nim" % [dir, os, cpu], fmAppend)
        f2.close

    f.write("""
else:
  include $1/other_types
  include $1/other_consts
  include $1/other_procs
""" % [dir])
    f.close

    discard open(f, "libc/$1/other_types.nim" % [dir], fmAppend)
    f.close
    discard open(f, "libc/$1/other_procs.nim" % [dir], fmAppend)
    f.close

  addf(tl, """
f = fopen("libc/$1/$2_$3_consts.nim", "w+");
fputs("# Generated by detect.nim\n\n", f);
""", dir, system.hostOS, system.hostCPU)
