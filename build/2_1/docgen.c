/* Generated by Nimrod Compiler v0.8.11 */
/*   (c) 2011 Andreas Rumpf */

typedef long int NI;
typedef unsigned long int NU;
#include "nimbase.h"

typedef struct NimStringDesc NimStringDesc;
typedef struct TGenericSeq TGenericSeq;
typedef struct TY10402 TY10402;
typedef struct TNimType TNimType;
typedef struct TY10418 TY10418;
typedef struct TY10790 TY10790;
typedef struct TY10414 TY10414;
typedef struct TY10410 TY10410;
typedef struct TY10788 TY10788;
typedef struct TY54525 TY54525;
typedef struct TY154017 TY154017;
typedef struct TNimNode TNimNode;
typedef struct TY51008 TY51008;
typedef struct TY154033 TY154033;
typedef struct TY154011 TY154011;
typedef struct TY146077 TY146077;
typedef struct TY146075 TY146075;
typedef struct TY54551 TY54551;
typedef struct TY46536 TY46536;
typedef struct TY54547 TY54547;
typedef struct TY53011 TY53011;
typedef struct TY54519 TY54519;
typedef struct TY151007 TY151007;
typedef struct TNimObject TNimObject;
typedef struct TY53005 TY53005;
typedef struct TY54529 TY54529;
typedef struct TY54527 TY54527;
typedef struct TY54539 TY54539;
typedef struct TY54543 TY54543;
typedef struct TY84016 TY84016;
typedef struct TY84014 TY84014;
typedef struct TY84012 TY84012;
typedef struct TY54549 TY54549;
typedef struct TY42013 TY42013;
struct TGenericSeq {
NI len;
NI space;
};
typedef NIM_CHAR TY239[100000001];
struct NimStringDesc {
  TGenericSeq Sup;
TY239 data;
};
struct TY10402 {
NI Refcount;
TNimType* Typ;
};
struct TY10418 {
NI Len;
NI Cap;
TY10402** D;
};
struct TY10414 {
NI Counter;
NI Max;
TY10410* Head;
TY10410** Data;
};
struct TY10788 {
NI Stackscans;
NI Cyclecollections;
NI Maxthreshold;
NI Maxstacksize;
NI Maxstackcells;
NI Cycletablesize;
};
struct TY10790 {
TY10418 Zct;
TY10418 Decstack;
TY10414 Cycleroots;
TY10418 Tempstack;
TY10788 Stat;
};
struct TNimType {
NI size;
NU8 kind;
NU8 flags;
TNimType* base;
TNimNode* node;
void* finalizer;
};
struct TNimNode {
NU8 kind;
NI offset;
TNimType* typ;
NCSTRING name;
NI len;
TNimNode** sons;
};
struct TY154011 {
TY146077* N;
TY51008* Refname;
TY51008* Header;
};
typedef TY51008* TY154013[21];
typedef TY51008* TY154043[5];
struct TY154017 {
NimStringDesc* Filename;
NimStringDesc* Basedir;
TY51008* Moddesc;
NI Id;
NI Splitafter;
TY154033* Tocpart;
NIM_BOOL Hastoc;
TY154013 Toc;
TY154013 Section;
TY146077* Indexfile;
TY146077* Theindex;
NimStringDesc* Indexvalfilename;
NI Indent;
NI Verbatim;
TY154043 Meta;
};
struct TY146077 {
NU8 Kind;
NimStringDesc* Text;
NI Level;
TY146075* Sons;
};
struct TY46536 {
NI16 Line;
NI16 Col;
int Fileindex;
};
struct TY54525 {
TY54551* Typ;
NimStringDesc* Comment;
TY46536 Info;
NU8 Flags;
NU8 Kind;
union {
struct {NI64 Intval;
} S1;
struct {NF64 Floatval;
} S2;
struct {NimStringDesc* Strval;
} S3;
struct {TY54547* Sym;
} S4;
struct {TY53011* Ident;
} S5;
struct {TY54519* Sons;
} S6;
} KindU;
};
typedef TY51008* TY51399[1];
typedef TY51008* TY161414[4];
typedef TY51008* TY174640[2];
struct TNimObject {
TNimType* m_type;
};
struct TY151007 {
  TNimObject Sup;
NU8 Kind;
NI Start;
NI Length;
NCSTRING Buf;
NI Pos;
NU8 State;
};
typedef NimStringDesc* TY151060[33];
struct TY53005 {
  TNimObject Sup;
NI Id;
};
struct TY53011 {
  TY53005 Sup;
NimStringDesc* S;
TY53011* Next;
NI H;
};
struct TY54529 {
TNimType* m_type;
NI Counter;
TY54527* Data;
};
struct TY54539 {
NU8 K;
NU8 S;
NU8 Flags;
TY54551* T;
TY51008* R;
NI A;
};
struct TY54547 {
  TY53005 Sup;
NU8 Kind;
NU8 Magic;
TY54551* Typ;
TY53011* Name;
TY46536 Info;
TY54547* Owner;
NU32 Flags;
TY54529 Tab;
TY54525* Ast;
NU32 Options;
NI Position;
NI Offset;
TY54539 Loc;
TY54543* Annex;
};
struct TY84012 {
NU8 Kind;
NI16 Length;
};
struct TY84016 {
NI Indent;
NI Linelen;
NI Pos;
NI Idx;
TY84014* Tokens;
NimStringDesc* Buf;
NI Pendingnl;
TY54519* Comstack;
NU8 Flags;
};
typedef NimStringDesc* TY76146[109];
typedef NimStringDesc* TY47016[4];
typedef NimStringDesc* TY156387[10];
typedef NimStringDesc* TY46866[1];
typedef NimStringDesc* TY156524[6];
typedef TY51008* TY162027[6];
typedef NimStringDesc* TY156543[8];
typedef TY51008* TY162565[8];
typedef NIM_CHAR TY155228[9];
typedef TY51008* TY160003[3];
typedef NI TY8414[16];
struct TY10410 {
TY10410* Next;
NI Key;
TY8414 Bits;
};
struct TY51008 {
  TNimObject Sup;
TY51008* Left;
TY51008* Right;
NI Length;
NimStringDesc* Data;
};
struct TY54551 {
  TY53005 Sup;
NU8 Kind;
TY54549* Sons;
TY54525* N;
NU8 Flags;
NU8 Callconv;
TY54547* Owner;
TY54547* Sym;
NI64 Size;
NI Align;
NI Containerid;
TY54539 Loc;
};
struct TY42013 {
  TNimObject Sup;
TY42013* Prev;
TY42013* Next;
};
struct TY54543 {
  TY42013 Sup;
NU8 Kind;
NIM_BOOL Generated;
TY51008* Name;
TY54525* Path;
};
struct TY154033 {
  TGenericSeq Sup;
  TY154011 data[SEQ_DECL_SIZE];
};
struct TY146075 {
  TGenericSeq Sup;
  TY146077* data[SEQ_DECL_SIZE];
};
struct TY54519 {
  TGenericSeq Sup;
  TY54525* data[SEQ_DECL_SIZE];
};
struct TY54527 {
  TGenericSeq Sup;
  TY54547* data[SEQ_DECL_SIZE];
};
struct TY84014 {
  TGenericSeq Sup;
  TY84012 data[SEQ_DECL_SIZE];
};
struct TY54549 {
  TGenericSeq Sup;
  TY54551* data[SEQ_DECL_SIZE];
};
static N_INLINE(void, asgnRefNoCycle)(void** Dest_12818, void* Src_12819);
static N_INLINE(TY10402*, Usrtocell_11212)(void* Usr_11214);
static N_INLINE(NI, Atomicinc_3001)(NI* Memloc_3004, NI X_3005);
static N_INLINE(NI, Atomicdec_3006)(NI* Memloc_3009, NI X_3010);
static N_INLINE(void, Rtladdzct_12201)(TY10402* C_12203);
N_NOINLINE(void, Addzct_11201)(TY10418* S_11204, TY10402* C_11205);
N_NIMCALL(NimStringDesc*, copyString)(NimStringDesc* Src_18308);
N_NIMCALL(TY54525*, Parsefile_91031)(NimStringDesc* Filename_91033);
N_NIMCALL(NimStringDesc*, nosaddFileExt)(NimStringDesc* Filename_37828, NimStringDesc* Ext_37829);
N_NIMCALL(TY154017*, Newdocumentor_154158)(NimStringDesc* Filename_154160);
N_NIMCALL(void*, newObj)(TNimType* Typ_13507, NI Size_13508);
N_NIMCALL(void*, newSeq)(TNimType* Typ_14004, NI Len_14005);
N_NIMCALL(NimStringDesc*, Getconfigvar_45134)(NimStringDesc* Key_45136);
N_NIMCALL(NI, nsuParseInt)(NimStringDesc* S_24425);
N_NIMCALL(void, Initindexfile_154128)(TY154017* D_154130);
N_NIMCALL(NimStringDesc*, nosChangeFileExt)(NimStringDesc* Filename_37820, NimStringDesc* Ext_37821);
N_NIMCALL(NimStringDesc*, nosextractFilename)(NimStringDesc* Path_37727);
N_NIMCALL(NIM_BOOL, nosexistsFile)(NimStringDesc* Filename_36003);
N_NIMCALL(TY146077*, Rstparse_146082)(NimStringDesc* Text_146084, NIM_BOOL Skippounds_146085, NimStringDesc* Filename_146086, NI Line_146087, NI Column_146088, NIM_BOOL* Hastoc_146090);
N_NIMCALL(NimStringDesc*, Readfile_3647)(NimStringDesc* Filename_3649);
N_NIMCALL(TY146077*, Findindexnode_154047)(TY146077* N_154049);
N_NIMCALL(TY146077*, Newrstnode_146094)(NU8 Kind_146096);
N_NIMCALL(NI, Rsonslen_146091)(TY146077* N_146093);
N_NIMCALL(void, Rawmessage_46557)(NU8 Msg_46559, NimStringDesc* Arg_46560);
N_NIMCALL(void, Clearindex_146126)(TY146077* Index_146128, NimStringDesc* Filename_146129);
N_NIMCALL(void, Addson_146101)(TY146077* Father_146103, TY146077* Son_146104);
N_NIMCALL(TY146077*, Newrstnode_146097)(NU8 Kind_146099, NimStringDesc* S_146100);
N_NIMCALL(void, Generatedoc_155996)(TY154017* D_155998, TY54525* N_155999);
N_NIMCALL(void, App_51031)(TY51008** A_51034, TY51008* B_51035);
N_NIMCALL(TY51008*, Gencomment_154663)(TY154017* D_154665, TY54525* N_154666);
N_NIMCALL(NIM_BOOL, nsuStartsWith)(NimStringDesc* S_24805, NimStringDesc* Prefix_24806);
N_NIMCALL(TY51008*, Renderrsttoout_154592)(TY154017* D_154594, TY146077* N_154595);
N_NIMCALL(TY51008*, Renderaux_154596)(TY154017* D_154598, TY146077* N_154599, NimStringDesc* Outer_154600);
N_NIMCALL(TY51008*, Ropef_51066)(NimStringDesc* Frmt_51068, TY51008** Args_51070, NI Args_51070Len0);
N_NIMCALL(TY51008*, Renderheadline_155093)(TY154017* D_155095, TY146077* N_155096);
N_NIMCALL(TY51008*, Torope_51046)(NimStringDesc* S_51048);
N_NIMCALL(NimStringDesc*, Rstnodetorefname_146105)(TY146077* N_146107);
N_NIMCALL(TGenericSeq*, setLengthSeq)(TGenericSeq* Seq_18603, NI Elemsize_18604, NI Newlen_18605);
N_NIMCALL(TY51008*, Dispf_154559)(NimStringDesc* Xml_154561, NimStringDesc* Tex_154562, TY51008** Args_154564, NI Args_154564Len0);
N_NIMCALL(TY51008*, Torope_51049)(NI64 I_51051);
static N_INLINE(void, appendChar)(NimStringDesc* Dest_18409, NIM_CHAR C_18410);
static N_INLINE(void, appendString)(NimStringDesc* Dest_18392, NimStringDesc* Src_18393);
N_NIMCALL(NimStringDesc*, rawNewString)(NI Space_18287);
N_NIMCALL(TY51008*, Renderoverline_155152)(TY154017* D_155154, TY146077* N_155155);
N_NIMCALL(NimStringDesc*, Disp_154545)(NimStringDesc* Xml_154547, NimStringDesc* Tex_154548);
N_NIMCALL(TY51008*, Renderfield_155760)(TY154017* D_155762, TY146077* N_155763);
N_NIMCALL(NimStringDesc*, Addnodes_146108)(TY146077* N_146110);
N_NIMCALL(NimStringDesc*, Esc_154492)(NimStringDesc* S_154494, NI Splitafter_154495);
N_NIMCALL(NI, Nextsplitpoint_154443)(NimStringDesc* S_154445, NI Start_154446);
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* A_18247, NimStringDesc* B_18248);
N_NIMCALL(NimStringDesc*, resizeString)(NimStringDesc* Dest_18382, NI Addlen_18383);
N_NIMCALL(void, Escchar_154429)(NimStringDesc** Dest_154432, NIM_CHAR C_154433);
N_NIMCALL(void, Addxmlchar_154414)(NimStringDesc** Dest_154417, NIM_CHAR C_154418);
N_NIMCALL(NimStringDesc*, addChar)(NimStringDesc* S_1603, NIM_CHAR C_1604);
N_NIMCALL(void, Addtexchar_154424)(NimStringDesc** Dest_154427, NIM_CHAR C_154428);
N_NIMCALL(NimStringDesc*, nsuStrip)(NimStringDesc* S_23959, NIM_BOOL Leading_23960, NIM_BOOL Trailing_23961);
N_NIMCALL(NI, nsuCmpIgnoreStyle)(NimStringDesc* A_23624, NimStringDesc* B_23625);
N_NIMCALL(void, Internalerror_46575)(NimStringDesc* Errmsg_46577);
N_NIMCALL(NimStringDesc*, Texcolumns_155745)(TY146077* N_155747);
N_NIMCALL(void, Dispa_154575)(TY51008** Dest_154578, NimStringDesc* Xml_154579, NimStringDesc* Tex_154580, TY51008** Args_154582, NI Args_154582Len0);
N_NIMCALL(void, Appf_51071)(TY51008** C_51074, NimStringDesc* Frmt_51075, TY51008** Args_51077, NI Args_51077Len0);
N_NIMCALL(TY146077*, Lastson_146654)(TY146077* N_146656);
N_NIMCALL(TY51008*, Renderimage_155604)(TY154017* D_155606, TY146077* N_155607);
N_NIMCALL(NimStringDesc*, Getfieldvalue_146111)(TY146077* N_146113, NimStringDesc* Fieldname_146114);
N_NIMCALL(TY51008*, Fieldaux_155600)(NimStringDesc* S_155602);
N_NIMCALL(NimStringDesc*, Getargument_146115)(TY146077* N_146117);
N_NIMCALL(TY51008*, Rendercodeblock_155658)(TY154017* D_155660, TY146077* N_155661);
N_NIMCALL(NU8, Getsourcelanguage_151102)(NimStringDesc* Name_151104);
N_NIMCALL(void, Initgeneraltokenizer_151105)(TY151007* G_151108, NimStringDesc* Buf_151109);
N_NIMCALL(void, Getnexttoken_151114)(TY151007* G_151117, NU8 Lang_151118);
N_NIMCALL(void, App_51036)(TY51008** A_51039, NimStringDesc* B_51040);
N_NIMCALL(NimStringDesc*, copyStrLast)(NimStringDesc* S_1928, NI First_1929, NI Last_1930);
N_NIMCALL(void, Deinitgeneraltokenizer_151110)(TY151007* G_151113);
N_NIMCALL(TY51008*, Rendercontainer_155720)(TY154017* D_155722, TY146077* N_155723);
N_NIMCALL(TY51008*, Renderindexterm_154638)(TY154017* D_154640, TY146077* N_154641);
N_NIMCALL(NimStringDesc*, nimIntToStr)(NI X_19003);
N_NIMCALL(void, Setindexpair_146118)(TY146077* Index_146120, TY146077* Key_146121, TY146077* Val_146122);
N_NIMCALL(NimStringDesc*, Tofilename_46583)(TY46536 Info_46585);
N_NIMCALL(NI, Tolinenumber_46589)(TY46536 Info_46591);
N_NIMCALL(NI, Tocolumn_46586)(TY46536 Info_46588);
N_NIMCALL(void, Genitem_155000)(TY154017* D_155002, TY54525* N_155003, TY54525* Namenode_155004, NU8 K_155005);
N_NIMCALL(NIM_BOOL, Isvisible_154743)(TY54525* N_154745);
static N_INLINE(NI, Sonslen_54803)(TY54525* N_54805);
N_NIMCALL(NimStringDesc*, Getname_154871)(TY54525* N_154873, NI Splitafter_154874);
N_NIMCALL(void, Internalerror_46571)(TY46536 Info_46573, NimStringDesc* Errmsg_46574);
N_NIMCALL(TY51008*, Genreccomment_154670)(TY154017* D_154672, TY54525* N_154673);
N_NIMCALL(void, Inittokrender_84048)(TY84016* R_84051, TY54525* N_84052, NU8 Renderflags_84055);
N_NIMCALL(void, Getnexttok_84056)(TY84016* R_84059, NU8* Kind_84061, NimStringDesc** Literal_84063);
N_NIMCALL(TY51008*, Ropeformatnamedvars_154213)(NimStringDesc* Frmt_154215, NimStringDesc** Varnames_154217, NI Varnames_154217Len0, TY51008** Varvalues_154219, NI Varvalues_154219Len0);
N_NIMCALL(NI, Getvaridx_154194)(NimStringDesc** Varnames_154197, NI Varnames_154197Len0, NimStringDesc* Id_154198);
N_NIMCALL(void, Setindexforsourceterm_154618)(TY154017* D_154620, TY146077* Name_154621, NI Id_154622);
N_NIMCALL(TY146077*, Getrstname_154936)(TY54525* N_154938);
N_NIMCALL(NIM_BOOL, Checkforfalse_155909)(TY54525* N_155911);
N_NIMCALL(NIM_BOOL, Identeq_53028)(TY53011* Id_53030, NimStringDesc* Name_53031);
static N_INLINE(TY54525*, Lastson_54809)(TY54525* N_54811);
N_NIMCALL(void, Tracedeps_155974)(TY154017* D_155976, TY54525* N_155977);
N_NIMCALL(NimStringDesc*, Getmodulefile_155934)(TY54525* N_155936);
N_NIMCALL(void, Writerope_51055)(TY51008* Head_51057, NimStringDesc* Filename_51058);
N_NIMCALL(TY51008*, Genoutfile_156439)(TY154017* D_156441);
N_NIMCALL(TY51008*, Rendertocentries_155566)(TY154017* D_155568, NI* J_155570, NI Lvl_155571);
N_NIMCALL(TY51008*, Rendertocentry_155557)(TY154017* D_155559, TY154011 E_155560);
N_NIMCALL(void, Gensection_156382)(TY154017* D_156384, NU8 Kind_156385);
N_NIMCALL(NimStringDesc*, ntgetDateStr)(void);
N_NIMCALL(NimStringDesc*, ntgetClockStr)(void);
N_NIMCALL(NimStringDesc*, Getoutfile_45144)(NimStringDesc* Filename_45146, NimStringDesc* Ext_45147);
N_NIMCALL(void, Generateindex_156551)(TY154017* D_156553);
N_NIMCALL(void, Sortindex_146123)(TY146077* A_146125);
N_NIMCALL(TY51008*, Renderrsttorst_155201)(TY154017* D_155203, TY146077* N_155204);
N_NIMCALL(NimStringDesc*, nsuRepeatChar)(NI Count_24638, NIM_CHAR C_24639);
N_NIMCALL(TY51008*, Renderrstsons_155205)(TY154017* D_155207, TY146077* N_155208);
N_NIMCALL(NI, Ropelen_51052)(TY51008* A_51054);
N_NIMCALL(NimStringDesc*, reprEnum)(NI E_19355, TNimType* Typ_19356);
N_NIMCALL(void, Commandrstaux_156581)(NimStringDesc* Filename_156583, NimStringDesc* Outext_156584);
STRING_LITERAL(TMP154046, "<wbr />", 7);
STRING_LITERAL(TMP195076, "nim", 3);
STRING_LITERAL(TMP195083, "split.item.toc", 14);
STRING_LITERAL(TMP195084, "txt", 3);
STRING_LITERAL(TMP195085, "html", 4);
STRING_LITERAL(TMP195153, "Index", 5);
STRING_LITERAL(TMP195154, "##", 2);
STRING_LITERAL(TMP195155, "$1", 2);
STRING_LITERAL(TMP195156, "<h$1><a class=\"toc-backref\" id=\"$2\" href=\"#$2_toc\">$3</a></h$1>", 63);
STRING_LITERAL(TMP195157, "\\rsth$4{$3}\\label{$2}$n", 23);
STRING_LITERAL(TMP195158, "", 0);
STRING_LITERAL(TMP195159, "<h$1 id=\"$2\">$3</h$1>", 21);
STRING_LITERAL(TMP195160, "<h$1 id=\"$2\"><center>$3</center></h$1>", 38);
STRING_LITERAL(TMP195161, "\\rstov$4{$3}\\label{$2}$n", 24);
STRING_LITERAL(TMP195162, "<hr />\012", 7);
STRING_LITERAL(TMP195163, "\\hrule\012", 7);
STRING_LITERAL(TMP195164, "<p>$1</p>\012", 10);
STRING_LITERAL(TMP195165, "$1$n$n", 6);
STRING_LITERAL(TMP195166, "<ul class=\"simple\">$1</ul>\012", 27);
STRING_LITERAL(TMP195167, "\\begin{itemize}$1\\end{itemize}\012", 31);
STRING_LITERAL(TMP195168, "<li>$1</li>\012", 12);
STRING_LITERAL(TMP195169, "\\item $1\012", 9);
STRING_LITERAL(TMP195170, "<ol class=\"simple\">$1</ol>\012", 27);
STRING_LITERAL(TMP195171, "\\begin{enumerate}$1\\end{enumerate}\012", 35);
STRING_LITERAL(TMP195172, "<dl class=\"docutils\">$1</dl>\012", 29);
STRING_LITERAL(TMP195173, "\\begin{description}$1\\end{description}\012", 39);
STRING_LITERAL(TMP195174, "<dt>$1</dt>\012", 12);
STRING_LITERAL(TMP195175, "\\item[$1] ", 10);
STRING_LITERAL(TMP195176, "<dd>$1</dd>\012", 12);
STRING_LITERAL(TMP195177, "$1\012", 3);
STRING_LITERAL(TMP195178, "<table class=\"docinfo\" frame=\"void\" rules=\"none\"><col class=\"do"
"cinfo-name\" /><col class=\"docinfo-content\" /><tbody valign=\"top\""
">$1</tbody></table>", 146);
STRING_LITERAL(TMP195179, " ", 1);
STRING_LITERAL(TMP195180, "&amp;", 5);
STRING_LITERAL(TMP195181, "&lt;", 4);
STRING_LITERAL(TMP195182, "&gt;", 4);
STRING_LITERAL(TMP195183, "&quot;", 6);
STRING_LITERAL(TMP195184, "\\_", 2);
STRING_LITERAL(TMP195185, "\\symbol{123}", 12);
STRING_LITERAL(TMP195186, "\\symbol{125}", 12);
STRING_LITERAL(TMP195187, "\\symbol{91}", 11);
STRING_LITERAL(TMP195188, "\\symbol{93}", 11);
STRING_LITERAL(TMP195189, "\\symbol{92}", 11);
STRING_LITERAL(TMP195190, "\\$", 2);
STRING_LITERAL(TMP195191, "\\&", 2);
STRING_LITERAL(TMP195192, "\\#", 2);
STRING_LITERAL(TMP195193, "\\%", 2);
STRING_LITERAL(TMP195194, "\\symbol{126}", 12);
STRING_LITERAL(TMP195195, "\\symbol{64}", 11);
STRING_LITERAL(TMP195196, "\\symbol{94}", 11);
STRING_LITERAL(TMP195197, "\\symbol{96}", 11);
STRING_LITERAL(TMP195198, "author", 6);
STRING_LITERAL(TMP195199, "version", 7);
STRING_LITERAL(TMP195200, "<tr>$1</tr>$n", 13);
STRING_LITERAL(TMP195201, "<th class=\"docinfo-name\">$1:</th>", 33);
STRING_LITERAL(TMP195202, "\\item[$1:]", 10);
STRING_LITERAL(TMP195203, "<td>$1</td>", 11);
STRING_LITERAL(TMP195204, " $1$n", 5);
STRING_LITERAL(TMP195205, "<table frame=\"void\">$1</table>", 30);
STRING_LITERAL(TMP195206, "\\begin{description}$n$1\\end{description}\012", 41);
STRING_LITERAL(TMP195207, "<th align=\"left\">$1</th>", 24);
STRING_LITERAL(TMP195208, "\\item[$1]", 9);
STRING_LITERAL(TMP195209, "<td align=\"left\">$1</td>$n", 26);
STRING_LITERAL(TMP195210, "renderRstToOut", 14);
STRING_LITERAL(TMP195211, "<pre>$1</pre>$n", 15);
STRING_LITERAL(TMP195212, "\\begin{rstpre}$n$1$n\\end{rstpre}$n", 34);
STRING_LITERAL(TMP195213, "<p>$1</p>", 9);
STRING_LITERAL(TMP195214, "$1<br />", 8);
STRING_LITERAL(TMP195215, "$1\\\\$n", 6);
STRING_LITERAL(TMP195216, "<blockquote><p>$1</p></blockquote>$n", 36);
STRING_LITERAL(TMP195217, "\\begin{quote}$1\\end{quote}$n", 28);
STRING_LITERAL(TMP195218, "<table border=\"1\" class=\"docutils\">$1</table>", 45);
STRING_LITERAL(TMP195219, "\\begin{table}\\begin{rsttab}{", 28);
STRING_LITERAL(TMP195220, "|X", 2);
STRING_LITERAL(TMP195221, "|}$n\\hline$n$1\\end{rsttab}\\end{table}", 37);
STRING_LITERAL(TMP195222, " & $1", 5);
STRING_LITERAL(TMP195223, "$1\\\\$n\\hline$n", 14);
STRING_LITERAL(TMP195224, "<th>$1</th>", 11);
STRING_LITERAL(TMP195225, "\\textbf{$1}", 11);
STRING_LITERAL(TMP195226, "<a class=\"reference external\" href=\"#$2\">$1</a>", 47);
STRING_LITERAL(TMP195227, "$1\\ref{$2}", 10);
STRING_LITERAL(TMP195228, "<a class=\"reference external\" href=\"$1\">$1</a>", 46);
STRING_LITERAL(TMP195229, "\\href{$1}{$1}", 13);
STRING_LITERAL(TMP195230, "<a class=\"reference external\" href=\"$2\">$1</a>", 46);
STRING_LITERAL(TMP195231, "\\href{$2}{$1}", 13);
STRING_LITERAL(TMP195232, "scale", 5);
STRING_LITERAL(TMP195233, " scale=\"$1\"", 11);
STRING_LITERAL(TMP195234, " scale=$1", 9);
STRING_LITERAL(TMP195235, "height", 6);
STRING_LITERAL(TMP195236, " height=\"$1\"", 12);
STRING_LITERAL(TMP195237, " height=$1", 10);
STRING_LITERAL(TMP195238, "width", 5);
STRING_LITERAL(TMP195239, " width=\"$1\"", 11);
STRING_LITERAL(TMP195240, " width=$1", 9);
STRING_LITERAL(TMP195241, "alt", 3);
STRING_LITERAL(TMP195242, " alt=\"$1\"", 9);
STRING_LITERAL(TMP195243, "align", 5);
STRING_LITERAL(TMP195244, " align=\"$1\"", 11);
STRING_LITERAL(TMP195245, "[$1]", 4);
STRING_LITERAL(TMP195246, "<img src=\"$1\"$2 />", 18);
STRING_LITERAL(TMP195247, "\\includegraphics$2{$1}", 22);
STRING_LITERAL(TMP195248, "renderCodeBlock", 15);
STRING_LITERAL(TMP195364, "<span class=\"$2\">$1</span>", 26);
STRING_LITERAL(TMP195365, "\\span$2{$1}", 11);
extern NIM_CONST TY151060 Tokenclasstostr_151059;
STRING_LITERAL(TMP195366, "<pre>$1</pre>", 13);
STRING_LITERAL(TMP195367, "<div>$1</div>", 13);
STRING_LITERAL(TMP195368, "<div class=\"$1\">$2</div>", 24);
STRING_LITERAL(TMP195369, "$2", 2);
STRING_LITERAL(TMP195370, "|$1|", 4);
STRING_LITERAL(TMP195371, "<sub>$1</sub>", 13);
STRING_LITERAL(TMP195372, "\\rstsub{$1}", 11);
STRING_LITERAL(TMP195373, "<sup>$1</sup>", 13);
STRING_LITERAL(TMP195374, "\\rstsup{$1}", 11);
STRING_LITERAL(TMP195375, "<em>$1</em>", 11);
STRING_LITERAL(TMP195376, "\\emph{$1}", 9);
STRING_LITERAL(TMP195377, "<strong>$1</strong>", 19);
STRING_LITERAL(TMP195378, "<cite>$1</cite>", 15);
STRING_LITERAL(TMP195379, "<em id=\"$1\">$2</em>", 19);
STRING_LITERAL(TMP195380, "$2\\label{$1}", 12);
STRING_LITERAL(TMP195381, "#", 1);
STRING_LITERAL(TMP195382, "<tt class=\"docutils literal\"><span class=\"pre\">$1</span></tt>", 61);
STRING_LITERAL(TMP195383, "\\texttt{$1}", 11);
STRING_LITERAL(TMP195384, "`", 1);
STRING_LITERAL(TMP195385, "getName()", 9);
STRING_LITERAL(TMP195386, "<span class=\"Comment\">$1</span>", 31);
STRING_LITERAL(TMP195387, "\\spanComment{$1}", 16);
STRING_LITERAL(TMP195388, "<span class=\"Keyword\">$1</span>", 31);
STRING_LITERAL(TMP195389, "\\spanKeyword{$1}", 16);
STRING_LITERAL(TMP195390, "<span class=\"Operator\">$1</span>", 32);
STRING_LITERAL(TMP195391, "\\spanOperator{$1}", 17);
STRING_LITERAL(TMP195392, "<span class=\"StringLit\">$1</span>", 33);
STRING_LITERAL(TMP195393, "\\spanStringLit{$1}", 18);
STRING_LITERAL(TMP195394, "<span class=\"CharLit\">$1</span>", 31);
STRING_LITERAL(TMP195395, "\\spanCharLit{$1}", 16);
STRING_LITERAL(TMP195396, "<span class=\"DecNumber\">$1</span>", 33);
STRING_LITERAL(TMP195397, "\\spanDecNumber{$1}", 18);
STRING_LITERAL(TMP195398, "<span class=\"FloatNumber\">$1</span>", 35);
STRING_LITERAL(TMP195399, "\\spanFloatNumber{$1}", 20);
STRING_LITERAL(TMP195400, "<span class=\"Identifier\">$1</span>", 34);
STRING_LITERAL(TMP195401, "\\spanIdentifier{$1}", 19);
STRING_LITERAL(TMP195402, "<span class=\"Other\">$1</span>", 29);
STRING_LITERAL(TMP195403, "\\spanOther{$1}", 14);
STRING_LITERAL(TMP195404, "docgen.genThing(", 16);
extern NIM_CONST TY76146 Toktypetostr_76145;
STRING_LITERAL(TMP195405, "$", 1);
STRING_LITERAL(TMP195406, "ropeFormatNamedVars", 19);
STRING_LITERAL(TMP195407, "}", 1);
STRING_LITERAL(TMP195408, "doc.item", 8);
STRING_LITERAL(TMP195410, "name", 4);
STRING_LITERAL(TMP195411, "header", 6);
STRING_LITERAL(TMP195412, "desc", 4);
STRING_LITERAL(TMP195413, "itemID", 6);
NIM_CONST TY47016 TMP195409 = {((NimStringDesc*) &TMP195410),
((NimStringDesc*) &TMP195411),
((NimStringDesc*) &TMP195412),
((NimStringDesc*) &TMP195413)}
;
STRING_LITERAL(TMP195414, "doc.item.toc", 12);
STRING_LITERAL(TMP195415, "getRstName()", 12);
STRING_LITERAL(TMP195416, "false", 5);
STRING_LITERAL(TMP195417, ", ", 2);
STRING_LITERAL(TMP195418, "<a class=\"reference external\" href=\"$1.html\">$1</a>", 51);
STRING_LITERAL(TMP195419, "getModuleFile()", 15);
STRING_LITERAL(TMP195420, "<li><a class=\"reference\" id=\"$1_toc\" href=\"#$1\">$2</a></li>$n", 61);
STRING_LITERAL(TMP195421, "\\item\\label{$1_toc} $2\\ref{$1}$n", 32);
STRING_LITERAL(TMP195422, "<ul class=\"simple\">$1</ul>", 26);
STRING_LITERAL(TMP195423, "\\begin{enumerate}$1\\end{enumerate}", 34);
STRING_LITERAL(TMP195424, "Imports", 7);
STRING_LITERAL(TMP195425, "Types", 5);
STRING_LITERAL(TMP195426, "Consts", 6);
STRING_LITERAL(TMP195427, "Vars", 4);
STRING_LITERAL(TMP195428, "Procs", 5);
STRING_LITERAL(TMP195429, "Methods", 7);
STRING_LITERAL(TMP195430, "Iterators", 9);
STRING_LITERAL(TMP195431, "Converters", 10);
STRING_LITERAL(TMP195432, "Macros", 6);
STRING_LITERAL(TMP195433, "Templates", 9);
NIM_CONST TY156387 Sectionnames_156386 = {((NimStringDesc*) &TMP195424),
((NimStringDesc*) &TMP195425),
((NimStringDesc*) &TMP195426),
((NimStringDesc*) &TMP195427),
((NimStringDesc*) &TMP195428),
((NimStringDesc*) &TMP195429),
((NimStringDesc*) &TMP195430),
((NimStringDesc*) &TMP195431),
((NimStringDesc*) &TMP195432),
((NimStringDesc*) &TMP195433)}
;
extern NIM_CONST TY156387 Sectionnames_156386;
STRING_LITERAL(TMP195434, "doc.section", 11);
STRING_LITERAL(TMP195436, "sectionid", 9);
STRING_LITERAL(TMP195437, "sectionTitle", 12);
STRING_LITERAL(TMP195438, "sectionTitleID", 14);
STRING_LITERAL(TMP195439, "content", 7);
NIM_CONST TY47016 TMP195435 = {((NimStringDesc*) &TMP195436),
((NimStringDesc*) &TMP195437),
((NimStringDesc*) &TMP195438),
((NimStringDesc*) &TMP195439)}
;
STRING_LITERAL(TMP195440, "doc.section.toc", 15);
STRING_LITERAL(TMP195441, "doc.toc", 7);
NIM_CONST TY46866 TMP195442 = {((NimStringDesc*) &TMP195439)}
;
STRING_LITERAL(TMP195443, "Module ", 7);
STRING_LITERAL(TMP195444, "doc.body_toc", 12);
STRING_LITERAL(TMP195445, "doc.body_no_toc", 15);
STRING_LITERAL(TMP195447, "title", 5);
STRING_LITERAL(TMP195448, "tableofcontents", 15);
STRING_LITERAL(TMP195449, "moduledesc", 10);
STRING_LITERAL(TMP195450, "date", 4);
STRING_LITERAL(TMP195451, "time", 4);
NIM_CONST TY156524 TMP195446 = {((NimStringDesc*) &TMP195447),
((NimStringDesc*) &TMP195448),
((NimStringDesc*) &TMP195449),
((NimStringDesc*) &TMP195450),
((NimStringDesc*) &TMP195451),
((NimStringDesc*) &TMP195439)}
;
STRING_LITERAL(TMP195452, "doc.file", 8);
NIM_CONST TY156543 TMP195453 = {((NimStringDesc*) &TMP195447),
((NimStringDesc*) &TMP195448),
((NimStringDesc*) &TMP195449),
((NimStringDesc*) &TMP195450),
((NimStringDesc*) &TMP195451),
((NimStringDesc*) &TMP195439),
((NimStringDesc*) &TMP195198),
((NimStringDesc*) &TMP195199)}
;
NIM_CONST TY155228 Lvltochar_155227 = {33,
61,
45,
126,
96,
60,
42,
124,
43}
;
STRING_LITERAL(TMP195454, "$n$1$2$n$1$3", 12);
extern NIM_CONST TY155228 Lvltochar_155227;
STRING_LITERAL(TMP195455, "$n$1$3$n$1$2$n$1$3", 18);
STRING_LITERAL(TMP195456, "$n$n$1$2$n$n", 12);
STRING_LITERAL(TMP195457, "$n$n$1$2", 8);
STRING_LITERAL(TMP195458, "$n$1* $2", 8);
STRING_LITERAL(TMP195459, "$n$1(#) $2", 10);
STRING_LITERAL(TMP195460, "$n$1  $2", 8);
STRING_LITERAL(TMP195461, "$n$1:$2:$3$4", 12);
STRING_LITERAL(TMP195462, "$n$1| $2", 8);
STRING_LITERAL(TMP195463, "`$1`_", 5);
STRING_LITERAL(TMP195464, "`$1 <$2>`_", 10);
STRING_LITERAL(TMP195465, "`$1`:$2:", 8);
STRING_LITERAL(TMP195466, "`$1`:sub:", 9);
STRING_LITERAL(TMP195467, "`$1`:sup:", 9);
STRING_LITERAL(TMP195468, "`$1`:idx:", 9);
STRING_LITERAL(TMP195469, "*$1*", 4);
STRING_LITERAL(TMP195470, "**$1**", 6);
STRING_LITERAL(TMP195471, "`$1`", 4);
STRING_LITERAL(TMP195472, "``$1``", 6);
STRING_LITERAL(TMP195473, "\\", 1);
STRING_LITERAL(TMP195474, "\\\\", 2);
STRING_LITERAL(TMP195475, "$n$n$1.. index::$n$2", 20);
STRING_LITERAL(TMP195476, "$n$n$1.. contents::", 19);
STRING_LITERAL(TMP195478, "\\-", 2);
STRING_LITERAL(TMP195479, "tex", 3);
NimStringDesc* Splitter_154045;
extern TY10790 Gch_10808;
extern TNimType* NTI154019; /* PDoc */
extern TNimType* NTI154033; /* seq[TTocEntry] */
extern NimStringDesc* Gindexfile_45082;
extern NU8 Gcmd_45083;
extern TNimType* NTI151007; /* TGeneralTokenizer */
extern NU32 Gglobaloptions_45078;
extern TNimType* NTI146008; /* TRstNodeKind */
static N_INLINE(TY10402*, Usrtocell_11212)(void* Usr_11214) {
TY10402* Result_11215;
Result_11215 = 0;
Result_11215 = ((TY10402*) ((NI32)((NU32)(((NI) (Usr_11214))) - (NU32)(((NI) (((NI)sizeof(TY10402))))))));
return Result_11215;
}
static N_INLINE(NI, Atomicinc_3001)(NI* Memloc_3004, NI X_3005) {
NI Result_7407;
Result_7407 = 0;
(*Memloc_3004) += X_3005;
Result_7407 = (*Memloc_3004);
return Result_7407;
}
static N_INLINE(NI, Atomicdec_3006)(NI* Memloc_3009, NI X_3010) {
NI Result_7606;
Result_7606 = 0;
(*Memloc_3009) -= X_3010;
Result_7606 = (*Memloc_3009);
return Result_7606;
}
static N_INLINE(void, Rtladdzct_12201)(TY10402* C_12203) {
Addzct_11201(&Gch_10808.Zct, C_12203);
}
static N_INLINE(void, asgnRefNoCycle)(void** Dest_12818, void* Src_12819) {
TY10402* C_12820;
NI LOC4;
TY10402* C_12822;
NI LOC9;
if (!!((Src_12819 == NIM_NIL))) goto LA2;
C_12820 = 0;
C_12820 = Usrtocell_11212(Src_12819);
LOC4 = Atomicinc_3001(&(*C_12820).Refcount, 8);
LA2: ;
if (!!(((*Dest_12818) == NIM_NIL))) goto LA6;
C_12822 = 0;
C_12822 = Usrtocell_11212((*Dest_12818));
LOC9 = Atomicdec_3006(&(*C_12822).Refcount, 8);
if (!((NU32)(LOC9) < (NU32)(8))) goto LA10;
Rtladdzct_12201(C_12822);
LA10: ;
LA6: ;
(*Dest_12818) = Src_12819;
}
N_NIMCALL(TY154017*, Newdocumentor_154158)(NimStringDesc* Filename_154160) {
TY154017* Result_154161;
NimStringDesc* S_154193;
Result_154161 = 0;
Result_154161 = (TY154017*) newObj(NTI154019, sizeof(TY154017));
asgnRefNoCycle((void**) &(*Result_154161).Tocpart, (TY154033*) newSeq(NTI154033, 0));
asgnRefNoCycle((void**) &(*Result_154161).Filename, copyString(Filename_154160));
(*Result_154161).Id = 100;
(*Result_154161).Splitafter = 20;
S_154193 = 0;
S_154193 = Getconfigvar_45134(((NimStringDesc*) &TMP195083));
if (!!(((S_154193) && (S_154193)->Sup.len == 0))) goto LA2;
(*Result_154161).Splitafter = nsuParseInt(S_154193);
LA2: ;
return Result_154161;
}
N_NIMCALL(TY146077*, Findindexnode_154047)(TY146077* N_154049) {
TY146077* Result_154050;
NI I_154113;
NI HEX3Atmp_154125;
NI LOC11;
NI Res_154127;
Result_154050 = 0;
if (!(N_154049 == NIM_NIL)) goto LA2;
Result_154050 = NIM_NIL;
goto LA1;
LA2: ;
if (!((*N_154049).Kind == ((NU8) 51))) goto LA4;
Result_154050 = (*N_154049).Sons->data[2];
if (!(Result_154050 == NIM_NIL)) goto LA7;
Result_154050 = Newrstnode_146094(((NU8) 9));
asgnRefNoCycle((void**) &(*N_154049).Sons->data[2], Result_154050);
goto LA6;
LA7: ;
if (!((*Result_154050).Kind == ((NU8) 0))) goto LA9;
Result_154050 = (*Result_154050).Sons->data[0];
goto LA6;
LA9: ;
LA6: ;
goto LA1;
LA4: ;
Result_154050 = NIM_NIL;
I_154113 = 0;
HEX3Atmp_154125 = 0;
LOC11 = Rsonslen_146091(N_154049);
HEX3Atmp_154125 = (NI32)(LOC11 - 1);
Res_154127 = 0;
Res_154127 = 0;
while (1) {
if (!(Res_154127 <= HEX3Atmp_154125)) goto LA12;
I_154113 = Res_154127;
Result_154050 = Findindexnode_154047((*N_154049).Sons->data[I_154113]);
if (!!((Result_154050 == NIM_NIL))) goto LA14;
goto BeforeRet;
LA14: ;
Res_154127 += 1;
} LA12: ;
LA1: ;
BeforeRet: ;
return Result_154050;
}
N_NIMCALL(void, Initindexfile_154128)(TY154017* D_154130) {
TY146077* H_154131;
NIM_BOOL Dummyhastoc_154132;
NimStringDesc* LOC4;
NIM_BOOL LOC6;
NimStringDesc* LOC9;
NIM_BOOL LOC11;
TY146077* LOC15;
H_154131 = 0;
Dummyhastoc_154132 = 0;
if (!(Gindexfile_45082->Sup.len == 0)) goto LA2;
goto BeforeRet;
LA2: ;
asgnRefNoCycle((void**) &Gindexfile_45082, nosaddFileExt(Gindexfile_45082, ((NimStringDesc*) &TMP195084)));
LOC4 = 0;
LOC4 = nosextractFilename((*D_154130).Filename);
asgnRefNoCycle((void**) &(*D_154130).Indexvalfilename, nosChangeFileExt(LOC4, ((NimStringDesc*) &TMP195085)));
LOC6 = nosexistsFile(Gindexfile_45082);
if (!LOC6) goto LA7;
LOC9 = 0;
LOC9 = Readfile_3647(Gindexfile_45082);
asgnRefNoCycle((void**) &(*D_154130).Indexfile, Rstparse_146082(LOC9, NIM_FALSE, Gindexfile_45082, 0, 1, &Dummyhastoc_154132));
asgnRefNoCycle((void**) &(*D_154130).Theindex, Findindexnode_154047((*D_154130).Indexfile));
LOC11 = ((*D_154130).Theindex == NIM_NIL);
if (LOC11) goto LA12;
LOC11 = !(((*(*D_154130).Theindex).Kind == ((NU8) 9)));
LA12: ;
if (!LOC11) goto LA13;
Rawmessage_46557(((NU8) 190), Gindexfile_45082);
LA13: ;
Clearindex_146126((*D_154130).Theindex, (*D_154130).Indexvalfilename);
goto LA5;
LA7: ;
asgnRefNoCycle((void**) &(*D_154130).Indexfile, Newrstnode_146094(((NU8) 0)));
H_154131 = Newrstnode_146094(((NU8) 2));
(*H_154131).Level = 1;
LOC15 = 0;
LOC15 = Newrstnode_146097(((NU8) 62), ((NimStringDesc*) &TMP195153));
Addson_146101(H_154131, LOC15);
Addson_146101((*D_154130).Indexfile, H_154131);
H_154131 = Newrstnode_146094(((NU8) 51));
Addson_146101(H_154131, NIM_NIL);
Addson_146101(H_154131, NIM_NIL);
asgnRefNoCycle((void**) &(*D_154130).Theindex, Newrstnode_146094(((NU8) 9)));
Addson_146101(H_154131, (*D_154130).Theindex);
Addson_146101((*D_154130).Indexfile, H_154131);
LA5: ;
BeforeRet: ;
}
N_NIMCALL(TY51008*, Renderaux_154596)(TY154017* D_154598, TY146077* N_154599, NimStringDesc* Outer_154600) {
TY51008* Result_154601;
NI I_154609;
NI HEX3Atmp_154615;
NI LOC1;
NI Res_154617;
TY51008* LOC3;
TY51399 LOC4;
Result_154601 = 0;
Result_154601 = NIM_NIL;
I_154609 = 0;
HEX3Atmp_154615 = 0;
LOC1 = Rsonslen_146091(N_154599);
HEX3Atmp_154615 = (NI32)(LOC1 - 1);
Res_154617 = 0;
Res_154617 = 0;
while (1) {
if (!(Res_154617 <= HEX3Atmp_154615)) goto LA2;
I_154609 = Res_154617;
LOC3 = 0;
LOC3 = Renderrsttoout_154592(D_154598, (*N_154599).Sons->data[I_154609]);
App_51031(&Result_154601, LOC3);
Res_154617 += 1;
} LA2: ;
memset((void*)&LOC4, 0, sizeof(LOC4));
LOC4[0] = Result_154601;
Result_154601 = Ropef_51066(Outer_154600, LOC4, 1);
return Result_154601;
}
N_NIMCALL(TY51008*, Dispf_154559)(NimStringDesc* Xml_154561, NimStringDesc* Tex_154562, TY51008** Args_154564, NI Args_154564Len0) {
TY51008* Result_154565;
Result_154565 = 0;
if (!!((Gcmd_45083 == ((NU8) 16)))) goto LA2;
Result_154565 = Ropef_51066(Xml_154561, Args_154564, Args_154564Len0);
goto LA1;
LA2: ;
Result_154565 = Ropef_51066(Tex_154562, Args_154564, Args_154564Len0);
LA1: ;
return Result_154565;
}
static N_INLINE(void, appendChar)(NimStringDesc* Dest_18409, NIM_CHAR C_18410) {
(*Dest_18409).data[((*Dest_18409).Sup.len)-0] = C_18410;
(*Dest_18409).data[((NI32)((*Dest_18409).Sup.len + 1))-0] = 0;
(*Dest_18409).Sup.len += 1;
}
static N_INLINE(void, appendString)(NimStringDesc* Dest_18392, NimStringDesc* Src_18393) {
memcpy(((NCSTRING) (&(*Dest_18392).data[((*Dest_18392).Sup.len)-0])), ((NCSTRING) ((*Src_18393).data)), ((int) ((NI32)((NI32)((*Src_18393).Sup.len + 1) * 1))));
(*Dest_18392).Sup.len += (*Src_18393).Sup.len;
}
N_NIMCALL(TY51008*, Renderheadline_155093)(TY154017* D_155095, TY146077* N_155096) {
TY51008* Result_155097;
NI I_155105;
NI HEX3Atmp_155149;
NI LOC1;
NI Res_155151;
TY51008* LOC3;
TY51008* Refname_155107;
NimStringDesc* LOC4;
NI Length_155118;
TY161414 LOC8;
NimStringDesc* LOC9;
TY161414 LOC10;
NimStringDesc* LOC11;
Result_155097 = 0;
Result_155097 = NIM_NIL;
I_155105 = 0;
HEX3Atmp_155149 = 0;
LOC1 = Rsonslen_146091(N_155096);
HEX3Atmp_155149 = (NI32)(LOC1 - 1);
Res_155151 = 0;
Res_155151 = 0;
while (1) {
if (!(Res_155151 <= HEX3Atmp_155149)) goto LA2;
I_155105 = Res_155151;
LOC3 = 0;
LOC3 = Renderrsttoout_154592(D_155095, (*N_155096).Sons->data[I_155105]);
App_51031(&Result_155097, LOC3);
Res_155151 += 1;
} LA2: ;
Refname_155107 = 0;
LOC4 = 0;
LOC4 = Rstnodetorefname_146105(N_155096);
Refname_155107 = Torope_51046(LOC4);
if (!(*D_155095).Hastoc) goto LA6;
Length_155118 = 0;
Length_155118 = (*D_155095).Tocpart->Sup.len;
(*D_155095).Tocpart = (TY154033*) setLengthSeq(&((*D_155095).Tocpart)->Sup, sizeof(TY154011), (NI32)(Length_155118 + 1));
asgnRefNoCycle((void**) &(*D_155095).Tocpart->data[Length_155118].Refname, Refname_155107);
asgnRefNoCycle((void**) &(*D_155095).Tocpart->data[Length_155118].N, N_155096);
asgnRefNoCycle((void**) &(*D_155095).Tocpart->data[Length_155118].Header, Result_155097);
memset((void*)&LOC8, 0, sizeof(LOC8));
LOC8[0] = Torope_51049(((NI64) ((*N_155096).Level)));
LOC8[1] = (*D_155095).Tocpart->data[Length_155118].Refname;
LOC8[2] = Result_155097;
LOC9 = 0;
LOC9 = rawNewString(1);
appendChar(LOC9, ((NIM_CHAR) (((NI) ((NI32)((NI32)((*N_155096).Level - 1) + 65))))));
appendString(LOC9, ((NimStringDesc*) &TMP195158));
LOC8[3] = Torope_51046(LOC9);
Result_155097 = Dispf_154559(((NimStringDesc*) &TMP195156), ((NimStringDesc*) &TMP195157), LOC8, 4);
goto LA5;
LA6: ;
memset((void*)&LOC10, 0, sizeof(LOC10));
LOC10[0] = Torope_51049(((NI64) ((*N_155096).Level)));
LOC10[1] = Refname_155107;
LOC10[2] = Result_155097;
LOC11 = 0;
LOC11 = rawNewString(1);
appendChar(LOC11, ((NIM_CHAR) (((NI) ((NI32)((NI32)((*N_155096).Level - 1) + 65))))));
appendString(LOC11, ((NimStringDesc*) &TMP195158));
LOC10[3] = Torope_51046(LOC11);
Result_155097 = Dispf_154559(((NimStringDesc*) &TMP195159), ((NimStringDesc*) &TMP195157), LOC10, 4);
LA5: ;
return Result_155097;
}
N_NIMCALL(TY51008*, Renderoverline_155152)(TY154017* D_155154, TY146077* N_155155) {
TY51008* Result_155156;
TY51008* T_155157;
NI I_155165;
NI HEX3Atmp_155198;
NI LOC1;
NI Res_155200;
TY51008* LOC3;
TY161414 LOC9;
NimStringDesc* LOC10;
NimStringDesc* LOC11;
Result_155156 = 0;
T_155157 = 0;
T_155157 = NIM_NIL;
I_155165 = 0;
HEX3Atmp_155198 = 0;
LOC1 = Rsonslen_146091(N_155155);
HEX3Atmp_155198 = (NI32)(LOC1 - 1);
Res_155200 = 0;
Res_155200 = 0;
while (1) {
if (!(Res_155200 <= HEX3Atmp_155198)) goto LA2;
I_155165 = Res_155200;
LOC3 = 0;
LOC3 = Renderrsttoout_154592(D_155154, (*N_155155).Sons->data[I_155165]);
App_51031(&T_155157, LOC3);
Res_155200 += 1;
} LA2: ;
Result_155156 = NIM_NIL;
if (!((*D_155154).Meta[(((NU8) 1))-0] == NIM_NIL)) goto LA5;
asgnRefNoCycle((void**) &(*D_155154).Meta[(((NU8) 1))-0], T_155157);
goto LA4;
LA5: ;
if (!((*D_155154).Meta[(((NU8) 2))-0] == NIM_NIL)) goto LA7;
asgnRefNoCycle((void**) &(*D_155154).Meta[(((NU8) 2))-0], T_155157);
goto LA4;
LA7: ;
memset((void*)&LOC9, 0, sizeof(LOC9));
LOC9[0] = Torope_51049(((NI64) ((*N_155155).Level)));
LOC10 = 0;
LOC10 = Rstnodetorefname_146105(N_155155);
LOC9[1] = Torope_51046(LOC10);
LOC9[2] = T_155157;
LOC11 = 0;
LOC11 = rawNewString(1);
appendChar(LOC11, ((NIM_CHAR) (((NI) ((NI32)((NI32)((*N_155155).Level - 1) + 65))))));
appendString(LOC11, ((NimStringDesc*) &TMP195158));
LOC9[3] = Torope_51046(LOC11);
Result_155156 = Dispf_154559(((NimStringDesc*) &TMP195160), ((NimStringDesc*) &TMP195161), LOC9, 4);
LA4: ;
return Result_155156;
}
N_NIMCALL(NimStringDesc*, Disp_154545)(NimStringDesc* Xml_154547, NimStringDesc* Tex_154548) {
NimStringDesc* Result_154549;
Result_154549 = 0;
if (!!((Gcmd_45083 == ((NU8) 16)))) goto LA2;
Result_154549 = copyString(Xml_154547);
goto LA1;
LA2: ;
Result_154549 = copyString(Tex_154548);
LA1: ;
return Result_154549;
}
N_NIMCALL(NI, Nextsplitpoint_154443)(NimStringDesc* S_154445, NI Start_154446) {
NI Result_154447;
Result_154447 = 0;
Result_154447 = Start_154446;
while (1) {
if (!(Result_154447 < (NI32)(S_154445->Sup.len + 0))) goto LA1;
switch (((NU8)(S_154445->data[Result_154447]))) {
case 95:
goto BeforeRet;
break;
case 97 ... 122:
if (!((NI32)(Result_154447 + 1) < (NI32)(S_154445->Sup.len + 0))) goto LA3;
if (!(((NU8)(S_154445->data[(NI32)(Result_154447 + 1)])) >= ((NU8)(65)) && ((NU8)(S_154445->data[(NI32)(Result_154447 + 1)])) <= ((NU8)(90)))) goto LA6;
goto BeforeRet;
LA6: ;
LA3: ;
break;
default:
break;
}
Result_154447 += 1;
} LA1: ;
Result_154447 -= 1;
BeforeRet: ;
return Result_154447;
}
static N_INLINE(NIM_BOOL, eqStrings)(NimStringDesc* A_18247, NimStringDesc* B_18248) {
NIM_BOOL Result_18249;
NIM_BOOL LOC5;
NIM_BOOL LOC9;
int LOC11;
Result_18249 = 0;
if (!(A_18247 == B_18248)) goto LA2;
Result_18249 = NIM_TRUE;
goto BeforeRet;
LA2: ;
LOC5 = (A_18247 == NIM_NIL);
if (LOC5) goto LA6;
LOC5 = (B_18248 == NIM_NIL);
LA6: ;
if (!LOC5) goto LA7;
Result_18249 = NIM_FALSE;
goto BeforeRet;
LA7: ;
LOC9 = ((*A_18247).Sup.len == (*B_18248).Sup.len);
if (!(LOC9)) goto LA10;
LOC11 = memcmp(((NCSTRING) ((*A_18247).data)), ((NCSTRING) ((*B_18248).data)), ((int) ((NI32)((*A_18247).Sup.len * 1))));
LOC9 = (LOC11 == ((NI32) 0));
LA10: ;
Result_18249 = LOC9;
goto BeforeRet;
BeforeRet: ;
return Result_18249;
}
N_NIMCALL(void, Addxmlchar_154414)(NimStringDesc** Dest_154417, NIM_CHAR C_154418) {
switch (((NU8)(C_154418))) {
case 38:
(*Dest_154417) = resizeString((*Dest_154417), 5);
appendString((*Dest_154417), ((NimStringDesc*) &TMP195180));
break;
case 60:
(*Dest_154417) = resizeString((*Dest_154417), 4);
appendString((*Dest_154417), ((NimStringDesc*) &TMP195181));
break;
case 62:
(*Dest_154417) = resizeString((*Dest_154417), 4);
appendString((*Dest_154417), ((NimStringDesc*) &TMP195182));
break;
case 34:
(*Dest_154417) = resizeString((*Dest_154417), 6);
appendString((*Dest_154417), ((NimStringDesc*) &TMP195183));
break;
default:
(*Dest_154417) = addChar((*Dest_154417), C_154418);
break;
}
}
N_NIMCALL(void, Addtexchar_154424)(NimStringDesc** Dest_154427, NIM_CHAR C_154428) {
switch (((NU8)(C_154428))) {
case 95:
(*Dest_154427) = resizeString((*Dest_154427), 2);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195184));
break;
case 123:
(*Dest_154427) = resizeString((*Dest_154427), 12);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195185));
break;
case 125:
(*Dest_154427) = resizeString((*Dest_154427), 12);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195186));
break;
case 91:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195187));
break;
case 93:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195188));
break;
case 92:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195189));
break;
case 36:
(*Dest_154427) = resizeString((*Dest_154427), 2);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195190));
break;
case 38:
(*Dest_154427) = resizeString((*Dest_154427), 2);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195191));
break;
case 35:
(*Dest_154427) = resizeString((*Dest_154427), 2);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195192));
break;
case 37:
(*Dest_154427) = resizeString((*Dest_154427), 2);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195193));
break;
case 126:
(*Dest_154427) = resizeString((*Dest_154427), 12);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195194));
break;
case 64:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195195));
break;
case 94:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195196));
break;
case 96:
(*Dest_154427) = resizeString((*Dest_154427), 11);
appendString((*Dest_154427), ((NimStringDesc*) &TMP195197));
break;
default:
(*Dest_154427) = addChar((*Dest_154427), C_154428);
break;
}
}
N_NIMCALL(void, Escchar_154429)(NimStringDesc** Dest_154432, NIM_CHAR C_154433) {
if (!!((Gcmd_45083 == ((NU8) 16)))) goto LA2;
Addxmlchar_154414(Dest_154432, C_154433);
goto LA1;
LA2: ;
Addtexchar_154424(Dest_154432, C_154433);
LA1: ;
}
N_NIMCALL(NimStringDesc*, Esc_154492)(NimStringDesc* S_154494, NI Splitafter_154495) {
NimStringDesc* Result_154496;
NI Partlen_154499;
NI J_154500;
NI K_154505;
NIM_BOOL LOC6;
NI I_154515;
NI Res_154541;
NI I_154538;
NI HEX3Atmp_154542;
NI Res_154544;
Result_154496 = 0;
Result_154496 = copyString(((NimStringDesc*) &TMP195158));
if (!(0 <= Splitafter_154495)) goto LA2;
Partlen_154499 = 0;
Partlen_154499 = 0;
J_154500 = 0;
J_154500 = 0;
while (1) {
if (!(J_154500 < S_154494->Sup.len)) goto LA4;
K_154505 = 0;
K_154505 = Nextsplitpoint_154443(S_154494, J_154500);
LOC6 = !(eqStrings(Splitter_154045, ((NimStringDesc*) &TMP195179)));
if (LOC6) goto LA7;
LOC6 = (Splitafter_154495 < (NI32)((NI32)((NI32)(Partlen_154499 + K_154505) - J_154500) + 1));
LA7: ;
if (!LOC6) goto LA8;
Partlen_154499 = 0;
Result_154496 = resizeString(Result_154496, Splitter_154045->Sup.len + 0);
appendString(Result_154496, Splitter_154045);
LA8: ;
I_154515 = 0;
Res_154541 = 0;
Res_154541 = J_154500;
while (1) {
if (!(Res_154541 <= K_154505)) goto LA10;
I_154515 = Res_154541;
Escchar_154429(&Result_154496, S_154494->data[I_154515]);
Res_154541 += 1;
} LA10: ;
Partlen_154499 += (NI32)((NI32)(K_154505 - J_154500) + 1);
J_154500 = (NI32)(K_154505 + 1);
} LA4: ;
goto LA1;
LA2: ;
I_154538 = 0;
HEX3Atmp_154542 = 0;
HEX3Atmp_154542 = (NI32)((NI32)(S_154494->Sup.len + 0) - 1);
Res_154544 = 0;
Res_154544 = 0;
while (1) {
if (!(Res_154544 <= HEX3Atmp_154542)) goto LA11;
I_154538 = Res_154544;
Escchar_154429(&Result_154496, S_154494->data[I_154538]);
Res_154544 += 1;
} LA11: ;
LA1: ;
return Result_154496;
}
N_NIMCALL(TY51008*, Renderfield_155760)(TY154017* D_155762, TY146077* N_155763) {
TY51008* Result_155764;
NIM_BOOL B_155765;
NimStringDesc* Fieldname_155775;
TY51008* Fieldval_155776;
NimStringDesc* LOC4;
NimStringDesc* LOC5;
NimStringDesc* LOC6;
NI LOC8;
NI LOC14;
NimStringDesc* LOC23;
Result_155764 = 0;
B_155765 = 0;
B_155765 = NIM_FALSE;
if (!(Gcmd_45083 == ((NU8) 16))) goto LA2;
Fieldname_155775 = 0;
Fieldname_155775 = Addnodes_146108((*N_155763).Sons->data[0]);
Fieldval_155776 = 0;
LOC4 = 0;
LOC4 = Addnodes_146108((*N_155763).Sons->data[1]);
LOC5 = 0;
LOC5 = nsuStrip(LOC4, NIM_TRUE, NIM_TRUE);
LOC6 = 0;
LOC6 = Esc_154492(LOC5, -1);
Fieldval_155776 = Torope_51046(LOC6);
LOC8 = nsuCmpIgnoreStyle(Fieldname_155775, ((NimStringDesc*) &TMP195198));
if (!(LOC8 == 0)) goto LA9;
if (!((*D_155762).Meta[(((NU8) 3))-0] == NIM_NIL)) goto LA12;
asgnRefNoCycle((void**) &(*D_155762).Meta[(((NU8) 3))-0], Fieldval_155776);
B_155765 = NIM_TRUE;
LA12: ;
goto LA7;
LA9: ;
LOC14 = nsuCmpIgnoreStyle(Fieldname_155775, ((NimStringDesc*) &TMP195199));
if (!(LOC14 == 0)) goto LA15;
if (!((*D_155762).Meta[(((NU8) 4))-0] == NIM_NIL)) goto LA18;
asgnRefNoCycle((void**) &(*D_155762).Meta[(((NU8) 4))-0], Fieldval_155776);
B_155765 = NIM_TRUE;
LA18: ;
goto LA7;
LA15: ;
LA7: ;
LA2: ;
if (!B_155765) goto LA21;
Result_155764 = NIM_NIL;
goto LA20;
LA21: ;
LOC23 = 0;
LOC23 = Disp_154545(((NimStringDesc*) &TMP195200), ((NimStringDesc*) &TMP195155));
Result_155764 = Renderaux_154596(D_155762, N_155763, LOC23);
LA20: ;
return Result_155764;
}
N_NIMCALL(NimStringDesc*, Texcolumns_155745)(TY146077* N_155747) {
NimStringDesc* Result_155748;
NI I_155756;
NI HEX3Atmp_155757;
NI Res_155759;
Result_155748 = 0;
Result_155748 = copyString(((NimStringDesc*) &TMP195158));
I_155756 = 0;
HEX3Atmp_155757 = 0;
HEX3Atmp_155757 = Rsonslen_146091(N_155747);
Res_155759 = 0;
Res_155759 = 1;
while (1) {
if (!(Res_155759 <= HEX3Atmp_155757)) goto LA1;
I_155756 = Res_155759;
Result_155748 = resizeString(Result_155748, 2);
appendString(Result_155748, ((NimStringDesc*) &TMP195220));
Res_155759 += 1;
} LA1: ;
return Result_155748;
}
N_NIMCALL(void, Dispa_154575)(TY51008** Dest_154578, NimStringDesc* Xml_154579, NimStringDesc* Tex_154580, TY51008** Args_154582, NI Args_154582Len0) {
if (!!((Gcmd_45083 == ((NU8) 16)))) goto LA2;
Appf_51071(Dest_154578, Xml_154579, Args_154582, Args_154582Len0);
goto LA1;
LA2: ;
Appf_51071(Dest_154578, Tex_154580, Args_154582, Args_154582Len0);
LA1: ;
}
N_NIMCALL(TY51008*, Fieldaux_155600)(NimStringDesc* S_155602) {
TY51008* Result_155603;
NimStringDesc* LOC1;
Result_155603 = 0;
LOC1 = 0;
LOC1 = nsuStrip(S_155602, NIM_TRUE, NIM_TRUE);
Result_155603 = Torope_51046(LOC1);
return Result_155603;
}
N_NIMCALL(TY51008*, Renderimage_155604)(TY154017* D_155606, TY146077* N_155607) {
TY51008* Result_155608;
TY51008* Options_155609;
NimStringDesc* S_155610;
TY51399 LOC4;
TY51399 LOC8;
TY51399 LOC12;
TY51399 LOC16;
TY51399 LOC20;
TY51399 LOC24;
TY174640 LOC25;
NimStringDesc* LOC26;
NI LOC28;
TY51008* LOC31;
Result_155608 = 0;
Options_155609 = 0;
Options_155609 = NIM_NIL;
S_155610 = 0;
S_155610 = Getfieldvalue_146111(N_155607, ((NimStringDesc*) &TMP195232));
if (!!(((S_155610) && (S_155610)->Sup.len == 0))) goto LA2;
memset((void*)&LOC4, 0, sizeof(LOC4));
LOC4[0] = Fieldaux_155600(S_155610);
Dispa_154575(&Options_155609, ((NimStringDesc*) &TMP195233), ((NimStringDesc*) &TMP195234), LOC4, 1);
LA2: ;
S_155610 = Getfieldvalue_146111(N_155607, ((NimStringDesc*) &TMP195235));
if (!!(((S_155610) && (S_155610)->Sup.len == 0))) goto LA6;
memset((void*)&LOC8, 0, sizeof(LOC8));
LOC8[0] = Fieldaux_155600(S_155610);
Dispa_154575(&Options_155609, ((NimStringDesc*) &TMP195236), ((NimStringDesc*) &TMP195237), LOC8, 1);
LA6: ;
S_155610 = Getfieldvalue_146111(N_155607, ((NimStringDesc*) &TMP195238));
if (!!(((S_155610) && (S_155610)->Sup.len == 0))) goto LA10;
memset((void*)&LOC12, 0, sizeof(LOC12));
LOC12[0] = Fieldaux_155600(S_155610);
Dispa_154575(&Options_155609, ((NimStringDesc*) &TMP195239), ((NimStringDesc*) &TMP195240), LOC12, 1);
LA10: ;
S_155610 = Getfieldvalue_146111(N_155607, ((NimStringDesc*) &TMP195241));
if (!!(((S_155610) && (S_155610)->Sup.len == 0))) goto LA14;
memset((void*)&LOC16, 0, sizeof(LOC16));
LOC16[0] = Fieldaux_155600(S_155610);
Dispa_154575(&Options_155609, ((NimStringDesc*) &TMP195242), ((NimStringDesc*) &TMP195158), LOC16, 1);
LA14: ;
S_155610 = Getfieldvalue_146111(N_155607, ((NimStringDesc*) &TMP195243));
if (!!(((S_155610) && (S_155610)->Sup.len == 0))) goto LA18;
memset((void*)&LOC20, 0, sizeof(LOC20));
LOC20[0] = Fieldaux_155600(S_155610);
Dispa_154575(&Options_155609, ((NimStringDesc*) &TMP195244), ((NimStringDesc*) &TMP195158), LOC20, 1);
LA18: ;
if (!!((Options_155609 == NIM_NIL))) goto LA22;
memset((void*)&LOC24, 0, sizeof(LOC24));
LOC24[0] = Options_155609;
Options_155609 = Dispf_154559(((NimStringDesc*) &TMP195155), ((NimStringDesc*) &TMP195245), LOC24, 1);
LA22: ;
memset((void*)&LOC25, 0, sizeof(LOC25));
LOC26 = 0;
LOC26 = Getargument_146115(N_155607);
LOC25[0] = Torope_51046(LOC26);
LOC25[1] = Options_155609;
Result_155608 = Dispf_154559(((NimStringDesc*) &TMP195246), ((NimStringDesc*) &TMP195247), LOC25, 2);
LOC28 = Rsonslen_146091(N_155607);
if (!(3 <= LOC28)) goto LA29;
LOC31 = 0;
LOC31 = Renderrsttoout_154592(D_155606, (*N_155607).Sons->data[2]);
App_51031(&Result_155608, LOC31);
LA29: ;
return Result_155608;
}
N_NIMCALL(TY51008*, Rendercodeblock_155658)(TY154017* D_155660, TY146077* N_155661) {
TY51008* Result_155662;
TY146077* M_155674;
NimStringDesc* Langstr_155684;
NimStringDesc* LOC7;
NU8 Lang_155685;
TY151007 G_155695;
NimStringDesc* LOC15;
TY174640 LOC16;
NimStringDesc* LOC17;
NimStringDesc* LOC18;
TY51399 LOC22;
Result_155662 = 0;
Result_155662 = NIM_NIL;
if (!((*N_155661).Sons->data[2] == NIM_NIL)) goto LA2;
goto BeforeRet;
LA2: ;
M_155674 = 0;
M_155674 = (*(*N_155661).Sons->data[2]).Sons->data[0];
if (!!(((*M_155674).Kind == ((NU8) 62)))) goto LA5;
Internalerror_46575(((NimStringDesc*) &TMP195248));
LA5: ;
Langstr_155684 = 0;
LOC7 = 0;
LOC7 = Getargument_146115(N_155661);
Langstr_155684 = nsuStrip(LOC7, NIM_TRUE, NIM_TRUE);
Lang_155685 = 0;
if (!((Langstr_155684) && (Langstr_155684)->Sup.len == 0)) goto LA9;
Lang_155685 = ((NU8) 1);
goto LA8;
LA9: ;
Lang_155685 = Getsourcelanguage_151102(Langstr_155684);
LA8: ;
if (!(Lang_155685 == ((NU8) 0))) goto LA12;
Rawmessage_46557(((NU8) 220), Langstr_155684);
Result_155662 = Torope_51046((*M_155674).Text);
goto LA11;
LA12: ;
memset((void*)&G_155695, 0, sizeof(G_155695));
G_155695.Sup.m_type = NTI151007;
Initgeneraltokenizer_151105(&G_155695, (*M_155674).Text);
while (1) {
Getnexttoken_151114(&G_155695, Lang_155685);
switch (G_155695.Kind) {
case ((NU8) 0):
goto LA14;
break;
case ((NU8) 1):
case ((NU8) 2):
LOC15 = 0;
LOC15 = copyStrLast((*M_155674).Text, (NI32)(G_155695.Start + 0), (NI32)((NI32)((NI32)(G_155695.Length + G_155695.Start) - 1) + 0));
App_51036(&Result_155662, LOC15);
break;
default:
memset((void*)&LOC16, 0, sizeof(LOC16));
LOC17 = 0;
LOC17 = copyStrLast((*M_155674).Text, (NI32)(G_155695.Start + 0), (NI32)((NI32)((NI32)(G_155695.Length + G_155695.Start) - 1) + 0));
LOC18 = 0;
LOC18 = Esc_154492(LOC17, -1);
LOC16[0] = Torope_51046(LOC18);
LOC16[1] = Torope_51046(Tokenclasstostr_151059[(G_155695.Kind)-0]);
Dispa_154575(&Result_155662, ((NimStringDesc*) &TMP195364), ((NimStringDesc*) &TMP195365), LOC16, 2);
break;
}
} LA14: ;
Deinitgeneraltokenizer_151110(&G_155695);
LA11: ;
if (!!((Result_155662 == NIM_NIL))) goto LA20;
memset((void*)&LOC22, 0, sizeof(LOC22));
LOC22[0] = Result_155662;
Result_155662 = Dispf_154559(((NimStringDesc*) &TMP195366), ((NimStringDesc*) &TMP195212), LOC22, 1);
LA20: ;
BeforeRet: ;
return Result_155662;
}
N_NIMCALL(TY51008*, Rendercontainer_155720)(TY154017* D_155722, TY146077* N_155723) {
TY51008* Result_155724;
TY51008* Arg_155725;
NimStringDesc* LOC1;
NimStringDesc* LOC2;
TY51399 LOC6;
TY174640 LOC7;
Result_155724 = 0;
Result_155724 = Renderrsttoout_154592(D_155722, (*N_155723).Sons->data[2]);
Arg_155725 = 0;
LOC1 = 0;
LOC1 = Getargument_146115(N_155723);
LOC2 = 0;
LOC2 = nsuStrip(LOC1, NIM_TRUE, NIM_TRUE);
Arg_155725 = Torope_51046(LOC2);
if (!(Arg_155725 == NIM_NIL)) goto LA4;
memset((void*)&LOC6, 0, sizeof(LOC6));
LOC6[0] = Result_155724;
Result_155724 = Dispf_154559(((NimStringDesc*) &TMP195367), ((NimStringDesc*) &TMP195155), LOC6, 1);
goto LA3;
LA4: ;
memset((void*)&LOC7, 0, sizeof(LOC7));
LOC7[0] = Arg_155725;
LOC7[1] = Result_155724;
Result_155724 = Dispf_154559(((NimStringDesc*) &TMP195368), ((NimStringDesc*) &TMP195369), LOC7, 2);
LA3: ;
return Result_155724;
}
N_NIMCALL(TY51008*, Renderindexterm_154638)(TY154017* D_154640, TY146077* N_154641) {
TY51008* Result_154642;
TY174640 LOC1;
TY146077* H_154659;
TY146077* A_154662;
NimStringDesc* LOC2;
NimStringDesc* LOC3;
NimStringDesc* LOC4;
Result_154642 = 0;
(*D_154640).Id += 1;
memset((void*)&LOC1, 0, sizeof(LOC1));
LOC1[0] = Torope_51049(((NI64) ((*D_154640).Id)));
LOC1[1] = Renderaux_154596(D_154640, N_154641, ((NimStringDesc*) &TMP195155));
Result_154642 = Dispf_154559(((NimStringDesc*) &TMP195379), ((NimStringDesc*) &TMP195380), LOC1, 2);
H_154659 = 0;
H_154659 = Newrstnode_146094(((NU8) 38));
A_154662 = 0;
LOC2 = 0;
LOC3 = 0;
LOC3 = Disp_154545(((NimStringDesc*) &TMP195381), ((NimStringDesc*) &TMP195158));
LOC4 = 0;
LOC4 = nimIntToStr((*D_154640).Id);
LOC2 = rawNewString((*D_154640).Indexvalfilename->Sup.len + LOC3->Sup.len + LOC4->Sup.len + 0);
appendString(LOC2, (*D_154640).Indexvalfilename);
appendString(LOC2, LOC3);
appendString(LOC2, LOC4);
A_154662 = Newrstnode_146097(((NU8) 62), LOC2);
Addson_146101(H_154659, A_154662);
Addson_146101(H_154659, A_154662);
Setindexpair_146118((*D_154640).Theindex, N_154641, H_154659);
return Result_154642;
}
N_NIMCALL(TY51008*, Renderrsttoout_154592)(TY154017* D_154594, TY146077* N_154595) {
TY51008* Result_155807;
NimStringDesc* LOC4;
NimStringDesc* LOC5;
NimStringDesc* LOC6;
NimStringDesc* LOC7;
NimStringDesc* LOC8;
NimStringDesc* LOC9;
NimStringDesc* LOC10;
NimStringDesc* LOC11;
NI I_155826;
NI HEX3Atmp_155903;
NI LOC12;
NI Res_155905;
TY51008* LOC14;
TY51399 LOC18;
NimStringDesc* LOC19;
NimStringDesc* LOC20;
NimStringDesc* LOC21;
NimStringDesc* LOC22;
NimStringDesc* LOC23;
NimStringDesc* LOC24;
NimStringDesc* LOC25;
NimStringDesc* LOC26;
NimStringDesc* LOC27;
NimStringDesc* LOC28;
NimStringDesc* LOC29;
NimStringDesc* LOC30;
NimStringDesc* LOC31;
NI LOC33;
NI I_155852;
NI HEX3Atmp_155906;
NI LOC36;
NI Res_155908;
TY51399 LOC38;
TY51399 LOC39;
NimStringDesc* LOC40;
NimStringDesc* LOC41;
TY174640 LOC42;
NimStringDesc* LOC43;
NimStringDesc* LOC44;
TY174640 LOC45;
TY146077* LOC49;
NimStringDesc* LOC50;
TY146077* LOC54;
NimStringDesc* LOC55;
NimStringDesc* LOC56;
TY174640 LOC57;
NimStringDesc* LOC58;
NimStringDesc* LOC59;
NimStringDesc* LOC60;
NimStringDesc* LOC61;
NimStringDesc* LOC62;
NimStringDesc* LOC66;
NimStringDesc* LOC67;
NimStringDesc* LOC68;
Result_155807 = 0;
if (!(N_154595 == NIM_NIL)) goto LA2;
Result_155807 = NIM_NIL;
goto BeforeRet;
LA2: ;
switch ((*N_154595).Kind) {
case ((NU8) 0):
Result_155807 = Renderaux_154596(D_154594, N_154595, ((NimStringDesc*) &TMP195155));
break;
case ((NU8) 1):
Result_155807 = Renderheadline_155093(D_154594, N_154595);
break;
case ((NU8) 2):
Result_155807 = Renderoverline_155152(D_154594, N_154595);
break;
case ((NU8) 3):
LOC4 = 0;
LOC4 = Disp_154545(((NimStringDesc*) &TMP195162), ((NimStringDesc*) &TMP195163));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC4);
break;
case ((NU8) 4):
LOC5 = 0;
LOC5 = Disp_154545(((NimStringDesc*) &TMP195164), ((NimStringDesc*) &TMP195165));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC5);
break;
case ((NU8) 5):
LOC6 = 0;
LOC6 = Disp_154545(((NimStringDesc*) &TMP195166), ((NimStringDesc*) &TMP195167));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC6);
break;
case ((NU8) 6):
case ((NU8) 8):
LOC7 = 0;
LOC7 = Disp_154545(((NimStringDesc*) &TMP195168), ((NimStringDesc*) &TMP195169));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC7);
break;
case ((NU8) 7):
LOC8 = 0;
LOC8 = Disp_154545(((NimStringDesc*) &TMP195170), ((NimStringDesc*) &TMP195171));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC8);
break;
case ((NU8) 9):
LOC9 = 0;
LOC9 = Disp_154545(((NimStringDesc*) &TMP195172), ((NimStringDesc*) &TMP195173));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC9);
break;
case ((NU8) 10):
Result_155807 = Renderaux_154596(D_154594, N_154595, ((NimStringDesc*) &TMP195155));
break;
case ((NU8) 11):
LOC10 = 0;
LOC10 = Disp_154545(((NimStringDesc*) &TMP195174), ((NimStringDesc*) &TMP195175));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC10);
break;
case ((NU8) 12):
LOC11 = 0;
LOC11 = Disp_154545(((NimStringDesc*) &TMP195176), ((NimStringDesc*) &TMP195177));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC11);
break;
case ((NU8) 13):
Result_155807 = NIM_NIL;
I_155826 = 0;
HEX3Atmp_155903 = 0;
LOC12 = Rsonslen_146091(N_154595);
HEX3Atmp_155903 = (NI32)(LOC12 - 1);
Res_155905 = 0;
Res_155905 = 0;
while (1) {
if (!(Res_155905 <= HEX3Atmp_155903)) goto LA13;
I_155826 = Res_155905;
LOC14 = 0;
LOC14 = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[I_155826]);
App_51031(&Result_155807, LOC14);
Res_155905 += 1;
} LA13: ;
if (!!((Result_155807 == NIM_NIL))) goto LA16;
memset((void*)&LOC18, 0, sizeof(LOC18));
LOC18[0] = Result_155807;
Result_155807 = Dispf_154559(((NimStringDesc*) &TMP195178), ((NimStringDesc*) &TMP195173), LOC18, 1);
LA16: ;
break;
case ((NU8) 14):
Result_155807 = Renderfield_155760(D_154594, N_154595);
break;
case ((NU8) 15):
LOC19 = 0;
LOC19 = Disp_154545(((NimStringDesc*) &TMP195201), ((NimStringDesc*) &TMP195202));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC19);
break;
case ((NU8) 16):
LOC20 = 0;
LOC20 = Disp_154545(((NimStringDesc*) &TMP195203), ((NimStringDesc*) &TMP195204));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC20);
break;
case ((NU8) 51):
Result_155807 = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[2]);
break;
case ((NU8) 17):
LOC21 = 0;
LOC21 = Disp_154545(((NimStringDesc*) &TMP195205), ((NimStringDesc*) &TMP195206));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC21);
break;
case ((NU8) 18):
LOC22 = 0;
LOC22 = Disp_154545(((NimStringDesc*) &TMP195200), ((NimStringDesc*) &TMP195155));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC22);
break;
case ((NU8) 19):
LOC23 = 0;
LOC23 = Disp_154545(((NimStringDesc*) &TMP195207), ((NimStringDesc*) &TMP195208));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC23);
break;
case ((NU8) 23):
LOC24 = 0;
LOC24 = Disp_154545(((NimStringDesc*) &TMP195209), ((NimStringDesc*) &TMP195204));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC24);
break;
case ((NU8) 20):
case ((NU8) 21):
case ((NU8) 22):
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
case ((NU8) 24):
LOC25 = 0;
LOC25 = Disp_154545(((NimStringDesc*) &TMP195211), ((NimStringDesc*) &TMP195212));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC25);
break;
case ((NU8) 25):
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
case ((NU8) 26):
LOC26 = 0;
LOC26 = Disp_154545(((NimStringDesc*) &TMP195213), ((NimStringDesc*) &TMP195165));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC26);
break;
case ((NU8) 27):
LOC27 = 0;
LOC27 = Disp_154545(((NimStringDesc*) &TMP195214), ((NimStringDesc*) &TMP195215));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC27);
break;
case ((NU8) 28):
LOC28 = 0;
LOC28 = Disp_154545(((NimStringDesc*) &TMP195216), ((NimStringDesc*) &TMP195217));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC28);
break;
case ((NU8) 29):
case ((NU8) 30):
LOC29 = 0;
LOC30 = 0;
LOC30 = Texcolumns_155745(N_154595);
LOC29 = rawNewString(LOC30->Sup.len + 65);
appendString(LOC29, ((NimStringDesc*) &TMP195219));
appendString(LOC29, LOC30);
appendString(LOC29, ((NimStringDesc*) &TMP195221));
LOC31 = 0;
LOC31 = Disp_154545(((NimStringDesc*) &TMP195218), LOC29);
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC31);
break;
case ((NU8) 31):
LOC33 = Rsonslen_146091(N_154595);
if (!(1 <= LOC33)) goto LA34;
Result_155807 = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[0]);
I_155852 = 0;
HEX3Atmp_155906 = 0;
LOC36 = Rsonslen_146091(N_154595);
HEX3Atmp_155906 = (NI32)(LOC36 - 1);
Res_155908 = 0;
Res_155908 = 1;
while (1) {
if (!(Res_155908 <= HEX3Atmp_155906)) goto LA37;
I_155852 = Res_155908;
memset((void*)&LOC38, 0, sizeof(LOC38));
LOC38[0] = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[I_155852]);
Dispa_154575(&Result_155807, ((NimStringDesc*) &TMP195155), ((NimStringDesc*) &TMP195222), LOC38, 1);
Res_155908 += 1;
} LA37: ;
memset((void*)&LOC39, 0, sizeof(LOC39));
LOC39[0] = Result_155807;
Result_155807 = Dispf_154559(((NimStringDesc*) &TMP195200), ((NimStringDesc*) &TMP195223), LOC39, 1);
goto LA32;
LA34: ;
Result_155807 = NIM_NIL;
LA32: ;
break;
case ((NU8) 33):
LOC40 = 0;
LOC40 = Disp_154545(((NimStringDesc*) &TMP195203), ((NimStringDesc*) &TMP195155));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC40);
break;
case ((NU8) 32):
LOC41 = 0;
LOC41 = Disp_154545(((NimStringDesc*) &TMP195224), ((NimStringDesc*) &TMP195225));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC41);
break;
case ((NU8) 34):
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
case ((NU8) 35):
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
case ((NU8) 36):
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
case ((NU8) 39):
memset((void*)&LOC42, 0, sizeof(LOC42));
LOC42[0] = Renderaux_154596(D_154594, N_154595, ((NimStringDesc*) &TMP195155));
LOC43 = 0;
LOC43 = Rstnodetorefname_146105(N_154595);
LOC42[1] = Torope_51046(LOC43);
Result_155807 = Dispf_154559(((NimStringDesc*) &TMP195226), ((NimStringDesc*) &TMP195227), LOC42, 2);
break;
case ((NU8) 37):
LOC44 = 0;
LOC44 = Disp_154545(((NimStringDesc*) &TMP195228), ((NimStringDesc*) &TMP195229));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC44);
break;
case ((NU8) 38):
memset((void*)&LOC45, 0, sizeof(LOC45));
LOC45[0] = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[0]);
LOC45[1] = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[1]);
Result_155807 = Dispf_154559(((NimStringDesc*) &TMP195230), ((NimStringDesc*) &TMP195231), LOC45, 2);
break;
case ((NU8) 41):
case ((NU8) 42):
Result_155807 = Renderaux_154596(D_154594, N_154595, ((NimStringDesc*) &TMP195155));
break;
case ((NU8) 48):
if (!!((Gcmd_45083 == ((NU8) 16)))) goto LA47;
LOC49 = 0;
LOC49 = Lastson_146654(N_154595);
LOC50 = 0;
LOC50 = Addnodes_146108(LOC49);
Result_155807 = Torope_51046(LOC50);
LA47: ;
break;
case ((NU8) 49):
if (!(Gcmd_45083 == ((NU8) 16))) goto LA52;
LOC54 = 0;
LOC54 = Lastson_146654(N_154595);
LOC55 = 0;
LOC55 = Addnodes_146108(LOC54);
Result_155807 = Torope_51046(LOC55);
LA52: ;
break;
case ((NU8) 45):
case ((NU8) 46):
Result_155807 = Renderimage_155604(D_154594, N_154595);
break;
case ((NU8) 47):
Result_155807 = Rendercodeblock_155658(D_154594, N_154595);
break;
case ((NU8) 50):
Result_155807 = Rendercontainer_155720(D_154594, N_154595);
break;
case ((NU8) 61):
case ((NU8) 52):
LOC56 = 0;
LOC56 = Disp_154545(((NimStringDesc*) &TMP195370), ((NimStringDesc*) &TMP195370));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC56);
break;
case ((NU8) 40):
Result_155807 = Renderaux_154596(D_154594, N_154595, ((NimStringDesc*) &TMP195158));
break;
case ((NU8) 53):
memset((void*)&LOC57, 0, sizeof(LOC57));
LOC57[0] = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[0]);
LOC57[1] = Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[1]);
Result_155807 = Dispf_154559(((NimStringDesc*) &TMP195364), ((NimStringDesc*) &TMP195365), LOC57, 2);
break;
case ((NU8) 54):
LOC58 = 0;
LOC58 = Disp_154545(((NimStringDesc*) &TMP195371), ((NimStringDesc*) &TMP195372));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC58);
break;
case ((NU8) 55):
LOC59 = 0;
LOC59 = Disp_154545(((NimStringDesc*) &TMP195373), ((NimStringDesc*) &TMP195374));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC59);
break;
case ((NU8) 57):
LOC60 = 0;
LOC60 = Disp_154545(((NimStringDesc*) &TMP195375), ((NimStringDesc*) &TMP195376));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC60);
break;
case ((NU8) 58):
LOC61 = 0;
LOC61 = Disp_154545(((NimStringDesc*) &TMP195377), ((NimStringDesc*) &TMP195225));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC61);
break;
case ((NU8) 59):
LOC62 = 0;
LOC62 = Disp_154545(((NimStringDesc*) &TMP195378), ((NimStringDesc*) &TMP195376));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC62);
break;
case ((NU8) 56):
if (!((*D_154594).Theindex == NIM_NIL)) goto LA64;
LOC66 = 0;
LOC66 = Disp_154545(((NimStringDesc*) &TMP195375), ((NimStringDesc*) &TMP195376));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC66);
goto LA63;
LA64: ;
Result_155807 = Renderindexterm_154638(D_154594, N_154595);
LA63: ;
break;
case ((NU8) 60):
LOC67 = 0;
LOC67 = Disp_154545(((NimStringDesc*) &TMP195382), ((NimStringDesc*) &TMP195383));
Result_155807 = Renderaux_154596(D_154594, N_154595, LOC67);
break;
case ((NU8) 62):
LOC68 = 0;
LOC68 = Esc_154492((*N_154595).Text, -1);
Result_155807 = Torope_51046(LOC68);
break;
case ((NU8) 44):
(*D_154594).Hastoc = NIM_TRUE;
break;
case ((NU8) 43):
asgnRefNoCycle((void**) &(*D_154594).Meta[(((NU8) 1))-0], Renderrsttoout_154592(D_154594, (*N_154595).Sons->data[0]));
break;
default:
Internalerror_46575(((NimStringDesc*) &TMP195210));
break;
}
BeforeRet: ;
return Result_155807;
}
N_NIMCALL(TY51008*, Gencomment_154663)(TY154017* D_154665, TY54525* N_154666) {
TY51008* Result_154667;
NIM_BOOL Dummyhastoc_154668;
NIM_BOOL LOC2;
NimStringDesc* LOC6;
NI LOC7;
NI LOC8;
TY146077* LOC9;
Result_154667 = 0;
Dummyhastoc_154668 = 0;
LOC2 = !(((*N_154666).Comment == NIM_NIL));
if (!(LOC2)) goto LA3;
LOC2 = nsuStartsWith((*N_154666).Comment, ((NimStringDesc*) &TMP195154));
LA3: ;
if (!LOC2) goto LA4;
LOC6 = 0;
LOC6 = Tofilename_46583((*N_154666).Info);
LOC7 = Tolinenumber_46589((*N_154666).Info);
LOC8 = Tocolumn_46586((*N_154666).Info);
LOC9 = 0;
LOC9 = Rstparse_146082((*N_154666).Comment, NIM_TRUE, LOC6, LOC7, LOC8, &Dummyhastoc_154668);
Result_154667 = Renderrsttoout_154592(D_154665, LOC9);
LA4: ;
return Result_154667;
}
static N_INLINE(NI, Sonslen_54803)(TY54525* N_54805) {
NI Result_55880;
Result_55880 = 0;
if (!(*N_54805).KindU.S6.Sons == 0) goto LA2;
Result_55880 = 0;
goto LA1;
LA2: ;
Result_55880 = (*N_54805).KindU.S6.Sons->Sup.len;
LA1: ;
return Result_55880;
}
N_NIMCALL(NIM_BOOL, Isvisible_154743)(TY54525* N_154745) {
NIM_BOOL Result_154746;
NIM_BOOL LOC5;
NI LOC6;
TY53011* V_154803;
NIM_BOOL LOC10;
Result_154746 = 0;
Result_154746 = NIM_FALSE;
if (!((*N_154745).Kind == ((NU8) 29))) goto LA2;
LOC6 = Sonslen_54803(N_154745);
LOC5 = (LOC6 == 2);
if (!(LOC5)) goto LA7;
LOC5 = ((*(*N_154745).KindU.S6.Sons->data[0]).Kind == ((NU8) 2));
LA7: ;
if (!LOC5) goto LA8;
V_154803 = 0;
V_154803 = (*(*N_154745).KindU.S6.Sons->data[0]).KindU.S5.Ident;
LOC10 = ((*V_154803).Sup.Id == 69);
if (LOC10) goto LA11;
LOC10 = ((*V_154803).Sup.Id == 70);
LA11: ;
Result_154746 = LOC10;
LA8: ;
goto LA1;
LA2: ;
if (!((*N_154745).Kind == ((NU8) 3))) goto LA12;
Result_154746 = (((*(*N_154745).KindU.S4.Sym).Flags &(1<<((((NU8) 3))&31)))!=0);
goto LA1;
LA12: ;
if (!((*N_154745).Kind == ((NU8) 34))) goto LA14;
Result_154746 = Isvisible_154743((*N_154745).KindU.S6.Sons->data[0]);
goto LA1;
LA14: ;
LA1: ;
return Result_154746;
}
N_NIMCALL(NimStringDesc*, Getname_154871)(TY54525* N_154873, NI Splitafter_154874) {
NimStringDesc* Result_154875;
NimStringDesc* LOC1;
NimStringDesc* LOC2;
NimStringDesc* LOC3;
NimStringDesc* LOC4;
Result_154875 = 0;
switch ((*N_154873).Kind) {
case ((NU8) 29):
Result_154875 = Getname_154871((*N_154873).KindU.S6.Sons->data[1], Splitafter_154874);
break;
case ((NU8) 34):
Result_154875 = Getname_154871((*N_154873).KindU.S6.Sons->data[0], Splitafter_154874);
break;
case ((NU8) 3):
Result_154875 = Esc_154492((*(*(*N_154873).KindU.S4.Sym).Name).S, Splitafter_154874);
break;
case ((NU8) 2):
Result_154875 = Esc_154492((*(*N_154873).KindU.S5.Ident).S, Splitafter_154874);
break;
case ((NU8) 43):
LOC1 = 0;
LOC2 = 0;
LOC2 = Esc_154492(((NimStringDesc*) &TMP195384), -1);
LOC3 = 0;
LOC3 = Getname_154871((*N_154873).KindU.S6.Sons->data[0], Splitafter_154874);
LOC4 = 0;
LOC4 = Esc_154492(((NimStringDesc*) &TMP195384), -1);
LOC1 = rawNewString(LOC2->Sup.len + LOC3->Sup.len + LOC4->Sup.len + 0);
appendString(LOC1, LOC2);
appendString(LOC1, LOC3);
appendString(LOC1, LOC4);
Result_154875 = LOC1;
break;
default:
Internalerror_46571((*N_154873).Info, ((NimStringDesc*) &TMP195385));
Result_154875 = copyString(((NimStringDesc*) &TMP195158));
break;
}
return Result_154875;
}
N_NIMCALL(TY51008*, Genreccomment_154670)(TY154017* D_154672, TY54525* N_154673) {
TY51008* Result_154674;
NI I_154716;
NI HEX3Atmp_154740;
NI LOC10;
NI Res_154742;
Result_154674 = 0;
if (!(N_154673 == NIM_NIL)) goto LA2;
Result_154674 = NIM_NIL;
goto BeforeRet;
LA2: ;
Result_154674 = Gencomment_154663(D_154672, N_154673);
if (!(Result_154674 == NIM_NIL)) goto LA5;
if (!!(((*N_154673).Kind >= ((NU8) 1) && (*N_154673).Kind <= ((NU8) 18)))) goto LA8;
I_154716 = 0;
HEX3Atmp_154740 = 0;
LOC10 = Sonslen_54803(N_154673);
HEX3Atmp_154740 = (NI32)(LOC10 - 1);
Res_154742 = 0;
Res_154742 = 0;
while (1) {
if (!(Res_154742 <= HEX3Atmp_154740)) goto LA11;
I_154716 = Res_154742;
Result_154674 = Genreccomment_154670(D_154672, (*N_154673).KindU.S6.Sons->data[I_154716]);
if (!!((Result_154674 == NIM_NIL))) goto LA13;
goto BeforeRet;
LA13: ;
Res_154742 += 1;
} LA11: ;
LA8: ;
goto LA4;
LA5: ;
asgnRefNoCycle((void**) &(*N_154673).Comment, copyString(NIM_NIL));
LA4: ;
BeforeRet: ;
return Result_154674;
}
N_NIMCALL(NI, Getvaridx_154194)(NimStringDesc** Varnames_154197, NI Varnames_154197Len0, NimStringDesc* Id_154198) {
NI Result_154199;
NI I_154207;
NI HEX3Atmp_154210;
NI Res_154212;
NI LOC3;
Result_154199 = 0;
I_154207 = 0;
HEX3Atmp_154210 = 0;
HEX3Atmp_154210 = (Varnames_154197Len0-1);
Res_154212 = 0;
Res_154212 = 0;
while (1) {
if (!(Res_154212 <= HEX3Atmp_154210)) goto LA1;
I_154207 = Res_154212;
LOC3 = nsuCmpIgnoreStyle(Varnames_154197[I_154207], Id_154198);
if (!(LOC3 == 0)) goto LA4;
Result_154199 = I_154207;
goto BeforeRet;
LA4: ;
Res_154212 += 1;
} LA1: ;
Result_154199 = -1;
BeforeRet: ;
return Result_154199;
}
N_NIMCALL(TY51008*, Ropeformatnamedvars_154213)(NimStringDesc* Frmt_154215, NimStringDesc** Varnames_154217, NI Varnames_154217Len0, TY51008** Varvalues_154219, NI Varvalues_154219Len0) {
TY51008* Result_154220;
NI I_154221;
NI L_154224;
NI Num_154225;
NI J_154280;
NIM_BOOL LOC7;
NimStringDesc* Id_154320;
NI Idx_154345;
NimStringDesc* Id_154349;
NI Idx_154390;
NI Start_154394;
NimStringDesc* LOC35;
Result_154220 = 0;
I_154221 = 0;
I_154221 = 0;
L_154224 = 0;
L_154224 = Frmt_154215->Sup.len;
Result_154220 = NIM_NIL;
Num_154225 = 0;
Num_154225 = 0;
while (1) {
if (!(I_154221 < L_154224)) goto LA1;
if (!((NU8)(Frmt_154215->data[I_154221]) == (NU8)(36))) goto LA3;
I_154221 += 1;
switch (((NU8)(Frmt_154215->data[I_154221]))) {
case 35:
App_51031(&Result_154220, Varvalues_154219[Num_154225]);
Num_154225 += 1;
I_154221 += 1;
break;
case 36:
App_51036(&Result_154220, ((NimStringDesc*) &TMP195405));
I_154221 += 1;
break;
case 48 ... 57:
J_154280 = 0;
J_154280 = 0;
while (1) {
J_154280 = (NI32)((NI32)((NI32)(J_154280 * 10) + ((NU8)(Frmt_154215->data[I_154221]))) - 48);
I_154221 += 1;
LOC7 = ((NI32)((NI32)(L_154224 + 0) - 1) < I_154221);
if (LOC7) goto LA8;
LOC7 = !((((NU8)(Frmt_154215->data[I_154221])) >= ((NU8)(48)) && ((NU8)(Frmt_154215->data[I_154221])) <= ((NU8)(57))));
LA8: ;
if (!LOC7) goto LA9;
goto LA5;
LA9: ;
} LA5: ;
if (!((NI32)((Varvalues_154219Len0-1) + 1) < J_154280)) goto LA12;
Internalerror_46575(((NimStringDesc*) &TMP195406));
LA12: ;
Num_154225 = J_154280;
App_51031(&Result_154220, Varvalues_154219[(NI32)(J_154280 - 1)]);
break;
case 65 ... 90:
case 97 ... 122:
case 128 ... 255:
Id_154320 = 0;
Id_154320 = copyString(((NimStringDesc*) &TMP195158));
while (1) {
Id_154320 = addChar(Id_154320, Frmt_154215->data[I_154221]);
I_154221 += 1;
if (!!((((NU8)(Frmt_154215->data[I_154221])) >= ((NU8)(65)) && ((NU8)(Frmt_154215->data[I_154221])) <= ((NU8)(90)) || ((NU8)(Frmt_154215->data[I_154221])) == ((NU8)(95)) || ((NU8)(Frmt_154215->data[I_154221])) >= ((NU8)(97)) && ((NU8)(Frmt_154215->data[I_154221])) <= ((NU8)(122)) || ((NU8)(Frmt_154215->data[I_154221])) >= ((NU8)(128)) && ((NU8)(Frmt_154215->data[I_154221])) <= ((NU8)(255))))) goto LA16;
goto LA14;
LA16: ;
} LA14: ;
Idx_154345 = 0;
Idx_154345 = Getvaridx_154194(Varnames_154217, Varnames_154217Len0, Id_154320);
if (!(0 <= Idx_154345)) goto LA19;
App_51031(&Result_154220, Varvalues_154219[Idx_154345]);
goto LA18;
LA19: ;
Rawmessage_46557(((NU8) 149), Id_154320);
LA18: ;
break;
case 123:
Id_154349 = 0;
Id_154349 = copyString(((NimStringDesc*) &TMP195158));
I_154221 += 1;
while (1) {
if (!!(((NU8)(Frmt_154215->data[I_154221]) == (NU8)(125)))) goto LA21;
if (!((NU8)(Frmt_154215->data[I_154221]) == (NU8)(0))) goto LA23;
Rawmessage_46557(((NU8) 21), ((NimStringDesc*) &TMP195407));
LA23: ;
Id_154349 = addChar(Id_154349, Frmt_154215->data[I_154221]);
I_154221 += 1;
} LA21: ;
I_154221 += 1;
Idx_154390 = 0;
Idx_154390 = Getvaridx_154194(Varnames_154217, Varnames_154217Len0, Id_154349);
if (!(0 <= Idx_154390)) goto LA26;
App_51031(&Result_154220, Varvalues_154219[Idx_154390]);
goto LA25;
LA26: ;
Rawmessage_46557(((NU8) 149), Id_154349);
LA25: ;
break;
default:
Internalerror_46575(((NimStringDesc*) &TMP195406));
break;
}
LA3: ;
Start_154394 = 0;
Start_154394 = I_154221;
while (1) {
if (!(I_154221 < L_154224)) goto LA28;
if (!!(((NU8)(Frmt_154215->data[I_154221]) == (NU8)(36)))) goto LA30;
I_154221 += 1;
goto LA29;
LA30: ;
goto LA28;
LA29: ;
} LA28: ;
if (!(Start_154394 <= (NI32)(I_154221 - 1))) goto LA33;
LOC35 = 0;
LOC35 = copyStrLast(Frmt_154215, Start_154394, (NI32)(I_154221 - 1));
App_51036(&Result_154220, LOC35);
LA33: ;
} LA1: ;
return Result_154220;
}
N_NIMCALL(void, Setindexforsourceterm_154618)(TY154017* D_154620, TY146077* Name_154621, NI Id_154622) {
TY146077* H_154634;
TY146077* A_154637;
NimStringDesc* LOC4;
NimStringDesc* LOC5;
NimStringDesc* LOC6;
if (!((*D_154620).Theindex == NIM_NIL)) goto LA2;
goto BeforeRet;
LA2: ;
H_154634 = 0;
H_154634 = Newrstnode_146094(((NU8) 38));
A_154637 = 0;
LOC4 = 0;
LOC5 = 0;
LOC5 = Disp_154545(((NimStringDesc*) &TMP195381), ((NimStringDesc*) &TMP195158));
LOC6 = 0;
LOC6 = nimIntToStr(Id_154622);
LOC4 = rawNewString((*D_154620).Indexvalfilename->Sup.len + LOC5->Sup.len + LOC6->Sup.len + 0);
appendString(LOC4, (*D_154620).Indexvalfilename);
appendString(LOC4, LOC5);
appendString(LOC4, LOC6);
A_154637 = Newrstnode_146097(((NU8) 62), LOC4);
Addson_146101(H_154634, A_154637);
Addson_146101(H_154634, A_154637);
A_154637 = Newrstnode_146094(((NU8) 56));
Addson_146101(A_154637, Name_154621);
Setindexpair_146118((*D_154620).Theindex, A_154637, H_154634);
BeforeRet: ;
}
N_NIMCALL(TY146077*, Getrstname_154936)(TY54525* N_154938) {
TY146077* Result_154939;
Result_154939 = 0;
switch ((*N_154938).Kind) {
case ((NU8) 29):
Result_154939 = Getrstname_154936((*N_154938).KindU.S6.Sons->data[1]);
break;
case ((NU8) 34):
Result_154939 = Getrstname_154936((*N_154938).KindU.S6.Sons->data[0]);
break;
case ((NU8) 3):
Result_154939 = Newrstnode_146097(((NU8) 62), (*(*(*N_154938).KindU.S4.Sym).Name).S);
break;
case ((NU8) 2):
Result_154939 = Newrstnode_146097(((NU8) 62), (*(*N_154938).KindU.S5.Ident).S);
break;
case ((NU8) 43):
Result_154939 = Getrstname_154936((*N_154938).KindU.S6.Sons->data[0]);
break;
default:
Internalerror_46571((*N_154938).Info, ((NimStringDesc*) &TMP195415));
Result_154939 = NIM_NIL;
break;
}
return Result_154939;
}
N_NIMCALL(void, Genitem_155000)(TY154017* D_155002, TY54525* N_155003, TY54525* Namenode_155004, NU8 K_155005) {
NIM_BOOL LOC2;
TY51008* Name_155006;
NimStringDesc* LOC5;
TY51008* Result_155007;
NimStringDesc* Literal_155008;
NU8 Kind_155009;
TY51008* Comm_155010;
TY84016 R_155011;
TY51399 LOC7;
NimStringDesc* LOC8;
TY51399 LOC9;
TY51399 LOC10;
NimStringDesc* LOC11;
TY51399 LOC12;
NimStringDesc* LOC13;
TY51399 LOC14;
NimStringDesc* LOC15;
TY51399 LOC16;
NimStringDesc* LOC17;
TY51399 LOC18;
NimStringDesc* LOC19;
TY51399 LOC20;
NimStringDesc* LOC21;
TY51399 LOC22;
NimStringDesc* LOC23;
NimStringDesc* LOC24;
NimStringDesc* LOC25;
TY161414 LOC26;
TY51008* LOC27;
NimStringDesc* LOC28;
TY161414 LOC29;
NimStringDesc* LOC30;
TY51008* LOC31;
TY146077* LOC32;
LOC2 = Isvisible_154743(Namenode_155004);
if (!!(LOC2)) goto LA3;
goto BeforeRet;
LA3: ;
Name_155006 = 0;
LOC5 = 0;
LOC5 = Getname_154871(Namenode_155004, -1);
Name_155006 = Torope_51046(LOC5);
Result_155007 = 0;
Result_155007 = NIM_NIL;
Literal_155008 = 0;
Literal_155008 = copyString(((NimStringDesc*) &TMP195158));
Kind_155009 = 0;
Kind_155009 = ((NU8) 1);
Comm_155010 = 0;
Comm_155010 = Genreccomment_154670(D_155002, N_155003);
memset((void*)&R_155011, 0, sizeof(R_155011));
Inittokrender_84048(&R_155011, N_155003, 30);
while (1) {
Getnexttok_84056(&R_155011, &Kind_155009, &Literal_155008);
switch (Kind_155009) {
case ((NU8) 1):
goto LA6;
break;
case ((NU8) 100):
memset((void*)&LOC7, 0, sizeof(LOC7));
LOC8 = 0;
LOC8 = Esc_154492(Literal_155008, -1);
LOC7[0] = Torope_51046(LOC8);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195386), ((NimStringDesc*) &TMP195387), LOC7, 1);
break;
case ((NU8) 3) ... ((NU8) 65):
memset((void*)&LOC9, 0, sizeof(LOC9));
LOC9[0] = Torope_51046(Literal_155008);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195388), ((NimStringDesc*) &TMP195389), LOC9, 1);
break;
case ((NU8) 99):
case ((NU8) 98):
memset((void*)&LOC10, 0, sizeof(LOC10));
LOC11 = 0;
LOC11 = Esc_154492(Literal_155008, -1);
LOC10[0] = Torope_51046(LOC11);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195390), ((NimStringDesc*) &TMP195391), LOC10, 1);
break;
case ((NU8) 74) ... ((NU8) 76):
memset((void*)&LOC12, 0, sizeof(LOC12));
LOC13 = 0;
LOC13 = Esc_154492(Literal_155008, -1);
LOC12[0] = Torope_51046(LOC13);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195392), ((NimStringDesc*) &TMP195393), LOC12, 1);
break;
case ((NU8) 79):
memset((void*)&LOC14, 0, sizeof(LOC14));
LOC15 = 0;
LOC15 = Esc_154492(Literal_155008, -1);
LOC14[0] = Torope_51046(LOC15);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195394), ((NimStringDesc*) &TMP195395), LOC14, 1);
break;
case ((NU8) 66) ... ((NU8) 70):
memset((void*)&LOC16, 0, sizeof(LOC16));
LOC17 = 0;
LOC17 = Esc_154492(Literal_155008, -1);
LOC16[0] = Torope_51046(LOC17);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195396), ((NimStringDesc*) &TMP195397), LOC16, 1);
break;
case ((NU8) 71) ... ((NU8) 73):
memset((void*)&LOC18, 0, sizeof(LOC18));
LOC19 = 0;
LOC19 = Esc_154492(Literal_155008, -1);
LOC18[0] = Torope_51046(LOC19);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195398), ((NimStringDesc*) &TMP195399), LOC18, 1);
break;
case ((NU8) 2):
memset((void*)&LOC20, 0, sizeof(LOC20));
LOC21 = 0;
LOC21 = Esc_154492(Literal_155008, -1);
LOC20[0] = Torope_51046(LOC21);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195400), ((NimStringDesc*) &TMP195401), LOC20, 1);
break;
case ((NU8) 102):
case ((NU8) 103):
case ((NU8) 104):
case ((NU8) 105):
App_51036(&Result_155007, Literal_155008);
break;
case ((NU8) 80):
case ((NU8) 81):
case ((NU8) 82):
case ((NU8) 83):
case ((NU8) 84):
case ((NU8) 85):
case ((NU8) 86):
case ((NU8) 87):
case ((NU8) 88):
case ((NU8) 89):
case ((NU8) 90):
case ((NU8) 91):
case ((NU8) 92):
case ((NU8) 93):
case ((NU8) 94):
case ((NU8) 95):
case ((NU8) 96):
case ((NU8) 97):
case ((NU8) 101):
memset((void*)&LOC22, 0, sizeof(LOC22));
LOC23 = 0;
LOC23 = Esc_154492(Literal_155008, -1);
LOC22[0] = Torope_51046(LOC23);
Dispa_154575(&Result_155007, ((NimStringDesc*) &TMP195402), ((NimStringDesc*) &TMP195403), LOC22, 1);
break;
default:
LOC24 = 0;
LOC24 = rawNewString(Toktypetostr_76145[(Kind_155009)-0]->Sup.len + 17);
appendString(LOC24, ((NimStringDesc*) &TMP195404));
appendString(LOC24, Toktypetostr_76145[(Kind_155009)-0]);
appendChar(LOC24, 41);
Internalerror_46571((*N_155003).Info, LOC24);
break;
}
} LA6: ;
(*D_155002).Id += 1;
LOC25 = 0;
LOC25 = Getconfigvar_45134(((NimStringDesc*) &TMP195408));
memset((void*)&LOC26, 0, sizeof(LOC26));
LOC26[0] = Name_155006;
LOC26[1] = Result_155007;
LOC26[2] = Comm_155010;
LOC26[3] = Torope_51049(((NI64) ((*D_155002).Id)));
LOC27 = 0;
LOC27 = Ropeformatnamedvars_154213(LOC25, TMP195409, 4, LOC26, 4);
App_51031(&(*D_155002).Section[(K_155005)-0], LOC27);
LOC28 = 0;
LOC28 = Getconfigvar_45134(((NimStringDesc*) &TMP195414));
memset((void*)&LOC29, 0, sizeof(LOC29));
LOC30 = 0;
LOC30 = Getname_154871(Namenode_155004, (*D_155002).Splitafter);
LOC29[0] = Torope_51046(LOC30);
LOC29[1] = Result_155007;
LOC29[2] = Comm_155010;
LOC29[3] = Torope_51049(((NI64) ((*D_155002).Id)));
LOC31 = 0;
LOC31 = Ropeformatnamedvars_154213(LOC28, TMP195409, 4, LOC29, 4);
App_51031(&(*D_155002).Toc[(K_155005)-0], LOC31);
LOC32 = 0;
LOC32 = Getrstname_154936(Namenode_155004);
Setindexforsourceterm_154618(D_155002, LOC32, (*D_155002).Id);
BeforeRet: ;
}
N_NIMCALL(NIM_BOOL, Checkforfalse_155909)(TY54525* N_155911) {
NIM_BOOL Result_155912;
NIM_BOOL LOC1;
Result_155912 = 0;
LOC1 = ((*N_155911).Kind == ((NU8) 2));
if (!(LOC1)) goto LA2;
LOC1 = Identeq_53028((*N_155911).KindU.S5.Ident, ((NimStringDesc*) &TMP195416));
LA2: ;
Result_155912 = LOC1;
return Result_155912;
}
static N_INLINE(TY54525*, Lastson_54809)(TY54525* N_54811) {
TY54525* Result_56817;
NI LOC1;
Result_56817 = 0;
LOC1 = Sonslen_54803(N_54811);
Result_56817 = (*N_54811).KindU.S6.Sons->data[(NI32)(LOC1 - 1)];
return Result_56817;
}
N_NIMCALL(NimStringDesc*, Getmodulefile_155934)(TY54525* N_155936) {
NimStringDesc* Result_155937;
Result_155937 = 0;
switch ((*N_155936).Kind) {
case ((NU8) 14):
case ((NU8) 15):
case ((NU8) 16):
Result_155937 = copyString((*N_155936).KindU.S3.Strval);
break;
case ((NU8) 2):
Result_155937 = copyString((*(*N_155936).KindU.S5.Ident).S);
break;
case ((NU8) 3):
Result_155937 = copyString((*(*(*N_155936).KindU.S4.Sym).Name).S);
break;
default:
Internalerror_46571((*N_155936).Info, ((NimStringDesc*) &TMP195419));
Result_155937 = copyString(((NimStringDesc*) &TMP195158));
break;
}
return Result_155937;
}
N_NIMCALL(void, Tracedeps_155974)(TY154017* D_155976, TY54525* N_155977) {
TY51399 LOC4;
NimStringDesc* LOC5;
if (!!(((*D_155976).Section[(((NU8) 6))-0] == NIM_NIL))) goto LA2;
App_51036(&(*D_155976).Section[(((NU8) 6))-0], ((NimStringDesc*) &TMP195417));
LA2: ;
memset((void*)&LOC4, 0, sizeof(LOC4));
LOC5 = 0;
LOC5 = Getmodulefile_155934(N_155977);
LOC4[0] = Torope_51046(LOC5);
Dispa_154575(&(*D_155976).Section[(((NU8) 6))-0], ((NimStringDesc*) &TMP195418), ((NimStringDesc*) &TMP195155), LOC4, 1);
}
N_NIMCALL(void, Generatedoc_155996)(TY154017* D_155998, TY54525* N_155999) {
TY51008* LOC4;
NI I_156091;
NI HEX3Atmp_156367;
NI LOC5;
NI Res_156369;
NI I_156156;
NI HEX3Atmp_156370;
NI LOC10;
NI Res_156372;
NI I_156221;
NI HEX3Atmp_156373;
NI LOC15;
NI Res_156375;
NI I_156286;
NI HEX3Atmp_156376;
NI LOC20;
NI Res_156378;
NIM_BOOL LOC23;
TY54525* LOC26;
NI I_156342;
NI HEX3Atmp_156379;
NI LOC27;
NI Res_156381;
if (!(N_155999 == NIM_NIL)) goto LA2;
goto BeforeRet;
LA2: ;
switch ((*N_155999).Kind) {
case ((NU8) 105):
LOC4 = 0;
LOC4 = Gencomment_154663(D_155998, N_155999);
App_51031(&(*D_155998).Moddesc, LOC4);
break;
case ((NU8) 69):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 10));
break;
case ((NU8) 70):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 11));
break;
case ((NU8) 74):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 12));
break;
case ((NU8) 72):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 14));
break;
case ((NU8) 73):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 15));
break;
case ((NU8) 71):
Genitem_155000(D_155998, N_155999, (*N_155999).KindU.S6.Sons->data[0], ((NU8) 13));
break;
case ((NU8) 87):
I_156091 = 0;
HEX3Atmp_156367 = 0;
LOC5 = Sonslen_54803(N_155999);
HEX3Atmp_156367 = (NI32)(LOC5 - 1);
Res_156369 = 0;
Res_156369 = 0;
while (1) {
if (!(Res_156369 <= HEX3Atmp_156367)) goto LA6;
I_156091 = Res_156369;
if (!!(((*(*N_155999).KindU.S6.Sons->data[I_156091]).Kind == ((NU8) 105)))) goto LA8;
Genitem_155000(D_155998, (*N_155999).KindU.S6.Sons->data[I_156091], (*(*N_155999).KindU.S6.Sons->data[I_156091]).KindU.S6.Sons->data[0], ((NU8) 9));
LA8: ;
Res_156369 += 1;
} LA6: ;
break;
case ((NU8) 88):
I_156156 = 0;
HEX3Atmp_156370 = 0;
LOC10 = Sonslen_54803(N_155999);
HEX3Atmp_156370 = (NI32)(LOC10 - 1);
Res_156372 = 0;
Res_156372 = 0;
while (1) {
if (!(Res_156372 <= HEX3Atmp_156370)) goto LA11;
I_156156 = Res_156372;
if (!!(((*(*N_155999).KindU.S6.Sons->data[I_156156]).Kind == ((NU8) 105)))) goto LA13;
Genitem_155000(D_155998, (*N_155999).KindU.S6.Sons->data[I_156156], (*(*N_155999).KindU.S6.Sons->data[I_156156]).KindU.S6.Sons->data[0], ((NU8) 8));
LA13: ;
Res_156372 += 1;
} LA11: ;
break;
case ((NU8) 90):
I_156221 = 0;
HEX3Atmp_156373 = 0;
LOC15 = Sonslen_54803(N_155999);
HEX3Atmp_156373 = (NI32)(LOC15 - 1);
Res_156375 = 0;
Res_156375 = 0;
while (1) {
if (!(Res_156375 <= HEX3Atmp_156373)) goto LA16;
I_156221 = Res_156375;
if (!!(((*(*N_155999).KindU.S6.Sons->data[I_156221]).Kind == ((NU8) 105)))) goto LA18;
Genitem_155000(D_155998, (*N_155999).KindU.S6.Sons->data[I_156221], (*(*N_155999).KindU.S6.Sons->data[I_156221]).KindU.S6.Sons->data[0], ((NU8) 7));
LA18: ;
Res_156375 += 1;
} LA16: ;
break;
case ((NU8) 101):
I_156286 = 0;
HEX3Atmp_156376 = 0;
LOC20 = Sonslen_54803(N_155999);
HEX3Atmp_156376 = (NI32)(LOC20 - 1);
Res_156378 = 0;
Res_156378 = 0;
while (1) {
if (!(Res_156378 <= HEX3Atmp_156376)) goto LA21;
I_156286 = Res_156378;
Generatedoc_155996(D_155998, (*N_155999).KindU.S6.Sons->data[I_156286]);
Res_156378 += 1;
} LA21: ;
break;
case ((NU8) 83):
LOC23 = Checkforfalse_155909((*(*N_155999).KindU.S6.Sons->data[0]).KindU.S6.Sons->data[0]);
if (!!(LOC23)) goto LA24;
LOC26 = 0;
LOC26 = Lastson_54809((*N_155999).KindU.S6.Sons->data[0]);
Generatedoc_155996(D_155998, LOC26);
LA24: ;
break;
case ((NU8) 102):
I_156342 = 0;
HEX3Atmp_156379 = 0;
LOC27 = Sonslen_54803(N_155999);
HEX3Atmp_156379 = (NI32)(LOC27 - 1);
Res_156381 = 0;
Res_156381 = 0;
while (1) {
if (!(Res_156381 <= HEX3Atmp_156379)) goto LA28;
I_156342 = Res_156381;
Tracedeps_155974(D_155998, (*N_155999).KindU.S6.Sons->data[I_156342]);
Res_156381 += 1;
} LA28: ;
break;
case ((NU8) 103):
Tracedeps_155974(D_155998, (*N_155999).KindU.S6.Sons->data[0]);
break;
default:
break;
}
BeforeRet: ;
}
N_NIMCALL(TY51008*, Rendertocentry_155557)(TY154017* D_155559, TY154011 E_155560) {
TY51008* Result_155561;
TY174640 LOC1;
Result_155561 = 0;
memset((void*)&LOC1, 0, sizeof(LOC1));
LOC1[0] = E_155560.Refname;
LOC1[1] = E_155560.Header;
Result_155561 = Dispf_154559(((NimStringDesc*) &TMP195420), ((NimStringDesc*) &TMP195421), LOC1, 2);
return Result_155561;
}
N_NIMCALL(TY51008*, Rendertocentries_155566)(TY154017* D_155568, NI* J_155570, NI Lvl_155571) {
TY51008* Result_155572;
NI A_155575;
TY51008* LOC5;
TY51008* LOC8;
TY51399 LOC12;
Result_155572 = 0;
Result_155572 = NIM_NIL;
while (1) {
if (!((*J_155570) <= ((*D_155568).Tocpart->Sup.len-1))) goto LA1;
A_155575 = 0;
A_155575 = (NI32)abs((*(*D_155568).Tocpart->data[(*J_155570)].N).Level);
if (!(A_155575 == Lvl_155571)) goto LA3;
LOC5 = 0;
LOC5 = Rendertocentry_155557(D_155568, (*D_155568).Tocpart->data[(*J_155570)]);
App_51031(&Result_155572, LOC5);
(*J_155570) += 1;
goto LA2;
LA3: ;
if (!(Lvl_155571 < A_155575)) goto LA6;
LOC8 = 0;
LOC8 = Rendertocentries_155566(D_155568, J_155570, A_155575);
App_51031(&Result_155572, LOC8);
goto LA2;
LA6: ;
goto LA1;
LA2: ;
} LA1: ;
if (!(1 < Lvl_155571)) goto LA10;
memset((void*)&LOC12, 0, sizeof(LOC12));
LOC12[0] = Result_155572;
Result_155572 = Dispf_154559(((NimStringDesc*) &TMP195422), ((NimStringDesc*) &TMP195423), LOC12, 1);
LA10: ;
return Result_155572;
}
N_NIMCALL(void, Gensection_156382)(TY154017* D_156384, NU8 Kind_156385) {
TY51008* Title_156402;
NimStringDesc* LOC4;
TY161414 LOC5;
NimStringDesc* LOC6;
TY161414 LOC7;
if (!((*D_156384).Section[(Kind_156385)-0] == NIM_NIL)) goto LA2;
goto BeforeRet;
LA2: ;
Title_156402 = 0;
Title_156402 = Torope_51046(Sectionnames_156386[(Kind_156385)-6]);
LOC4 = 0;
LOC4 = Getconfigvar_45134(((NimStringDesc*) &TMP195434));
memset((void*)&LOC5, 0, sizeof(LOC5));
LOC5[0] = Torope_51049(((NI64) (Kind_156385)));
LOC5[1] = Title_156402;
LOC5[2] = Torope_51049(((NI64) ((NI32)(Kind_156385 + 50))));
LOC5[3] = (*D_156384).Section[(Kind_156385)-0];
asgnRefNoCycle((void**) &(*D_156384).Section[(Kind_156385)-0], Ropeformatnamedvars_154213(LOC4, TMP195435, 4, LOC5, 4));
LOC6 = 0;
LOC6 = Getconfigvar_45134(((NimStringDesc*) &TMP195440));
memset((void*)&LOC7, 0, sizeof(LOC7));
LOC7[0] = Torope_51049(((NI64) (Kind_156385)));
LOC7[1] = Title_156402;
LOC7[2] = Torope_51049(((NI64) ((NI32)(Kind_156385 + 50))));
LOC7[3] = (*D_156384).Toc[(Kind_156385)-0];
asgnRefNoCycle((void**) &(*D_156384).Toc[(Kind_156385)-0], Ropeformatnamedvars_154213(LOC6, TMP195435, 4, LOC7, 4));
BeforeRet: ;
}
N_NIMCALL(TY51008*, Genoutfile_156439)(TY154017* D_156441) {
TY51008* Result_156442;
TY51008* Code_156443;
TY51008* Toc_156444;
TY51008* Title_156445;
TY51008* Content_156446;
NimStringDesc* Bodyname_156447;
NI J_156448;
NU8 I_156479;
NU8 Res_156548;
NimStringDesc* LOC5;
TY51399 LOC6;
NU8 I_156507;
NU8 Res_156550;
NimStringDesc* LOC11;
NimStringDesc* LOC12;
NimStringDesc* LOC13;
NimStringDesc* LOC17;
TY162027 LOC18;
NimStringDesc* LOC19;
NimStringDesc* LOC20;
NimStringDesc* LOC24;
TY162565 LOC25;
NimStringDesc* LOC26;
NimStringDesc* LOC27;
Result_156442 = 0;
Code_156443 = 0;
Toc_156444 = 0;
Title_156445 = 0;
Content_156446 = 0;
Bodyname_156447 = 0;
J_156448 = 0;
J_156448 = 0;
Toc_156444 = Rendertocentries_155566(D_156441, &J_156448, 1);
Code_156443 = NIM_NIL;
Content_156446 = NIM_NIL;
Title_156445 = NIM_NIL;
I_156479 = 0;
Res_156548 = 0;
Res_156548 = ((NU8) 0);
while (1) {
if (!(Res_156548 <= ((NU8) 20))) goto LA1;
I_156479 = Res_156548;
Gensection_156382(D_156441, I_156479);
App_51031(&Toc_156444, (*D_156441).Toc[(I_156479)-0]);
Res_156548 += 1;
} LA1: ;
if (!!((Toc_156444 == NIM_NIL))) goto LA3;
LOC5 = 0;
LOC5 = Getconfigvar_45134(((NimStringDesc*) &TMP195441));
memset((void*)&LOC6, 0, sizeof(LOC6));
LOC6[0] = Toc_156444;
Toc_156444 = Ropeformatnamedvars_154213(LOC5, TMP195442, 1, LOC6, 1);
LA3: ;
I_156507 = 0;
Res_156550 = 0;
Res_156550 = ((NU8) 0);
while (1) {
if (!(Res_156550 <= ((NU8) 20))) goto LA7;
I_156507 = Res_156550;
App_51031(&Code_156443, (*D_156441).Section[(I_156507)-0]);
Res_156550 += 1;
} LA7: ;
if (!!(((*D_156441).Meta[(((NU8) 1))-0] == NIM_NIL))) goto LA9;
Title_156445 = (*D_156441).Meta[(((NU8) 1))-0];
goto LA8;
LA9: ;
LOC11 = 0;
LOC12 = 0;
LOC12 = nosChangeFileExt((*D_156441).Filename, ((NimStringDesc*) &TMP195158));
LOC13 = 0;
LOC13 = nosextractFilename(LOC12);
LOC11 = rawNewString(LOC13->Sup.len + 7);
appendString(LOC11, ((NimStringDesc*) &TMP195443));
appendString(LOC11, LOC13);
Title_156445 = Torope_51046(LOC11);
LA8: ;
if (!(*D_156441).Hastoc) goto LA15;
Bodyname_156447 = copyString(((NimStringDesc*) &TMP195444));
goto LA14;
LA15: ;
Bodyname_156447 = copyString(((NimStringDesc*) &TMP195445));
LA14: ;
LOC17 = 0;
LOC17 = Getconfigvar_45134(Bodyname_156447);
memset((void*)&LOC18, 0, sizeof(LOC18));
LOC18[0] = Title_156445;
LOC18[1] = Toc_156444;
LOC18[2] = (*D_156441).Moddesc;
LOC19 = 0;
LOC19 = ntgetDateStr();
LOC18[3] = Torope_51046(LOC19);
LOC20 = 0;
LOC20 = ntgetClockStr();
LOC18[4] = Torope_51046(LOC20);
LOC18[5] = Code_156443;
Content_156446 = Ropeformatnamedvars_154213(LOC17, TMP195446, 6, LOC18, 6);
if (!!(((Gglobaloptions_45078 &(1<<((((NU8) 6))&31)))!=0))) goto LA22;
LOC24 = 0;
LOC24 = Getconfigvar_45134(((NimStringDesc*) &TMP195452));
memset((void*)&LOC25, 0, sizeof(LOC25));
LOC25[0] = Title_156445;
LOC25[1] = Toc_156444;
LOC25[2] = (*D_156441).Moddesc;
LOC26 = 0;
LOC26 = ntgetDateStr();
LOC25[3] = Torope_51046(LOC26);
LOC27 = 0;
LOC27 = ntgetClockStr();
LOC25[4] = Torope_51046(LOC27);
LOC25[5] = Content_156446;
LOC25[6] = (*D_156441).Meta[(((NU8) 3))-0];
LOC25[7] = (*D_156441).Meta[(((NU8) 4))-0];
Code_156443 = Ropeformatnamedvars_154213(LOC24, TMP195453, 8, LOC25, 8);
goto LA21;
LA22: ;
Code_156443 = Content_156446;
LA21: ;
Result_156442 = Code_156443;
return Result_156442;
}
N_NIMCALL(TY51008*, Renderrstsons_155205)(TY154017* D_155207, TY146077* N_155208) {
TY51008* Result_155209;
NI I_155217;
NI HEX3Atmp_155219;
NI LOC1;
NI Res_155221;
TY51008* LOC3;
Result_155209 = 0;
I_155217 = 0;
HEX3Atmp_155219 = 0;
LOC1 = Rsonslen_146091(N_155208);
HEX3Atmp_155219 = (NI32)(LOC1 - 1);
Res_155221 = 0;
Res_155221 = 0;
while (1) {
if (!(Res_155221 <= HEX3Atmp_155219)) goto LA2;
I_155217 = Res_155221;
LOC3 = 0;
LOC3 = Renderrsttorst_155201(D_155207, (*N_155208).Sons->data[I_155217]);
App_51031(&Result_155209, LOC3);
Res_155221 += 1;
} LA2: ;
return Result_155209;
}
N_NIMCALL(TY51008*, Renderrsttorst_155201)(TY154017* D_155203, TY146077* N_155204) {
TY51008* Result_155226;
TY51008* Ind_155243;
NimStringDesc* LOC4;
NI L_155244;
TY160003 LOC5;
NimStringDesc* LOC6;
NI L_155249;
TY160003 LOC7;
NimStringDesc* LOC8;
TY174640 LOC9;
NimStringDesc* LOC10;
TY174640 LOC11;
TY174640 LOC15;
TY174640 LOC19;
TY174640 LOC20;
TY174640 LOC24;
NI L_155383;
NI LOC25;
TY161414 LOC26;
NI LOC27;
NimStringDesc* LOC28;
TY174640 LOC29;
TY51399 LOC30;
TY174640 LOC31;
TY174640 LOC32;
TY51399 LOC33;
TY51399 LOC34;
TY51399 LOC35;
TY51399 LOC36;
TY51399 LOC37;
TY51399 LOC38;
TY51399 LOC39;
NIM_BOOL LOC41;
TY174640 LOC48;
TY51399 LOC49;
Result_155226 = 0;
Result_155226 = NIM_NIL;
if (!(N_155204 == NIM_NIL)) goto LA2;
goto BeforeRet;
LA2: ;
Ind_155243 = 0;
LOC4 = 0;
LOC4 = nsuRepeatChar((*D_155203).Indent, 32);
Ind_155243 = Torope_51046(LOC4);
switch ((*N_155204).Kind) {
case ((NU8) 0):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
break;
case ((NU8) 1):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
L_155244 = 0;
L_155244 = Ropelen_51052(Result_155226);
memset((void*)&LOC5, 0, sizeof(LOC5));
LOC5[0] = Ind_155243;
LOC5[1] = Result_155226;
LOC6 = 0;
LOC6 = nsuRepeatChar(L_155244, Lvltochar_155227[((*N_155204).Level)-0]);
LOC5[2] = Torope_51046(LOC6);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195454), LOC5, 3);
break;
case ((NU8) 2):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
L_155249 = 0;
L_155249 = Ropelen_51052(Result_155226);
memset((void*)&LOC7, 0, sizeof(LOC7));
LOC7[0] = Ind_155243;
LOC7[1] = Result_155226;
LOC8 = 0;
LOC8 = nsuRepeatChar(L_155249, Lvltochar_155227[((*N_155204).Level)-0]);
LOC7[2] = Torope_51046(LOC8);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195455), LOC7, 3);
break;
case ((NU8) 3):
memset((void*)&LOC9, 0, sizeof(LOC9));
LOC9[0] = Ind_155243;
LOC10 = 0;
LOC10 = nsuRepeatChar((NI32)(78 - (*D_155203).Indent), 45);
LOC9[1] = Torope_51046(LOC10);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195456), LOC9, 2);
break;
case ((NU8) 4):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC11, 0, sizeof(LOC11));
LOC11[0] = Ind_155243;
LOC11[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195457), LOC11, 2);
break;
case ((NU8) 6):
(*D_155203).Indent += 2;
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
if (!!((Result_155226 == NIM_NIL))) goto LA13;
memset((void*)&LOC15, 0, sizeof(LOC15));
LOC15[0] = Ind_155243;
LOC15[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195458), LOC15, 2);
LA13: ;
(*D_155203).Indent -= 2;
break;
case ((NU8) 8):
(*D_155203).Indent += 4;
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
if (!!((Result_155226 == NIM_NIL))) goto LA17;
memset((void*)&LOC19, 0, sizeof(LOC19));
LOC19[0] = Ind_155243;
LOC19[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195459), LOC19, 2);
LA17: ;
(*D_155203).Indent -= 4;
break;
case ((NU8) 17):
case ((NU8) 13):
case ((NU8) 9):
case ((NU8) 10):
case ((NU8) 26):
case ((NU8) 15):
case ((NU8) 16):
case ((NU8) 37):
case ((NU8) 5):
case ((NU8) 7):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
break;
case ((NU8) 11):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC20, 0, sizeof(LOC20));
LOC20[0] = Ind_155243;
LOC20[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195457), LOC20, 2);
break;
case ((NU8) 12):
(*D_155203).Indent += 2;
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
if (!!(((*(*N_155204).Sons->data[0]).Kind == ((NU8) 5)))) goto LA22;
memset((void*)&LOC24, 0, sizeof(LOC24));
LOC24[0] = Ind_155243;
LOC24[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195460), LOC24, 2);
LA22: ;
(*D_155203).Indent -= 2;
break;
case ((NU8) 14):
Result_155226 = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[0]);
L_155383 = 0;
LOC25 = Ropelen_51052(Result_155226);
L_155383 = (((NI32)(LOC25 + 3) >= 30) ? (NI32)(LOC25 + 3) : 30);
(*D_155203).Indent += L_155383;
memset((void*)&LOC26, 0, sizeof(LOC26));
LOC26[0] = Ind_155243;
LOC26[1] = Result_155226;
LOC27 = Ropelen_51052(Result_155226);
LOC28 = 0;
LOC28 = nsuRepeatChar((NI32)((NI32)(L_155383 - LOC27) - 2), 32);
LOC26[2] = Torope_51046(LOC28);
LOC26[3] = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[1]);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195461), LOC26, 4);
(*D_155203).Indent -= L_155383;
break;
case ((NU8) 27):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC29, 0, sizeof(LOC29));
LOC29[0] = Ind_155243;
LOC29[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195462), LOC29, 2);
break;
case ((NU8) 28):
(*D_155203).Indent += 2;
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
(*D_155203).Indent -= 2;
break;
case ((NU8) 39):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC30, 0, sizeof(LOC30));
LOC30[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195463), LOC30, 1);
break;
case ((NU8) 38):
memset((void*)&LOC31, 0, sizeof(LOC31));
LOC31[0] = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[0]);
LOC31[1] = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[1]);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195464), LOC31, 2);
break;
case ((NU8) 53):
Result_155226 = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[0]);
memset((void*)&LOC32, 0, sizeof(LOC32));
LOC32[0] = Result_155226;
LOC32[1] = Renderrsttorst_155201(D_155203, (*N_155204).Sons->data[1]);
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195465), LOC32, 2);
break;
case ((NU8) 54):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC33, 0, sizeof(LOC33));
LOC33[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195466), LOC33, 1);
break;
case ((NU8) 55):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC34, 0, sizeof(LOC34));
LOC34[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195467), LOC34, 1);
break;
case ((NU8) 56):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC35, 0, sizeof(LOC35));
LOC35[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195468), LOC35, 1);
break;
case ((NU8) 57):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC36, 0, sizeof(LOC36));
LOC36[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195469), LOC36, 1);
break;
case ((NU8) 58):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC37, 0, sizeof(LOC37));
LOC37[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195470), LOC37, 1);
break;
case ((NU8) 59):
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC38, 0, sizeof(LOC38));
LOC38[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195471), LOC38, 1);
break;
case ((NU8) 60):
(*D_155203).Verbatim += 1;
Result_155226 = Renderrstsons_155205(D_155203, N_155204);
memset((void*)&LOC39, 0, sizeof(LOC39));
LOC39[0] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195472), LOC39, 1);
(*D_155203).Verbatim -= 1;
break;
case ((NU8) 62):
LOC41 = ((*D_155203).Verbatim == 0);
if (!(LOC41)) goto LA42;
LOC41 = eqStrings((*N_155204).Text, ((NimStringDesc*) &TMP195473));
LA42: ;
if (!LOC41) goto LA43;
Result_155226 = Torope_51046(((NimStringDesc*) &TMP195474));
goto LA40;
LA43: ;
Result_155226 = Torope_51046((*N_155204).Text);
LA40: ;
break;
case ((NU8) 51):
(*D_155203).Indent += 3;
if (!!(((*N_155204).Sons->data[2] == NIM_NIL))) goto LA46;
Result_155226 = Renderrstsons_155205(D_155203, (*N_155204).Sons->data[2]);
LA46: ;
(*D_155203).Indent -= 3;
memset((void*)&LOC48, 0, sizeof(LOC48));
LOC48[0] = Ind_155243;
LOC48[1] = Result_155226;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195475), LOC48, 2);
break;
case ((NU8) 44):
memset((void*)&LOC49, 0, sizeof(LOC49));
LOC49[0] = Ind_155243;
Result_155226 = Ropef_51066(((NimStringDesc*) &TMP195476), LOC49, 1);
break;
default:
Rawmessage_46557(((NU8) 191), reprEnum((*N_155204).Kind, NTI146008));
break;
}
BeforeRet: ;
return Result_155226;
}
N_NIMCALL(void, Generateindex_156551)(TY154017* D_156553) {
TY51008* LOC4;
if (!!(((*D_156553).Theindex == NIM_NIL))) goto LA2;
Sortindex_146123((*D_156553).Theindex);
LOC4 = 0;
LOC4 = Renderrsttorst_155201(D_156553, (*D_156553).Indexfile);
Writerope_51055(LOC4, Gindexfile_45082);
LA2: ;
}
N_NIMCALL(void, Commanddoc_154001)(NimStringDesc* Filename_154003) {
TY54525* Ast_156568;
NimStringDesc* LOC1;
TY154017* D_156580;
TY51008* LOC5;
NimStringDesc* LOC6;
Ast_156568 = 0;
LOC1 = 0;
LOC1 = nosaddFileExt(Filename_154003, ((NimStringDesc*) &TMP195076));
Ast_156568 = Parsefile_91031(LOC1);
if (!(Ast_156568 == NIM_NIL)) goto LA3;
goto BeforeRet;
LA3: ;
D_156580 = 0;
D_156580 = Newdocumentor_154158(Filename_154003);
Initindexfile_154128(D_156580);
(*D_156580).Hastoc = NIM_TRUE;
Generatedoc_155996(D_156580, Ast_156568);
LOC5 = 0;
LOC5 = Genoutfile_156439(D_156580);
LOC6 = 0;
LOC6 = Getoutfile_45144(Filename_154003, ((NimStringDesc*) &TMP195085));
Writerope_51055(LOC5, LOC6);
Generateindex_156551(D_156580);
BeforeRet: ;
}
N_NIMCALL(void, Commandrstaux_156581)(NimStringDesc* Filename_156583, NimStringDesc* Outext_156584) {
NimStringDesc* Filen_156585;
TY154017* D_156586;
TY146077* Rst_156588;
NimStringDesc* LOC1;
TY51008* Code_156589;
NimStringDesc* LOC2;
Filen_156585 = 0;
Filen_156585 = nosaddFileExt(Filename_156583, ((NimStringDesc*) &TMP195084));
D_156586 = 0;
D_156586 = Newdocumentor_154158(Filen_156585);
Initindexfile_154128(D_156586);
Rst_156588 = 0;
LOC1 = 0;
LOC1 = Readfile_3647(Filen_156585);
Rst_156588 = Rstparse_146082(LOC1, NIM_FALSE, Filen_156585, 0, 1, &(*D_156586).Hastoc);
asgnRefNoCycle((void**) &(*D_156586).Moddesc, Renderrsttoout_154592(D_156586, Rst_156588));
Code_156589 = 0;
Code_156589 = Genoutfile_156439(D_156586);
LOC2 = 0;
LOC2 = Getoutfile_45144(Filename_156583, Outext_156584);
Writerope_51055(Code_156589, LOC2);
Generateindex_156551(D_156586);
}
N_NIMCALL(void, Commandrst2html_154004)(NimStringDesc* Filename_154006) {
Commandrstaux_156581(Filename_154006, ((NimStringDesc*) &TMP195085));
}
N_NIMCALL(void, Commandrst2tex_154007)(NimStringDesc* Filename_154009) {
asgnRefNoCycle((void**) &Splitter_154045, copyString(((NimStringDesc*) &TMP195478)));
Commandrstaux_156581(Filename_154009, ((NimStringDesc*) &TMP195479));
}
N_NOINLINE(void, docgenInit)(void) {
asgnRefNoCycle((void**) &Splitter_154045, copyString(((NimStringDesc*) &TMP154046)));
}

